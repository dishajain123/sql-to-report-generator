...
2. Classifies the account based on overdue days and updates the asset classification and provisioning percentage.
...
6. Inserts the account's classification change into the NPA_AUDIT_LOG table.
7. Handles exceptions by logging the error in the NPA_AUDIT_LOG table.

## Business Rules

### R1 — Classify account as
...
- v_overdue_days > 1095 AND NOT (v_doubtful_since > 1095)
...
| v_overdue_days > 1095 AND NOT (v_doubtful_since > 1095) | DOUBTFUL3 |
...
- Inserts a record into the NPA_AUDIT_LOG table with the account ID, old classification, new classification, and changed on date.
...
- Inserts a record into the NPA_AUDIT_LOG table with the account ID, old classification, new classification, and changed on date, and raises the exception.
...
If the account does not exist, logs 'ACCOUNT_NOT_FOUND'. For any other exception, logs the error message and raises the exception.
