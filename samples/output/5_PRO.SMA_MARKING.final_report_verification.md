...
| Run Timestamp | `2026-09-03T03:49:58.907200+00:00` |
| Object ID | `obj_7bd00a4d3ff9` |
...
| Item | Value |
|---|---|
| Run ID | `telemetry_abb1e0625fc8` |
| Total LLM Calls | `12` |
| Successful Calls | `12` |
| Failed Calls | `0` |
| Prompt Tokens | `92274` |
| Completion Tokens | `20140` |
| Total Tokens | `112414` |
| Telemetry Availability | `available` |

| Stage | Calls | Success | Failure | Tokens | Availability |
|---|---:|---:|---:|---:|---|
| extraction | 11 | 11 | 0 | 64160 | available |
| synthesis | 1 | 1 | 0 | 48254 | available |

## Business Rule Summary

| Priority | Rule | Output | Business Purpose |
|---|---|---|---|
| 🔴 1 | Reset negative DPD to zero [CONFLICT] (`rule_3d914a7c3c49`) | `DPD_IntService, DPD_NoCredit, DPD_Overdrawn, DPD_Overdue, DPD_StockStmt` | Ensures that negative overdue days for internal service, no credit, overdrawn, overdue, and stock statement are set to zero. |
| 🔴 2 | Calculate maximum overdue days [CONFLICT] (`rule_8b5b2339842c`) | `DPD_Max` | Determines the maximum overdue days for an account based on various conditions. |
| 🔴 3 | Assign account-level SMA fields in order [CONFLICT] (`rule_00f5328f340f`) | `SMA_CLASS, SMA_REASON, SMA_DT` | The account-level SMA fields in order is assigned. |
| 🔴 4 | Update SMA movement history [CONFLICT] (`rule_5e7f62a279da`) | `Not specified` | Updates the SMA movement history for accounts. |
| 🟠 5 | Update customer movement history [LLM_ONLY] (`rule_0f9025a7c94f`) | `EffectiveToTimeKey` | Updates the movement from date and to date for customer movement history records. |
| 🔴 6 | Reset negative DPD values to zero [CONFLICT] (`rule_6097f150db38`) | `DPD_IntService, DPD_NoCredit, DPD_Overdrawn, DPD_Overdue, DPD_Renewal, DPD_StockStmt` | Negative overdue-day values are reset to zero before the maximum overdue days is calculated. |
| 🔴 7 | Propagate customer-level SMA status [CONFLICT] (`rule_17b76dad5b17`) | `FLGSMA, SMA_DT` | Propagate customer-level SMA status. |
| 🔴 8 | Assign customer movement description by key [CONFLICT] (`rule_7e6059d465d1`) | `CustMoveDescription` | Customer-level SMA status is aggregated from linked SMA-marked accounts. Customer movement descriptions are assigned from the applicable as… |
| 🔴 9 | Clear SMA fields before reprocessing [CONFLICT] (`rule_800f501f8018`) | `SMA_CLASS, SMA_REASON, SMA_DT, FLGSMA` | Existing SMA classification fields are cleared before the account is reprocessed. |
| 🟠 10 | Update DPD_Max [MATCHED] (`rule_3794267c5c42`) | `DPD_Max` | The procedure sets DPD_Max to the source-defined value. |
| 🟠 11 | Update DPD_Max [MATCHED] (`rule_515e102afe3f`) | `DPD_Max` | The procedure sets DPD_Max to the source-defined value when COALESCE(A.DPD_Overdrawn, 0) > 0 OR COALESCE(A.DPD_Overdue, 0) > 0. |
| 🟠 12 | Update SMA_CLASS [MATCHED] (`rule_046dce107da9`) | `SMA_CLASS` | The procedure sets SMA_CLASS to the source-defined value. |
| 🔴 13 | Update EffectiveToTimeKey, MovementToDate [CONFLICT] (`rule_95a6376b7d82`) | `EffectiveToTimeKey, MovementToDate` | The procedure sets EffectiveToTimeKey, MovementToDate to the source-defined value when AA.EffectiveToTimeKey = 49999 AND B.CustomerAcID IS… |
| 🔴 14 | Update EffectiveToTimeKey, MovementToDate [CONFLICT] (`rule_a6f6db894bd4`) | `EffectiveToTimeKey, MovementToDate` | The procedure sets EffectiveToTimeKey, MovementToDate to the source-defined value when AA.EffectiveToTimeKey = 49999 AND B.SourceSystemCust… |
...
- **By validation status:** insufficient_evidence = 9, verified = 5
...
- **Rule grounding coverage:** 13 / 14 (92.9%)
- **Conflicts:** 34
- **Contradictions:** 54
- **Review required items:** 92
- **Review required:** Yes
