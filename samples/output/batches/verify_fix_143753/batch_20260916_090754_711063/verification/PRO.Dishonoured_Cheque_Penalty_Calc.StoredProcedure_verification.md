...
| Run Timestamp | `2026-09-16T09:10:04.030434+00:00` |
| Object ID | `obj_4c8f8a44b029` |
...
|---|---:|---:|---:|---:|---|
| extraction | 1 | 1 | 0 | 6930 | available |
| synthesis | 6 | 6 | 0 | 64431 | available |
| synthesis_revision | 1 | 1 | 0 | 28396 | available |
...
| 🟢 5 | Insert suspension records [MATCHED] (`rule__5__10`) | `AccountId` | Insert records into the temporary table for accounts to suspend cheque books. |
| 🟢 6 | Update penalty amount [MATCHED] (`rule__1__11`) | `PenaltyAmount` | Set the penalty amount for dishonoured cheques where the penalty has not been applied. |
| 🟢 7 | Mark cheque for review [MATCHED] (`rule__2__12`) | `HoldForReview` | Mark cheques for review if the dishonour reason is not specified. |
| 🟢 8 | Apply penalty to cheque [MATCHED] (`rule__3__13`) | `PenaltyApplied` | Apply penalties to cheques where the penalty amount is not null. |
| 🟢 9 | Calculate dishonour count [MATCHED] (`rule__4__14`) | `DishonourCount` | Calculate the count of dishonoured cheques for each account within a specified lookback window. |
| 🟢 10 | Suspend cheque book [MATCHED] (`rule__7`) | `AccountId` | Determine accounts to suspend cheque books based on repeated dishonours. |
| 🟢 11 | Update outstanding balance [MATCHED] (`rule__8`) | `OutstandingBalance` | Update outstanding balances in loan accounts. |
| 🟢 12 | Log account status transition [MATCHED] (`rule__9`) | `AccountId` | Log account status transitions and process completion status. |
| 🟢 13 | Suspend cheque book [MATCHED] (`rule__5__24`) | `ChequeBookSuspended` | Suspend the cheque book for loan accounts with dishonoured cheques having a dishonour date matching the process date and a non-null penalty… |
| 🟢 14 | Suspend cheque book [MATCHED] (`rule__5__29`) | `ChequeBookSuspended` | Suspend the cheque book for accounts with a sufficient number of dishonoured cheques within the lookback window and where the cheque book i… |
| 🟢 15 | Update penalty amount [MATCHED] (`rule__1`) | `PenaltyAmount` | Calculate and update the penalty amount for each dishonoured cheque based on the provided time key. |
| 🟠 16 | Set hold for review [LLM_ONLY] (`rule__2`) | `HoldForReview` | Mark cheques that need review for penalty application. |
| 🔴 17 | Apply penalty [CONFLICT] (`rule__3`) | `PenaltyApplied` | Apply the calculated penalty to the cheques. |
| 🟠 18 | Insert into staging [LLM_ONLY] (`rule__4`) | `AccountId, PenaltyAmount, DishonourCount` | Insert records into a staging table for further processing. |
...
| 5 | Insert suspension records (rule__5__10) | INSERT INTO #NewSuspensions (AccountId) SELECT P.AccountId FROM #PenaltyStaging P INNER JOIN PRO.LoanAccountCal A ON P.AccountId = A.AccountId WHERE P.DishonourCount >= 3 AND (A.ChequeBookSuspended <> 'Y' OR A.ChequeBookSuspended IS NULL) | Not cited | 03_batch3_main_body+batch3_nested_block:embedded_07_27 | 03_batch3_main_body+batch3_nested_block:embedded_07_27 | Verified |
| 6 | Update penalty amount (rule__1__11) | D.DishonourDate = @ProcessDate AND D.PenaltyApplied = 'N' | Not cited | 03_batch3_main_body+batch3_nested_block:embedded_01_13 | 03_batch3_main_body+batch3_nested_block:embedded_01_13 | Verified |
| 7 | Mark cheque for review (rule__2__12) | D.DishonourDate = @ProcessDate AND D.DishonourReason IS NULL | Not cited | 03_batch3_main_body+batch3_nested_block:embedded_02_22 | 03_batch3_main_body+batch3_nested_block:embedded_02_22 | Verified |
| 8 | Apply penalty to cheque (rule__3__13) | D.DishonourDate = @ProcessDate AND NOT D.PenaltyAmount IS NULL | Not cited | 03_batch3_main_body+batch3_nested_block:embedded_04_24 | 03_batch3_main_body+batch3_nested_block:embedded_04_24 | Verified |
| 9 | Calculate dishonour count (rule__4__14) | D.DishonourDate = @ProcessDate AND D.PenaltyAmount IS NOT NULL | Not cited | 03_batch3_main_body+batch3_nested_block:embedded_05_25 | 03_batch3_main_body+batch3_nested_block:embedded_05_25 | Verified |
| 10 | Suspend cheque book (rule__7) | P.DishonourCount >= 3 AND (A.ChequeBookSuspended <> 'Y' OR A.ChequeBookSuspended IS NULL) | Not cited | 03_batch3_main_body+batch3_nested_block:embedded_07_27 | 03_batch3_main_body+batch3_nested_block:embedded_07_27 | Verified |
| 11 | Update outstanding balance (rule__8) | D.DishonourDate = @ProcessDate AND NOT D.PenaltyAmount IS NULL | Not cited | 03_batch3_main_body+batch3_nested_block:embedded_03_23 | 03_batch3_main_body+batch3_nested_block:embedded_03_23 | Verified |
| 12 | Log account status transition (rule__9) | AccountId IN (SELECT AccountId FROM #NewSuspensions) | Not cited | 03_batch3_main_body+batch3_nested_block:embedded_08_28 | 03_batch3_main_body+batch3_nested_block:embedded_08_28 | Verified |
| 13 | Suspend cheque book (rule__5__24) | D.DishonourDate = @ProcessDate AND D.PenaltyAmount IS NOT NULL | Not cited | 03_batch3_main_body+batch3_nested_block:embedded_03_23 | 03_batch3_main_body+batch3_nested_block:embedded_03_23 | Verified |
| 14 | Suspend cheque book (rule__5__29) | P.DishonourCount >= 3 AND (A.ChequeBookSuspended <> 'Y' OR A.ChequeBookSuspended IS NULL) | Not cited | 03_batch3_main_body+batch3_nested_block:embedded_07_27 | 03_batch3_main_body+batch3_nested_block:embedded_07_27 | Verified |
| 15 | Update penalty amount (rule__1) | D.PenaltyAmount | Not cited | Not cited | Not cited | Verified |
| 16 | Set hold for review (rule__2) | D.HoldForReview | Not cited | Not cited | Not cited | Needs Review |
| 17 | Apply penalty (rule__3) | D.PenaltyApplied | Not cited | Not cited | Not cited | Needs Review |
| 18 | Insert into staging (rule__4) | INSERT INTO #PenaltyStaging | Not cited | Not cited | Not cited | Needs Review |
...
- **Disposition:** covered_by_rule=68, technical_only=8, uncovered=30
...
- **By validation status:** unverified = 4, verified = 14
...
- **Quality score:** 84.00293650793651/100
- **Statement coverage:** 30 / 50 (60.0%)
- **Rule grounding coverage:** 16 / 18 (88.9%)
