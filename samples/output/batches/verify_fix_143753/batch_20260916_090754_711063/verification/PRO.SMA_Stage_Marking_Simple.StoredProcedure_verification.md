...
| Run Timestamp | `2026-09-16T09:09:19.920255+00:00` |
| Object ID | `obj_f88fd9f08d12` |
...
| Item | Value |
|---|---|
| Run ID | `telemetry_681dcd22908b` |
| Total LLM Calls | `8` |
| Successful Calls | `8` |
| Failed Calls | `0` |
| Prompt Tokens | `64978` |
| Completion Tokens | `10965` |
| Total Tokens | `75943` |
| Telemetry Availability | `available` |

| Stage | Calls | Success | Failure | Tokens | Availability |
|---|---:|---:|---:|---:|---|
| extraction | 1 | 1 | 0 | 5718 | available |
| synthesis | 5 | 5 | 0 | 28911 | available |
| synthesis_revision | 2 | 2 | 0 | 41314 | available |

## Business Rule Summary

| Priority | Rule | Output | Business Purpose |
|---|---|---|---|
| 🟠 1 | Determine SmaStage [MATCHED] (`deterministic_case_0027_0032_807_smastage`) | `SmaStage` | First matching row wins; ELSE includes false or NULL predicates. |
| 🟠 2 | Determine FlagSma [MATCHED] (`deterministic_case_0040_0043_1270_flagsma`) | `FlagSma` | First matching row wins; ELSE includes false or NULL predicates. |
| 🟠 3 | Determine SmaReason [MATCHED] (`deterministic_case_0050_0054_1570_smareason`) | `SmaReason` | First matching row wins; ELSE includes false or NULL predicates. |
| ⚠️ 4 | Determine SMA stage [MATCHED] (`rule__1`) | `SmaStage` | Assign an SMA stage to accounts based on their overdue days. |
| ⚠️ 5 | Set SMA flag [MATCHED] (`rule__2`) | `FlagSma` | Set the SMA flag based on the SMA stage. |
| ⚠️ 6 | Set SMA reason [CONFLICT] (`rule__3`) | `SmaReason` | Assign the SMA reason based on the facility type of the account. |
| ⚠️ 7 | Update SMA stage date [MATCHED] (`rule__4`) | `SmaStageDate` | Calculate and update the SMA stage date for accounts flagged for SMA. |
| ⚠️ 8 | Clear SMA status for non-overdue accounts [MATCHED] (`rule__5`) | `SmaStage, FlagSma, SmaReason` | For accounts that are no longer overdue, clear the SMA stage and related fields. |
| ⚠️ 9 | Increment run count [MATCHED] (`rule__6`) | `RunCount` | Increment the run count for the SMA_Stage_Marking_Simple process. |
| ⚠️ 10 | Update run status on error [MATCHED] (`rule__7`) | `COMPLETED, ErrorDate, ErrorDescription, RunCount` | If an exception occurs during the 'SMA_Stage_Marking_Simple' process, update the run status to indicate the process is not completed, recor… |
...
|---|---|---|---|---|---|---|
| 1 | Determine SmaStage (deterministic_case_0027_0032_807_smastage) | Not cited | source \| Lines 27-32 | Not cited | Not cited | Verified |
| 2 | Determine FlagSma (deterministic_case_0040_0043_1270_flagsma) | Not cited | source \| Lines 40-43 | Not cited | Not cited | Verified |
| 3 | Determine SmaReason (deterministic_case_0050_0054_1570_smareason) | Not cited | source \| Lines 50-54 | Not cited | Not cited | Verified |
| 4 | Determine SMA stage (rule__1) | UPDATE A SET A.SmaStage = (CASE WHEN A.OverdueDays BETWEEN 1 AND 30 THEN 'SMA_0' WHEN A.OverdueDays BETWEEN 31 AND 60 THEN 'SMA_1' WHEN A.OverdueDays BETWEEN 61 AND 90 THEN 'SMA_2' END) FROM PRO.AccountCal A WHERE A.OverdueDays > 0 | Not cited | Not cited | Not cited | Verified |
| 5 | Set SMA flag (rule__2) | UPDATE A SET A.FlagSma = CASE WHEN A.SmaStage IS NOT NULL THEN 'Y' ELSE 'N' END FROM PRO.AccountCal A | Not cited | Not cited | Not cited | Verified |
| 6 | Set SMA reason (rule__3) | UPDATE A SET A.SmaReason = CASE WHEN A.FacilityType IN ('CC', 'OD') THEN 'CASH CREDIT / OVERDRAFT OVERDUE' WHEN A.FacilityType IN ('TL', 'DL') THEN 'TERM LOAN OVERDUE' END FROM PRO.AccountCal A | Not cited | Not cited | Not cited | Needs Review |
| 7 | Update SMA stage date (rule__4) | UPDATE A SET A.SmaStageDate = DATEADD(DAY, -A.OverdueDays + 1, @ProcessDate) FROM PRO.AccountCal A WHERE A.FlagSma = 'Y' | Not cited | Not cited | Not cited | Verified |
| 8 | Clear SMA status for non-overdue accounts (rule__5) | UPDATE A SET A.SmaStage = NULL, A.FlagSma = 'N', A.SmaReason = NULL FROM PRO.AccountCal A WHERE A.OverdueDays = 0 | Not cited | Not cited | Not cited | Verified |
| 9 | Increment run count (rule__6) | UPDATE PRO.RunStatus SET RunCount = ISNULL(RunCount, 0) + 1 WHERE ProcessName = 'SMA_Stage_Marking_Simple' | Not cited | Not cited | Not cited | Verified |
| 10 | Update run status on error (rule__7) | UPDATE PRO.RunStatus SET COMPLETED = 'N', ErrorDate = GETDATE(), ErrorDescription = ERROR_MESSAGE(), RunCount = ISNULL(RunCount, 0) + 1 WHERE ProcessName = 'SMA_Stage_Marking_Simple' | Not cited | Not cited | Not cited | Verified |
...
- **Disposition:** covered_by_rule=49, uncovered=24
...
| UPDATE | uncovered | Lines 59-61 | 03_batch3_main_body+batch3_nested_block:chunk_text_09, 03_batch3_main_body+batch3_nested_block | UPDATE PRO.RunStatus SET COMPLETED = 'Y', ErrorDate = NULL, ErrorDescription = NULL, RunCount = ISNULL(RunCount, 0) + 1 WHERE ProcessName = 'SMA_Stage_Marking_Simple' |
...
| UPDATE | uncovered | Lines 59-61 | 03_batch3_main_body+batch3_nested_block:embedded_06_16, 03_batch3_main_body+batch3_nested_block | UPDATE PRO.RunStatus SET COMPLETED = 'Y', ErrorDate = NULL, ErrorDescription = NULL, RunCount = ISNULL(RunCount, 0) + 1 WHERE ProcessName = 'SMA_Stage_Marking_Simple' |
...
| UPDATE | uncovered | samples/02_SMA_Stage_Marking_Simple.sql | 03_batch3_main_body+batch3_nested_block:chunk_text_04, 03_batch3_main_body+batch3_nested_block:chunk_text_04, 03_batch3_main_body+batch3_nested_block | UPDATE A SET A.SmaStage = ( CASE WHEN A.OverdueDays BETWEEN 1 AND 30 THEN 'SMA_0' WHEN A.OverdueDays BETWEEN 31 AND 60 THEN 'SMA_1' WHEN A.OverdueDays BETWEEN 61 AND 90 THEN 'SMA_2' ELSE NULL END ) FROM PRO.AccountCal A WHERE A.OverdueDa... |
...
| UPDATE | uncovered | samples/02_SMA_Stage_Marking_Simple.sql | 03_batch3_main_body+batch3_nested_block:chunk_text_06, 03_batch3_main_body+batch3_nested_block:chunk_text_06, 03_batch3_main_body+batch3_nested_block | UPDATE A SET A.SmaReason = ( CASE WHEN A.FacilityType IN ('CC', 'OD') THEN 'CASH CREDIT / OVERDRAFT OVERDUE' WHEN A.FacilityType IN ('TL', 'DL') THEN 'TERM LOAN OVERDUE' ELSE 'OTHER FACILITY OVERDUE' END ) FROM PRO.AccountCal A WHERE A.F... |
...
| UPDATE | uncovered | samples/02_SMA_Stage_Marking_Simple.sql | 03_batch3_main_body+batch3_nested_block:chunk_text_09, 03_batch3_main_body+batch3_nested_block:chunk_text_09, 03_batch3_main_body+batch3_nested_block | UPDATE PRO.RunStatus SET COMPLETED = 'Y', ErrorDate = NULL, ErrorDescription = NULL, RunCount = ISNULL(RunCount, 0) + 1 WHERE ProcessName = 'SMA_Stage_Marking_Simple' |
| UPDATE | uncovered | unavailable | 03_batch3_main_body+batch3_nested_block:embedded_01_11, 03_batch3_main_body+batch3_nested_block:embedded_01_11, 03_batch3_main_body+batch3_nested_block | UPDATE A SET A.SmaStage = ( CASE WHEN A.OverdueDays BETWEEN 1 AND 30 THEN 'SMA_0' WHEN A.OverdueDays BETWEEN 31 AND 60 THEN 'SMA_1' WHEN A.OverdueDays BETWEEN 61 AND 90 THEN 'SMA_2' ELSE NULL END ) FROM PRO.AccountCal A WHERE A.OverdueDa... |
...
| UPDATE | uncovered | unavailable | 03_batch3_main_body+batch3_nested_block:embedded_03_13, 03_batch3_main_body+batch3_nested_block:embedded_03_13, 03_batch3_main_body+batch3_nested_block | UPDATE A SET A.SmaReason = ( CASE WHEN A.FacilityType IN ('CC', 'OD') THEN 'CASH CREDIT / OVERDRAFT OVERDUE' WHEN A.FacilityType IN ('TL', 'DL') THEN 'TERM LOAN OVERDUE' ELSE 'OTHER FACILITY OVERDUE' END ) FROM PRO.AccountCal A WHERE A.F... |
...
| UPDATE | uncovered | unavailable | 03_batch3_main_body+batch3_nested_block:embedded_06_16, 03_batch3_main_body+batch3_nested_block:embedded_06_16, 03_batch3_main_body+batch3_nested_block | UPDATE PRO.RunStatus SET COMPLETED = 'Y', ErrorDate = NULL, ErrorDescription = NULL, RunCount = ISNULL(RunCount, 0) + 1 WHERE ProcessName = 'SMA_Stage_Marking_Simple' |
...
| CASE_BRANCH | covered_by_rule | Lines 51-51 | unavailable | WHEN A.FacilityType IN ( , ) THEN |
| CASE_BRANCH | covered_by_rule | Lines 52-52 | unavailable | WHEN A.FacilityType IN ( , ) THEN |
...
- **By validation status:** unverified = 1, verified = 9
...
- **Quality score:** 76.59574468085106/100
- **Statement coverage:** 18 / 26 (69.2%)
- **Rule grounding coverage:** 7 / 10 (70.0%)
...
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: UPDATE A SET A.SmaStage = (CASE WHEN A.OverdueDays BETWEEN 1 AND 30 THEN 'SMA_0' WHEN A.OverdueDays BETWEEN 31 AND 60 THEN 'SMA_1' WHEN A.OverdueDays BETWEEN 61 AND 90 THEN 'SMA_2' END) FROM PRO.AccountCal A WHERE A.OverdueDays > 0 AND A.FlagSma = 'Y'
...
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: UPDATE A SET A.SmaStageDate = DATEADD(DAY, -A.OverdueDays + 1, @ProcessDate) FROM PRO.AccountCal A WHERE A.FlagSma = 'Y' AND A.OverdueDays > 0
