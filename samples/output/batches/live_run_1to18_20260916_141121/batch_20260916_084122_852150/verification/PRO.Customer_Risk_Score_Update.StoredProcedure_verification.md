...
| Run Timestamp | `2026-09-16T08:50:41.187533+00:00` |
| Object ID | `obj_9cfe9a17e845` |
...
| Item | Value |
|---|---|
| Run ID | `telemetry_bd310767f0c4` |
| Total LLM Calls | `5` |
| Successful Calls | `5` |
| Failed Calls | `0` |
| Prompt Tokens | `74741` |
| Completion Tokens | `12796` |
| Total Tokens | `87537` |
| Telemetry Availability | `available` |

| Stage | Calls | Success | Failure | Tokens | Availability |
|---|---:|---:|---:|---:|---|
| extraction | 1 | 1 | 0 | 6292 | available |
| synthesis | 2 | 2 | 0 | 25147 | available |
| synthesis_revision | 2 | 2 | 0 | 56098 | available |

## Business Rule Summary

| Priority | Rule | Output | Business Purpose |
|---|---|---|---|
| ⚠️ 1 | Calculate Utilization Ratio [MATCHED] (`rule__1`) | `UtilizationRatio` | Determine the utilization ratio for each customer by dividing their current balance by their credit limit. |
| ⚠️ 2 | Calculate Risk Score [MATCHED] (`rule__2`) | `RiskScore` | Determine the risk score for each customer based on overdue days, utilization ratio, and prior defaults. |
| ⚠️ 3 | Determine Risk Tier [CONFLICT] (`rule__3`) | `RiskTier` | Classify each customer into a risk tier based on their risk score. |
| ⚠️ 4 | Calculate Recommended Limit Adjustment Percentage [CONFLICT] (`rule__4`) | `RecommendedLimitAdjustmentPct` | Determine the recommended limit adjustment percentage for each customer based on their risk tier and prior default count. |
| ⚠️ 5 | Stage Risk Score and Risk Tier [MATCHED] (`rule__5`) | `CustomerId, RiskScore, RiskTier` | Stage the risk score and risk tier for each customer into a temporary table. |
| ⚠️ 6 | Update Risk Score History [MATCHED] (`rule__6`) | `Target.RiskScore, Target.RiskTier, Target.LastScoredDate` | Update the risk score history with the new risk scores and risk tiers. |
| ⚠️ 7 | Log Risk Score Audit [MATCHED] (`rule__7`) | `CustomerId, ScoreDate, RiskScore, RiskTier` | Log the risk score audit for each customer. |
...
|---|---|---|---|---|---|---|
| 1 | Calculate Utilization Ratio (rule__1) | UPDATE C SET C.UtilizationRatio = C.CurrentBalance / C.CreditLimit FROM PRO.CustomerRiskProfile C WHERE C.CreditLimit IS NOT NULL AND C.CreditLimit > 0 | Not cited | 03_batch3_main_body+batch3_nested_block:embedded_01_29 | Not cited | Verified |
| 2 | Calculate Risk Score (rule__2) | UPDATE C SET C.RiskScore = (ISNULL(C.OverdueDays, 0) * 0.5) + (ISNULL(C.UtilizationRatio, 0) * 100 * 0.3) + (ISNULL(C.PriorDefaultCount, 0) * 20 * 0.2) FROM PRO.CustomerRiskProfile C | Not cited | 03_batch3_main_body+batch3_nested_block:embedded_02_30 | Not cited | Verified |
| 3 | Determine Risk Tier (rule__3) | UPDATE C SET C.RiskTier = CASE WHEN C.RiskScore < 20 THEN 'LOW' WHEN C.RiskScore < 50 THEN 'MODERATE' WHEN C.RiskScore < 80 THEN 'HIGH' ELSE 'SEVERE' END FROM PRO.CustomerRiskProfile C | Not cited | 03_batch3_main_body+batch3_nested_block:embedded_03_31 | Not cited | Verified |
| 4 | Calculate Recommended Limit Adjustment Percentage (rule__4) | UPDATE C SET C.RecommendedLimitAdjustmentPct = CASE WHEN C.RiskTier = 'LOW' THEN CASE WHEN ISNULL(C.PriorDefaultCount, 0) = 0 THEN 10 ELSE 0 END WHEN C.RiskTier = 'MODERATE' THEN 0 WHEN C.RiskTier = 'HIGH' THEN -15 WHEN C.RiskTier = 'SEVERE' THEN -40 ELSE 0 E… | Not cited | 03_batch3_main_body+batch3_nested_block:embedded_04_32 | Not cited | Verified |
| 5 | Stage Risk Score and Risk Tier (rule__5) | INSERT INTO #RiskScoreStaging (CustomerId, RiskScore, RiskTier) SELECT CustomerId, RiskScore, RiskTier FROM PRO.CustomerRiskProfile WHERE RiskScore IS NOT NULL | Not cited | 03_batch3_main_body+batch3_nested_block:embedded_06_34 | Not cited | Verified |
| 6 | Update Risk Score History (rule__6) | MERGE PRO.RiskScoreHistory AS Target USING #RiskScoreStaging AS Source ON Target.CustomerId = Source.CustomerId WHEN MATCHED THEN UPDATE SET Target.RiskScore = Source.RiskScore, Target.RiskTier = Source.RiskTier, Target.LastScoredDate = @ProcessDate WHEN NOT… | Not cited | 03_batch3_main_body+batch3_nested_block:embedded_08_36 | Not cited | Verified |
| 7 | Log Risk Score Audit (rule__7) | INSERT INTO PRO.RiskScoreAuditLog (CustomerId, ScoreDate, RiskScore, RiskTier) SELECT CustomerId, @ProcessDate, RiskScore, RiskTier FROM PRO.CustomerRiskProfile WHERE PrevRiskScore IS NULL AND RiskScore IS NOT NULL | Not cited | 03_batch3_main_body+batch3_nested_block:embedded_09_37 | Not cited | Verified |
...
- `CONFLICT` tables_written (`recon_000ad3f48c41`): full_source
- `CONFLICT` rule (`recon_1d527655ec69`): rule__3, 03_batch3_main_body+batch3_nested_block, 03_batch3_main_body+batch3_nested_block:chunk_text_07;03_batch3_main_body+batch3_nested_block:chunk_text_19;03_batch3_main_body+batch3_nested_block:chunk_text_20;03_batch3_main_body+batch3_nested_block:chunk_text_21;03_batch3_main_body+batch3_nested_block:embedded_01_29;03_batch3_main_body+batch3_nested_block:embedded_03_31;03_batch3_main_body+batch3_nested_block:embedded_06_34;03_batch3_main_body+batch3_nested_block:embedded_08_36 - Deterministic evidence conflicts with the synthesized claim.
- `CONFLICT` rule (`recon_c466347812e5`): rule__4, 03_batch3_main_body+batch3_nested_block, 03_batch3_main_body+batch3_nested_block:chunk_text_10;03_batch3_main_body+batch3_nested_block:chunk_text_14;03_batch3_main_body+batch3_nested_block:embedded_01_29;03_batch3_main_body+batch3_nested_block:embedded_04_32;03_batch3_main_body+batch3_nested_block:embedded_05_33 - Deterministic evidence conflicts with the synthesized claim.

## Quality Summary

- **Overall status:** REVIEW_REQUIRED
- **Quality score:** 88.52631578947368/100
