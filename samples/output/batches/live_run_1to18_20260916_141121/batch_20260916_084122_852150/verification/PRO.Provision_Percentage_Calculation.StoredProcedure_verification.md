...
| Run Timestamp | `2026-09-16T08:42:54.109836+00:00` |
| Object ID | `obj_890a2d3ea714` |
...
| Item | Value |
|---|---|
| Run ID | `telemetry_3fcc715d05e6` |
| Total LLM Calls | `6` |
| Successful Calls | `6` |
| Failed Calls | `0` |
| Prompt Tokens | `45849` |
| Completion Tokens | `8269` |
| Total Tokens | `54118` |
| Telemetry Availability | `available` |

| Stage | Calls | Success | Failure | Tokens | Availability |
|---|---:|---:|---:|---:|---|
| extraction | 1 | 1 | 0 | 4833 | available |
| synthesis | 3 | 3 | 0 | 17113 | available |
| synthesis_revision | 2 | 2 | 0 | 32172 | available |
...
| ⚠️ 2 | Set provision percentage by asset class [LLM_ONLY] (`rule__1`) | `ProvisionPct` | Determine the provision percentage based on the asset class of the account. |
| ⚠️ 3 | Increase provision percentage for unsecured accounts [MATCHED] (`rule__2`) | `ProvisionPct` | Increase the provision percentage by 10% for unsecured accounts with non-standard asset classes. |
| ⚠️ 4 | Calculate provision amount [MATCHED] (`rule__3`) | `ProvisionAmount` | Calculate the provision amount based on the outstanding balance and provision percentage. |
...
| ⚠️ 6 | Flag accounts for senior review [MATCHED] (`rule__5`) | `SeniorReviewFlag` | Flag accounts for senior review if the provision amount exceeds 1,000,000. |
| ⚠️ 7 | Update process status [MATCHED] (`rule__6`) | `COMPLETED, ErrorDate, ErrorDescription, RunCount` | Update the process status to mark the process as completed and increment the run count. |
| ⚠️ 8 | Update run status on error [CONFLICT] (`rule__7`) | `COMPLETED, ErrorDate, ErrorDescription, RunCount` | If an error occurs during the 'Provision_Percentage_Calculation' process, update the run status to indicate the process is not completed, r… |
...
| 2 | Set provision percentage by asset class (rule__1) | UPDATE A SET A.ProvisionPct = (CASE A.AssetClass... END) FROM PRO.AccountCal A | Not cited | source_sql | Not cited | Verified |
| 3 | Increase provision percentage for unsecured accounts (rule__2) | UPDATE A SET A.ProvisionPct = A.ProvisionPct + 10.00 FROM PRO.AccountCal A WHERE A.SecuredFlag = 'N' AND A.AssetClass <> 'STANDARD' | Not cited | source_sql | Not cited | Verified |
| 4 | Calculate provision amount (rule__3) | UPDATE A SET A.ProvisionAmount = (A.OutstandingBalance * A.ProvisionPct) / 100 FROM PRO.AccountCal A WHERE A.OutstandingBalance > 0 | Not cited | source_sql | Not cited | Verified |
| 5 | Ensure provision amount does not exceed balance (rule__4) | UPDATE A SET A.ProvisionAmount = A.OutstandingBalance FROM PRO.AccountCal A WHERE A.ProvisionAmount > A.OutstandingBalance | Not cited | source_sql | Not cited | Verified |
| 6 | Flag accounts for senior review (rule__5) | UPDATE A SET A.SeniorReviewFlag = 'Y' FROM PRO.AccountCal A WHERE A.ProvisionAmount > 1000000 | Not cited | source_sql | Not cited | Verified |
| 7 | Update process status (rule__6) | UPDATE PRO.RunStatus SET COMPLETED = 'Y', ErrorDate = NULL, ErrorDescription = NULL, RunCount = ISNULL(RunCount, 0) + 1 WHERE ProcessName = 'Provision_Percentage_Calculation' | Not cited | source_sql | Not cited | Verified |
| 8 | Update run status on error (rule__7) | UPDATE PRO.RunStatus SET COMPLETED = 'N', ErrorDate = GETDATE(), ErrorDescription = ERROR_MESSAGE(), RunCount = ISNULL(RunCount, 0) + 1 WHERE ProcessName = 'Provision_Percentage_Calculation' | Not cited | source_sql | Not cited | Verified |
...
- `LLM_ONLY` rule (`recon_96eaf1bbbf03`): rule__1, source_sql - No deterministic evidence was found for this claim.
- `CONFLICT` rule (`recon_03933e5e929d`): rule__7, source_sql, 03_batch3_main_body+batch3_nested_block:chunk_text_08;03_batch3_main_body+batch3_nested_block:embedded_06_15;04_batch3_exception:chunk_text_02;04_batch3_exception:embedded_01_05 - Deterministic evidence conflicts with the synthesized claim.
- `DETERMINISTIC_ONLY` coverage (`recon_8ad02b68a123`): 03_batch3_main_body+batch3_nested_block, 03_batch3_main_body+batch3_nested_block:chunk_text_03 - Deterministic evidence is present in the source but no synthesized rule referenced it.
- `DETERMINISTIC_ONLY` coverage (`recon_8ad02b68a123`): 03_batch3_main_body+batch3_nested_block, 03_batch3_main_body+batch3_nested_block:chunk_text_04 - Deterministic evidence is present in the source but no synthesized rule referenced it.

## Quality Summary

- **Overall status:** REVIEW_REQUIRED
- **Quality score:** 87.86184210526315/100
