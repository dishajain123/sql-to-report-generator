...
| Run Timestamp | `2026-09-16T08:47:37.302300+00:00` |
| Object ID | `obj_8ece2542e210` |
...
| Item | Value |
|---|---|
| Run ID | `telemetry_c78711129246` |
| Total LLM Calls | `6` |
| Successful Calls | `6` |
| Failed Calls | `0` |
| Prompt Tokens | `72030` |
| Completion Tokens | `11834` |
| Total Tokens | `83864` |
| Telemetry Availability | `available` |

| Stage | Calls | Success | Failure | Tokens | Availability |
|---|---:|---:|---:|---:|---|
| extraction | 1 | 1 | 0 | 6274 | available |
| synthesis | 3 | 3 | 0 | 27913 | available |
| synthesis_revision | 2 | 2 | 0 | 49677 | available |

## Business Rule Summary

| Priority | Rule | Output | Business Purpose |
|---|---|---|---|
| ⚠️ 1 | Calculate coverage ratio [MATCHED] (`rule__2`) | `CoverageRatio` | Calculate the coverage ratio for accounts with non-null outstanding balances. |
| ⚠️ 2 | Conditionally append to review reason [MATCHED] (`rule__4`) | `ReviewReason` | Conditionally append '_QUARTER_END' to the review reason if the process date is within a certain range. |
| ⚠️ 3 | Insert into collections queue [CONFLICT] (`rule__7`) | `AccountId, EscalationDate, Reason` | Insert data into the PRO.CollectionsQueue table based on the coverage ratio and review reason. |
...
| ⚠️ 5 | Insert account data [CONFLICT] (`rule__1`) | `AccountId, OutstandingBalance, ProvisionAmount, CoverageRatio, ReviewReason` | Insert account data into the #ProvisionCoverage temporary table from the PRO.LoanAccountCal table. |
| ⚠️ 6 | Update review reason [MATCHED] (`rule__3`) | `ReviewReason` | Update the review reason based on the coverage ratio. |
...
|---|---|---|---|---|---|---|
| 1 | Calculate coverage ratio (rule__2) | CASE WHEN A.OutstandingBalance > 0 THEN A.ProvisionAmount / A.OutstandingBalance ELSE NULL END | Not cited | 03_batch3_main_body+batch3_nested_block:chunk_text_08 | dependency_0004; dependency_0005; dependency_0006; dependency_0007 | Verified |
| 2 | Conditionally append to review reason (rule__4) | UPDATE S SET S.ReviewReason = S.ReviewReason + '_QUARTER_END' FROM #ProvisionCoverage S WHERE S.CoverageRatio IS NOT NULL AND S.CoverageRatio < 0.6 | Not cited | 03_batch3_main_body+batch3_nested_block:chunk_text_18 | dependency_0004; dependency_0005; dependency_0006; dependency_0007 | Verified |
| 3 | Insert into collections queue (rule__7) | INSERT INTO PRO.CollectionsQueue (AccountId, EscalationDate, Reason) SELECT AccountId, @ProcessDate, CASE WHEN CoverageRatio < 0.25 THEN 'SEVERE_PROVISION_SHORTFALL' ELSE 'MODERATE_PROVISION_SHORTFALL' END FROM #ProvisionCoverage WHERE ReviewReason LIKE '%UND… | Not cited | 03_batch3_main_body+batch3_nested_block:embedded_08_31 | dependency_0004; dependency_0005; dependency_0006; dependency_0007 | Verified |
| 4 | Determine Reason (deterministic_decision_4861_5218_2_reason) | Not cited | source \| Lines 117-124 | Not cited | Not cited | Verified |
| 5 | Insert account data (rule__1) | INSERT INTO #ProvisionCoverage (AccountId, OutstandingBalance, ProvisionAmount, CoverageRatio, ReviewReason) SELECT A.AccountId, A.OutstandingBalance, A.ProvisionAmount, CASE WHEN A.OutstandingBalance > 0 THEN A.ProvisionAmount / A.OutstandingBalance ELSE NUL… | Not cited | Not cited | Not cited | Needs Review |
| 6 | Update review reason (rule__3) | CASE WHEN S.CoverageRatio IS NULL THEN 'NO_BALANCE' WHEN S.CoverageRatio < 0.25 THEN 'SEVERE_UNDERCOVER' WHEN S.CoverageRatio < 0.5 THEN 'MODERATE_UNDERCOVER' WHEN S.CoverageRatio >= 0.5 AND S.CoverageRatio < 1 THEN 'ADEQUATE' ELSE 'FULLY_COVERED' END | Not cited | Not cited | Not cited | Verified |
...
- **By validation status:** unverified = 2, verified = 6
...
- **Quality score:** 85.77954545454544/100
