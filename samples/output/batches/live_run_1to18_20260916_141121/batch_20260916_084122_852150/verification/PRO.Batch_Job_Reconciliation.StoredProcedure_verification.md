...
| Run Timestamp | `2026-09-16T08:57:10.405678+00:00` |
| Object ID | `obj_4117700b7512` |
...
| Item | Value |
|---|---|
| Run ID | `telemetry_031b9f5ab5ff` |
| Total LLM Calls | `6` |
| Successful Calls | `6` |
| Failed Calls | `0` |
| Prompt Tokens | `79810` |
| Completion Tokens | `12154` |
| Total Tokens | `91964` |
| Telemetry Availability | `available` |

| Stage | Calls | Success | Failure | Tokens | Availability |
|---|---:|---:|---:|---:|---|
| extraction | 1 | 1 | 0 | 6694 | available |
| synthesis | 3 | 3 | 0 | 28077 | available |
| synthesis_revision | 2 | 2 | 0 | 57193 | available |

## Business Rule Summary

| Priority | Rule | Output | Business Purpose |
|---|---|---|---|
| 🟠 1 | Determine Outcome [MATCHED] (`deterministic_decision_1739_2891_1_outcome`) | `Outcome` | First matching row wins. SQL type conversion still applies; ELSE includes false or NULL predicates. |
| 🟠 2 | Determine ShortfallPct [MATCHED] (`deterministic_decision_1739_2891_2_shortfallpct`) | `ShortfallPct` | First matching row wins. SQL type conversion still applies; ELSE includes false or NULL predicates. |
| ⚠️ 3 | Insert reconciliation results [CONFLICT] (`rule__1`) | `Outcome, FeedName, ShortfallPct, SeverityTier, ReconciledOn` | Inserts reconciliation results into the temporary table based on the feed data for the specified time key. |
| ⚠️ 4 | Update retry count [MATCHED] (`rule__2__3`) | `RetryCount` | Updates the RetryCount in PRO.BatchFeedRegistry to 1 for feeds marked as 'RETRY_QUEUED'. |
| ⚠️ 5 | Update severity tier [LLM_ONLY] (`rule__3`) | `SeverityTier` | Updates the SeverityTier in #ReconciliationResults based on the outcome of the feed. |
| ⚠️ 6 | Merge reconciliation summary [LLM_ONLY] (`rule__4`) | `Outcome, FeedName, ShortfallPct, SeverityTier, LastReconciledDate` | Merges reconciliation summary data into the BatchReconciliationSummary table. |
| ⚠️ 7 | Insert escalation records [MATCHED] (`rule__7`) | `Reason, AccountId, EscalationDate` | Inserts escalation records into CollectionsQueue for feeds with a 'CRITICAL' severity tier. |
...
|---|---|---|---|---|---|---|
| 1 | Determine Outcome (deterministic_decision_1739_2891_1_outcome) | Not cited | source \| Lines 46-70 | Not cited | Not cited | Verified |
| 2 | Determine ShortfallPct (deterministic_decision_1739_2891_2_shortfallpct) | Not cited | source \| Lines 46-70 | Not cited | Not cited | Verified |
| 3 | Insert reconciliation results (rule__1) | INSERT INTO #ReconciliationResults (FeedName, Outcome, ShortfallPct, SeverityTier, ReconciledOn) SELECT FeedName, CASE... END, CASE... END, NULL, @ProcessDate FROM PRO.BatchFeedRegistry WHERE FeedDate = @ProcessDate | Not cited | Not cited | Not cited | Needs Review |
| 4 | Update retry count (rule__2__3) | UPDATE PRO.BatchFeedRegistry SET RetryCount = 1 WHERE FeedDate = @ProcessDate AND FeedName IN (SELECT FeedName FROM #ReconciliationResults WHERE Outcome = 'RETRY_QUEUED') | Not cited | Not cited | Not cited | Verified |
| 5 | Update severity tier (rule__3) | UPDATE R SET R.SeverityTier = CASE... END FROM #ReconciliationResults R | Not cited | Not cited | Not cited | Needs Review |
| 6 | Merge reconciliation summary (rule__4) | MERGE PRO.BatchReconciliationSummary AS Target USING #ReconciliationResults AS Source ON Target.FeedName = Source.FeedName WHEN MATCHED THEN UPDATE SET Target.Outcome = Source.Outcome, Target.ShortfallPct = Source.ShortfallPct, Target.SeverityTier = Source.Se… | Not cited | Not cited | Not cited | Needs Review |
| 7 | Insert escalation records (rule__7) | INSERT INTO PRO.CollectionsQueue (AccountId, EscalationDate, Reason) SELECT FeedName, ReconciledOn, CASE WHEN SeverityTier = 'CRITICAL' THEN 'FEED_RECONCILIATION_CRITICAL' ELSE 'FEED_RECONCILIATION_FAILED' END FROM #ReconciliationResults WHERE Outcome = 'FAIL… | Not cited | Not cited | Not cited | Verified |
...
| decision_chain_006:branch_001 | Outcome = 'RECONCILED' | source |
| decision_chain_006:branch_002 | Outcome = 'NOT_APPLICABLE' | source |
| decision_chain_006:branch_003 | Outcome = 'RETRY_QUEUED' | source |
| decision_chain_006:branch_004 | Outcome = 'FAILED' AND ShortfallPct > 25 | source |
| decision_chain_006:branch_005 | Outcome = 'FAILED' | source |
...
- **Disposition:** covered_by_rule=56, technical_only=8, uncovered=46
...
| STATEMENT | uncovered | Lines 7-7 | 03_batch3_main_body+batch3_nested_block:chunk_text_04, 03_batch3_main_body+batch3_nested_block | DECLARE @MonthEndDate DATE = EOMONTH(@ProcessDate) |
| STATEMENT | uncovered | Lines 9-9 | 03_batch3_main_body+batch3_nested_block:chunk_text_05, 03_batch3_main_body+batch3_nested_block | IF OBJECT_ID('tempdb..#ReconciliationResults') IS NOT NULL |
| STATEMENT | uncovered | Lines 10-10 | 03_batch3_main_body+batch3_nested_block:chunk_text_06, 03_batch3_main_body+batch3_nested_block | DROP TABLE #ReconciliationResults |
| STATEMENT | uncovered | Lines 12-27 | 03_batch3_main_body+batch3_nested_block:chunk_text_07, 03_batch3_main_body+batch3_nested_block | CREATE TABLE #ReconciliationResults ( FeedName VARCHAR(50), Outcome VARCHAR(20), ShortfallPct DECIMAL(9,4), SeverityTier VARCHAR(10), ReconciledOn DATE ) -- Rule 1: NULL check - a feed with no expected count on record -- cannot be reconc... |
...
| UPDATE | uncovered | Lines 91-98 | 03_batch3_main_body+batch3_nested_block:chunk_text_12, 03_batch3_main_body+batch3_nested_block | UPDATE T SET T.SeverityTier = 'CRITICAL' FROM PRO.BatchReconciliationSummary T WHERE T.Outcome <> 'RECONCILED' AND T.FirstReconciledDate <= DATEADD(DAY, -14, @ProcessDate) -- Rule 6: persist every outcome computed above into the log, --... |
| INSERT | uncovered | Lines 99-104 | 03_batch3_main_body+batch3_nested_block:chunk_text_13, 03_batch3_main_body+batch3_nested_block | INSERT INTO PRO.BatchReconciliationLog (FeedName, Outcome, ReconciledOn) SELECT R.FeedName, R.Outcome, R.ReconciledOn FROM #ReconciliationResults R -- Rule 7: status transition and audit - failed feeds are -- escalated to the collections... |
...
| UPDATE | uncovered | Lines 111-113 | 03_batch3_main_body+batch3_nested_block:chunk_text_15, 03_batch3_main_body+batch3_nested_block | UPDATE PRO.ACLRUNNINGPROCESSSTATUS SET COMPLETED = 'Y', ERRORDATE = NULL, ERRORDESCRIPTION = NULL, COUNT = ISNULL(COUNT, 0) + 1 WHERE RUNNINGPROCESSNAME = 'Batch_Job_Reconciliation' |
| STATEMENT | uncovered | Lines 115-115 | 03_batch3_main_body+batch3_nested_block:chunk_text_16, 03_batch3_main_body+batch3_nested_block | END TRY |
...
| UPDATE | uncovered | Lines 91-98 | 03_batch3_main_body+batch3_nested_block:embedded_05_21, 03_batch3_main_body+batch3_nested_block | UPDATE T SET T.SeverityTier = 'CRITICAL' FROM PRO.BatchReconciliationSummary T WHERE T.Outcome <> 'RECONCILED' AND T.FirstReconciledDate <= DATEADD(DAY, -14, @ProcessDate) -- Rule 6: persist every outcome computed above into the log, --... |
| INSERT | uncovered | Lines 99-104 | 03_batch3_main_body+batch3_nested_block:embedded_06_22, 03_batch3_main_body+batch3_nested_block | INSERT INTO PRO.BatchReconciliationLog (FeedName, Outcome, ReconciledOn) SELECT R.FeedName, R.Outcome, R.ReconciledOn FROM #ReconciliationResults R -- Rule 7: status transition and audit - failed feeds are -- escalated to the collections... |
...
| UPDATE | uncovered | Lines 111-113 | 03_batch3_main_body+batch3_nested_block:embedded_08_24, 03_batch3_main_body+batch3_nested_block | UPDATE PRO.ACLRUNNINGPROCESSSTATUS SET COMPLETED = 'Y', ERRORDATE = NULL, ERRORDESCRIPTION = NULL, COUNT = ISNULL(COUNT, 0) + 1 WHERE RUNNINGPROCESSNAME = 'Batch_Job_Reconciliation' |
...
| UPDATE | uncovered | samples/18_Batch_Job_Reconciliation.sql | 03_batch3_main_body+batch3_nested_block:chunk_text_12, 03_batch3_main_body+batch3_nested_block:chunk_text_12, 03_batch3_main_body+batch3_nested_block | UPDATE T SET T.SeverityTier = 'CRITICAL' FROM PRO.BatchReconciliationSummary T WHERE T.Outcome <> 'RECONCILED' AND T.FirstReconciledDate <= DATEADD(DAY, -14, @ProcessDate) -- Rule 6: persist every outcome computed above into the log, --... |
| INSERT | uncovered | samples/18_Batch_Job_Reconciliation.sql | 03_batch3_main_body+batch3_nested_block:chunk_text_13, 03_batch3_main_body+batch3_nested_block:chunk_text_13, 03_batch3_main_body+batch3_nested_block | INSERT INTO PRO.BatchReconciliationLog (FeedName, Outcome, ReconciledOn) SELECT R.FeedName, R.Outcome, R.ReconciledOn FROM #ReconciliationResults R -- Rule 7: status transition and audit - failed feeds are -- escalated to the collections... |
...
| UPDATE | uncovered | samples/18_Batch_Job_Reconciliation.sql | 03_batch3_main_body+batch3_nested_block:chunk_text_15, 03_batch3_main_body+batch3_nested_block:chunk_text_15, 03_batch3_main_body+batch3_nested_block | UPDATE PRO.ACLRUNNINGPROCESSSTATUS SET COMPLETED = 'Y', ERRORDATE = NULL, ERRORDESCRIPTION = NULL, COUNT = ISNULL(COUNT, 0) + 1 WHERE RUNNINGPROCESSNAME = 'Batch_Job_Reconciliation' |
| READ | uncovered | unavailable | 03_batch3_main_body+batch3_nested_block:embedded_01_17, 03_batch3_main_body+batch3_nested_block:embedded_01_17, 03_batch3_main_body+batch3_nested_block | INSERT INTO #ReconciliationResults (FeedName, Outcome, ShortfallPct, SeverityTier, ReconciledOn) SELECT FeedName, CASE WHEN ExpectedRowCount IS NULL OR ExpectedRowCount = 0 THEN 'NOT_APPLICABLE' WHEN ActualRowCount >= ExpectedRowCount TH... |
...
| UPDATE | uncovered | unavailable | 03_batch3_main_body+batch3_nested_block:embedded_05_21, 03_batch3_main_body+batch3_nested_block:embedded_05_21, 03_batch3_main_body+batch3_nested_block | UPDATE T SET T.SeverityTier = 'CRITICAL' FROM PRO.BatchReconciliationSummary T WHERE T.Outcome <> 'RECONCILED' AND T.FirstReconciledDate <= DATEADD(DAY, -14, @ProcessDate) -- Rule 6: persist every outcome computed above into the log, --... |
...
| UPDATE | uncovered | unavailable | 03_batch3_main_body+batch3_nested_block:embedded_08_24, 03_batch3_main_body+batch3_nested_block:embedded_08_24, 03_batch3_main_body+batch3_nested_block | UPDATE PRO.ACLRUNNINGPROCESSSTATUS SET COMPLETED = 'Y', ERRORDATE = NULL, ERRORDESCRIPTION = NULL, COUNT = ISNULL(COUNT, 0) + 1 WHERE RUNNINGPROCESSNAME = 'Batch_Job_Reconciliation' |
...
| IF_BRANCH | covered_by_rule | samples/18_Batch_Job_Reconciliation.sql | full_source | Outcome = 'RECONCILED' |
| IF_BRANCH | covered_by_rule | samples/18_Batch_Job_Reconciliation.sql | full_source | Outcome = 'NOT_APPLICABLE' |
| IF_BRANCH | covered_by_rule | samples/18_Batch_Job_Reconciliation.sql | full_source | Outcome = 'RETRY_QUEUED' |
| IF_BRANCH | uncovered | samples/18_Batch_Job_Reconciliation.sql | full_source | Outcome = 'FAILED' AND ShortfallPct > 25 |
| IF_BRANCH | covered_by_rule | samples/18_Batch_Job_Reconciliation.sql | full_source | Outcome = 'FAILED' |
| ELSE | covered_by_rule | samples/18_Batch_Job_Reconciliation.sql | full_source | ELSE |
...
| CASE | uncovered | Lines 125-125 | unavailable | CASE WHEN SeverityTier = THEN ELSE END |
| CASE_BRANCH | uncovered | Lines 125-125 | unavailable | CASE WHEN SeverityTier = THEN ELSE END |
| ELSE | uncovered | Lines 125-125 | unavailable | CASE WHEN SeverityTier = THEN ELSE END |
...
- **By rule type:** deterministic_decision_table = 2, explicit = 5
- **By validation status:** unverified = 3, verified = 4
...
- `LLM_ONLY` tables_written (`recon_dc5de928af01`): full_source
- `CONFLICT` rule (`recon_e8aedcc4c5af`): rule__1, 03_batch3_main_body+batch3_nested_block:embedded_01_17 - Deterministic evidence conflicts with the synthesized claim.
- `LLM_ONLY` rule (`recon_ff358d431543`): rule__3 - No deterministic evidence was found for this claim.

## Quality Summary

- **Overall status:** REVIEW_REQUIRED
- **Quality score:** 76.46666666666667/100
...
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: UPDATE #ReconciliationResults SET R.SeverityTier = CASE WHEN ShortfallPct > 50 THEN 'CRITICAL' WHEN ShortfallPct > 20 AND ShortfallPct <= 50 THEN 'HIGH' WHEN ShortfallPct > 10 AND ShortfallPct <= 20 THEN 'MEDIUM' ELSE 'LOW' END
