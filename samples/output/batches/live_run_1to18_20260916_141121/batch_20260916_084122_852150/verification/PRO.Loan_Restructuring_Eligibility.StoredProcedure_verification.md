...
| Run Timestamp | `2026-09-16T08:46:46.250585+00:00` |
| Object ID | `obj_e4fc72fc188b` |
...
| Item | Value |
|---|---|
| Run ID | `telemetry_76ad9e212e5b` |
| Total LLM Calls | `5` |
| Successful Calls | `5` |
| Failed Calls | `0` |
| Prompt Tokens | `49124` |
| Completion Tokens | `11267` |
| Total Tokens | `60391` |
| Telemetry Availability | `available` |

| Stage | Calls | Success | Failure | Tokens | Availability |
|---|---:|---:|---:|---:|---|
| extraction | 1 | 1 | 0 | 6967 | available |
| synthesis | 3 | 3 | 0 | 27041 | available |
| synthesis_revision | 1 | 1 | 0 | 26383 | available |

## Business Rule Summary

| Priority | Rule | Output | Business Purpose |
|---|---|---|---|
| 🔴 1 | Determine RestructureEligible status [CONFLICT] (`rule__2__11`) | `RestructureEligible` | Set the RestructureEligible status of a loan based on specific conditions. |
| 🟠 2 | Determine RestructureEligible [MATCHED] (`deterministic_decision_1241_2314_0_restructureeligible`) | `RestructureEligible` | First matching row wins. SQL type conversion still applies; ELSE includes false or NULL predicates. |
| 🟠 3 | Update RestructureEligible 1 [LLM_ONLY] (`rule__1`) | `RestructureEligible` | Set the RestructureEligible field to 'Y' for loans that meet the first set of conditions. |
| 🟠 4 | Update RestructureEligible 2 [LLM_ONLY] (`rule__2`) | `RestructureEligible` | Set the RestructureEligible field to 'N' for loans that meet the second set of conditions. |
| 🟠 5 | Update RestructureEligible 3 [LLM_ONLY] (`rule__3`) | `RestructureEligible` | Set the RestructureEligible field to 'Y' for loans that meet the third set of conditions. |
| 🟠 6 | Update RevisedTenureMonths [LLM_ONLY] (`rule__5`) | `RevisedTenureMonths` | Update the RevisedTenureMonths field in the #RestructureDecisions table based on certain conditions. |
| 🟠 7 | Determine restructuring eligibility [LLM_ONLY] (`rule__1__5`) | `RestructureEligible` | Set the restructuring eligibility for loan accounts based on the process date, asset class, overdue days, and prior restructure count. |
| 🟠 8 | Set restructuring eligibility to 'SCHEME_CLOSED' [LLM_ONLY] (`rule__2__6`) | `RestructureEligible` | Set the restructuring eligibility for loan accounts to 'SCHEME_CLOSED' if the process date is on or after the scheme cutoff date. |
| 🟢 9 | Set restructuring eligibility to 'NOT_ASSESSED' [MATCHED] (`rule__3__7`) | `RestructureEligible` | Set the restructuring eligibility for loan accounts to 'NOT_ASSESSED' if the outstanding balance is null. |
| 🔴 10 | Update revised tenure months [CONFLICT] (`rule__4`) | `RevisedTenureMonths` | Update the revised tenure months for eligible accounts based on the outstanding balance. |
| 🔴 11 | Update account status and last payment due date [CONFLICT] (`rule__5__9`) | `AccountStatus, LastPaymentDueDate, PriorRestructureCount` | Update the account status, last payment due date, and prior restructure count for eligible accounts. |
| 🔴 12 | Insert into restructuring audit log [CONFLICT] (`rule__6`) | `AccountId, DecisionDate, EligibleFlag` | Insert restructuring decisions into the restructuring audit log for accounts evaluated on the process date. |
...
|---|---|---|---|---|---|---|
| 1 | Determine RestructureEligible status (rule__2__11) | UPDATE PRO.LoanAccountCal SET A.RestructureEligible = 'Y' WHERE <condition> | Not cited | 03_batch3_main_body+batch3_nested_block:embedded_01_24; 03_batch3_main_body+batch3_nested_block:embedded_02_25; 03_batch3_main_body+batch3_nested_block:embedded_03_26 | dependency_0001; dependency_0002; dependency_0003; dependency_0004 | Needs Review |
| 2 | Determine RestructureEligible (deterministic_decision_1241_2314_0_restructureeligible) | Not cited | source \| Lines 33-55 | Not cited | Not cited | Verified |
| 3 | Update RestructureEligible 1 (rule__1) | A.AssetClass = 'Commercial' AND A.Balance > 1000000 AND A.TenureMonths > 60 | Not cited | Not cited | Not cited | Needs Review |
| 4 | Update RestructureEligible 2 (rule__2) | A.AssetClass = 'Residential' AND A.Balance > 500000 AND A.TenureMonths > 36 | Not cited | Not cited | Not cited | Needs Review |
| 5 | Update RestructureEligible 3 (rule__3) | A.AssetClass = 'Commercial' AND A.Balance > 500000 AND A.TenureMonths > 24 | Not cited | Not cited | Not cited | Needs Review |
| 6 | Update RevisedTenureMonths (rule__5) | D.AssetClassAtEval = 'Commercial' AND D.RevisedTenureMonths < 60 | Not cited | Not cited | Not cited | Needs Review |
| 7 | Determine restructuring eligibility (rule__1__5) | UPDATE A SET A.RestructureEligible = CASE... END FROM PRO.LoanAccountCal A | Not cited | Not cited | Not cited | Needs Review |
| 8 | Set restructuring eligibility to 'SCHEME_CLOSED' (rule__2__6) | UPDATE A SET A.RestructureEligible = 'SCHEME_CLOSED' FROM PRO.LoanAccountCal A | Not cited | Not cited | Not cited | Needs Review |
| 9 | Set restructuring eligibility to 'NOT_ASSESSED' (rule__3__7) | UPDATE A SET A.RestructureEligible = 'NOT_ASSESSED' FROM PRO.LoanAccountCal A WHERE A.OutstandingBalance IS NULL | Not cited | Not cited | Not cited | Verified |
| 10 | Update revised tenure months (rule__4) | UPDATE D SET D.RevisedTenureMonths = CASE... END FROM #RestructureDecisions D INNER JOIN PRO.LoanAccountCal A ON A.AccountId = D.AccountId WHERE D.EligibleFlag = 'Y' | Not cited | Not cited | Not cited | Needs Review |
| 11 | Update account status and last payment due date (rule__5__9) | UPDATE A SET A.AccountStatus = 'RESTRUCTURED', A.LastPaymentDueDate = DATEADD(MONTH, D.RevisedTenureMonths, @ProcessDate), A.PriorRestructureCount = ISNULL(A.PriorRestructureCount, 0) + 1 FROM PRO.LoanAccountCal A INNER JOIN #RestructureDecisions D ON D.Accou… | Not cited | Not cited | Not cited | Needs Review |
| 12 | Insert into restructuring audit log (rule__6) | INSERT INTO PRO.RestructureAuditLog (AccountId, DecisionDate, EligibleFlag) SELECT R.AccountId, R.LastEvaluatedDate, R.EligibleFlag FROM PRO.RestructureRegister R WHERE R.LastEvaluatedDate = @ProcessDate AND R.EligibleFlag = 'Y' | Not cited | Not cited | Not cited | Needs Review |
...
- **Disposition:** covered_by_rule=78, technical_only=5, uncovered=22
...
- **By rule type:** deterministic_decision_table = 1, explicit = 11
- **By validation status:** unverified = 10, verified = 2
...
- **Quality score:** 73.38461538461539/100
- **Statement coverage:** 26 / 44 (59.1%)
- **Rule grounding coverage:** 5 / 12 (41.7%)
...
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: A.AssetClass = 'Commercial' AND A.Balance > 1000000 AND A.TenureMonths > 60
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: A.AssetClass = 'Residential' AND A.Balance > 500000 AND A.TenureMonths > 36
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: A.AssetClass = 'Commercial' AND A.Balance > 500000 AND A.TenureMonths > 24
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: D.AssetClassAtEval = 'Commercial' AND D.RevisedTenureMonths < 60
...
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: UPDATE PRO.LoanAccountCal SET A.RestructureEligible = 'Y' WHERE <condition>
