...
| Run Timestamp | `2026-09-16T08:35:34.649407+00:00` |
| Object ID | `obj_6f3adb81cb4c` |
...
|---|---:|---:|---:|---:|---|
| extraction | 1 | 1 | 0 | 5172 | available |
| synthesis | 3 | 3 | 0 | 20266 | available |

## Business Rule Summary

| Priority | Rule | Output | Business Purpose |
|---|---|---|---|
| 🔴 1 | Insert asset class movements [CONFLICT] (`rule__1`) | `AssetClassMovementHistory, TimeKey, AccountId, PreviousClass, CurrentClass, MovementDate` | Records asset class movements for accounts into PRO.AssetClassMovementHistory. |
| 🔴 2 | Insert customer class movements [CONFLICT] (`rule__2`) | `CustomerClassMovementHistory, TimeKey, CustomerId, WorstClass, MovementDate` | Records customer class movements into PRO.CustomerClassMovementHistory. |
| 🟠 3 | Truncate previous asset class [LLM_ONLY] (`rule__3`) | `Not specified` | Clears previous asset class data in PRO.PreviousAssetClass. |
| 🟢 4 | Update movement count [MATCHED] (`rule__5`) | `MovementCount` | Increments the movement count in PRO.RunStatistics. |
| 🟢 5 | Flag multi-account movements [MATCHED] (`rule__6`) | `MultiAccountMovementFlag` | Flags customers with more than one account movement in PRO.CustomerCal. |
| 🔴 6 | Update run status [CONFLICT] (`rule__7`) | `RunStatus, COMPLETED, ErrorDate, ErrorDescription, RunCount` | Marks the run as completed and increments the run count in PRO.RunStatus. |
| 🟢 7 | Update run status on error [MATCHED] (`rule__1__7`) | `COMPLETED, ErrorDate, ErrorDescription, RunCount` | If an exception occurs during the NPA_Movement_Audit_Log process, the run status is updated to indicate the process did not complete succes… |
...
|---|---|---|---|---|---|---|
| 1 | Insert asset class movements (rule__1) | INSERT INTO PRO.AssetClassMovementHistory (TimeKey, AccountId, PreviousClass, CurrentClass, MovementDate) SELECT @TimeKey, B.AccountId, A.AssetClass, B.AssetClass, GETDATE() FROM PRO.PreviousAssetClass A RIGHT OUTER JOIN PRO.AccountCal B ON A.AccountId = B.Ac… | Not cited | Not cited | Not cited | Needs Review |
| 2 | Insert customer class movements (rule__2) | INSERT INTO PRO.CustomerClassMovementHistory (TimeKey, CustomerId, WorstClass, MovementDate) SELECT @TimeKey, B.CustomerId, MIN(B.AssetClass), GETDATE() FROM PRO.AccountCal B WHERE B.AssetClass <> 'STANDARD' GROUP BY B.CustomerId | Not cited | Not cited | Not cited | Needs Review |
...
| 7 | Update run status on error (rule__1__7) | UPDATE PRO.RunStatus SET COMPLETED = 'N', ErrorDate = GETDATE(), ErrorDescription = ERROR_MESSAGE(), RunCount = ISNULL(RunCount, 0) + 1 WHERE ProcessName = 'NPA_Movement_Audit_Log' | Not cited | BEGIN CATCH... END CATCH | dependency_0001; dependency_0002 | Verified |
...
- `CONFLICT` tables_read (`recon_8b5816ef3b53`): full_source
- `CONFLICT` tables_written (`recon_a6e2353b36f9`): full_source
- `CONFLICT` rule (`recon_87e891e739b7`): rule__1, 03_batch3_main_body+batch3_nested_block:embedded_01_11 - Deterministic evidence conflicts with the synthesized claim.
- `CONFLICT` rule (`recon_a33d23835162`): rule__2, 03_batch3_main_body+batch3_nested_block:embedded_02_12 - Deterministic evidence conflicts with the synthesized claim.
- `LLM_ONLY` rule (`recon_85e74f4fef3e`): rule__3 - No deterministic evidence was found for this claim.

## Quality Summary

- **Overall status:** REVIEW_REQUIRED
- **Quality score:** 86.24137931034483/100
