...
| Run Timestamp | `2026-09-16T08:33:20.636372+00:00` |
| Object ID | `obj_6df4a37e0d2e` |
...
| Item | Value |
|---|---|
| Run ID | `telemetry_1b5e4f090de7` |
| Total LLM Calls | `5` |
| Successful Calls | `5` |
| Failed Calls | `0` |
| Prompt Tokens | `33238` |
| Completion Tokens | `6245` |
| Total Tokens | `39483` |
| Telemetry Availability | `available` |

| Stage | Calls | Success | Failure | Tokens | Availability |
|---|---:|---:|---:|---:|---|
| extraction | 1 | 1 | 0 | 5011 | available |
| synthesis | 3 | 3 | 0 | 17091 | available |
| synthesis_revision | 1 | 1 | 0 | 17381 | available |

## Business Rule Summary

| Priority | Rule | Output | Business Purpose |
|---|---|---|---|
| 🟠 1 | Determine AssetClass [MATCHED] (`deterministic_case_0028_0033_866_assetclass`) | `AssetClass` | First matching row wins; ELSE includes false or NULL predicates. |
| 🟠 2 | Determine NpaFlag [MATCHED] (`deterministic_case_0041_0044_1351_npaflag`) | `NpaFlag` | First matching row wins; ELSE includes false or NULL predicates. |
| 🟠 3 | Classify NPA based on time key [LLM_ONLY] (`rule__1`) | `NPA_Classification` | Classifies non-performing assets based on the provided time key. |
| 🟢 4 | Determine AssetClass [MATCHED] (`rule__1__2`) | `AssetClass` | Classify the account based on the number of days past due. |
| 🟢 5 | Set NpaFlag [MATCHED] (`rule__2`) | `NpaFlag` | Set the NpaFlag field based on the AssetClass value. |
| 🟢 6 | Calculate AddlProvision [MATCHED] (`rule__3`) | `AddlProvision` | Calculate the additional provision for accounts flagged as NPA. |
| 🟢 7 | Set ClassificationDate [MATCHED] (`rule__4`) | `ClassificationDate` | Set the ClassificationDate for accounts newly marked as NPA. |
| 🟢 8 | Set RestructureReviewFlag [MATCHED] (`rule__5`) | `RestructureReviewFlag` | Set the RestructureReviewFlag for accounts with DaysPastDue between 61 and 90. |
| 🟢 9 | Update RunStatus [MATCHED] (`rule__6`) | `COMPLETED, ErrorDate, ErrorDescription, RunCount` | Update the run status to mark the process as completed and increment the run count. |
| 🔴 10 | Update run status on error [CONFLICT] (`rule__1__8`) | `COMPLETED, ErrorDate, ErrorDescription, RunCount` | If an exception occurs during the 'NPA_Classification_Simple' process, the run status is marked as incomplete, the error date is set to the… |
...
|---|---|---|---|---|---|---|
| 1 | Determine AssetClass (deterministic_case_0028_0033_866_assetclass) | Not cited | source \| Lines 28-33 | Not cited | Not cited | Verified |
| 2 | Determine NpaFlag (deterministic_case_0041_0044_1351_npaflag) | Not cited | source \| Lines 41-44 | Not cited | Not cited | Verified |
| 3 | Classify NPA based on time key (rule__1) | Not cited | Not cited | Not cited | Not cited | Verified |
| 4 | Determine AssetClass (rule__1__2) | UPDATE A SET A.AssetClass = (CASE WHEN A.DaysPastDue <= 90 THEN 'STANDARD' WHEN A.DaysPastDue BETWEEN 91 AND 180 THEN 'SUBSTANDARD' WHEN A.DaysPastDue BETWEEN 181 AND 365 THEN 'DOUBTFUL' ELSE 'LOSS' END) FROM PRO.AccountCal A WHERE A.AccountStatus = 'ACTIVE' | Not cited | Not cited | Not cited | Verified |
| 5 | Set NpaFlag (rule__2) | UPDATE A SET A.NpaFlag = (CASE WHEN A.AssetClass = 'STANDARD' THEN 'N' ELSE 'Y' END) FROM PRO.AccountCal A WHERE A.AccountStatus = 'ACTIVE' | Not cited | Not cited | Not cited | Verified |
| 6 | Calculate AddlProvision (rule__3) | UPDATE A SET A.AddlProvision = (A.OutstandingBalance * A.AddlProvisionPer) / 100 FROM PRO.AccountCal A WHERE A.NpaFlag = 'Y' AND ISNULL(A.AddlProvisionPer, 0) <> 0 | Not cited | Not cited | Not cited | Verified |
| 7 | Set ClassificationDate (rule__4) | UPDATE A SET A.ClassificationDate = @ProcessDate FROM PRO.AccountCal A WHERE A.NpaFlag = 'Y' AND A.ClassificationDate IS NULL | Not cited | Not cited | Not cited | Verified |
| 8 | Set RestructureReviewFlag (rule__5) | UPDATE A SET A.RestructureReviewFlag = 'Y' FROM PRO.AccountCal A WHERE A.DaysPastDue BETWEEN 61 AND 90 | Not cited | Not cited | Not cited | Verified |
| 9 | Update RunStatus (rule__6) | UPDATE PRO.RunStatus SET COMPLETED = 'Y', ErrorDate = NULL, ErrorDescription = NULL, RunCount = ISNULL(RunCount, 0) + 1 WHERE ProcessName = 'NPA_Classification_Simple' | Not cited | Not cited | Not cited | Verified |
| 10 | Update run status on error (rule__1__8) | UPDATE PRO.RunStatus SET COMPLETED = 'N', ErrorDate = GETDATE(), ErrorDescription = ERROR_MESSAGE(), RunCount = ISNULL(RunCount, 0) + 1 WHERE ProcessName = 'NPA_Classification_Simple' | Not cited | Not cited | source_sql | Needs Review |
...
- **Disposition:** covered_by_rule=50, uncovered=14
...
| SELECT | uncovered | Lines 6-8 | 03_batch3_main_body+batch3_nested_block:chunk_text_03, 03_batch3_main_body+batch3_nested_block | DECLARE @ProcessDate DATE = (SELECT [Date] FROM SysDayMatrix WHERE TimeKey = @TimeKey) -- Rule 1: classify the account's asset class by days-past-due |
...
| STATEMENT | uncovered | Lines 59-59 | 03_batch3_main_body+batch3_nested_block:chunk_text_10, 03_batch3_main_body+batch3_nested_block | END TRY |
...
| READ | uncovered | unavailable | 03_batch3_main_body+batch3_nested_block:chunk_text_03, 03_batch3_main_body+batch3_nested_block:chunk_text_03, 03_batch3_main_body+batch3_nested_block | DECLARE @ProcessDate DATE = (SELECT [Date] FROM SysDayMatrix WHERE TimeKey = @TimeKey) -- Rule 1: classify the account's asset class by days-past-due |
...
- **By rule type:** deterministic_decision_table = 2, explicit = 8
...
- `LLM_ONLY` rule (`recon_dcaf86993227`): rule__1 - No deterministic evidence was found for this claim.
- `CONFLICT` rule (`recon_07da7f7f74f1`): rule__1__8, 03_batch3_main_body+batch3_nested_block:chunk_text_09;03_batch3_main_body+batch3_nested_block:embedded_06_16;04_batch3_exception:chunk_text_02;04_batch3_exception:embedded_01_05 - Deterministic evidence conflicts with the synthesized claim.
...
- **Quality score:** 77.64146341463415/100
...
- Automated style check flagged possible leftover technical jargon in the synthesized output: try/catch.
