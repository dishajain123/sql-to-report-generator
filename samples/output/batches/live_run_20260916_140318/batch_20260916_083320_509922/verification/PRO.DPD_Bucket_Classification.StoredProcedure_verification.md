...
| Run Timestamp | `2026-09-16T08:36:31.792278+00:00` |
| Object ID | `obj_ff7556b8a40e` |
...
| Item | Value |
|---|---|
| Run ID | `telemetry_84c8cb6682d3` |
| Total LLM Calls | `5` |
| Successful Calls | `5` |
| Failed Calls | `0` |
| Prompt Tokens | `152945` |
| Completion Tokens | `21954` |
| Total Tokens | `174899` |
| Telemetry Availability | `available` |

| Stage | Calls | Success | Failure | Tokens | Availability |
|---|---:|---:|---:|---:|---|
| extraction | 1 | 1 | 0 | 9342 | available |
| synthesis | 2 | 2 | 0 | 70099 | available |
| synthesis_retry | 1 | 1 | 0 | 32759 | available |
| synthesis_revision | 1 | 1 | 0 | 62699 | available |

## Business Rule Summary

| Priority | Rule | Output | Business Purpose |
|---|---|---|---|
| 🟠 1 | Determine BucketWorsened [MATCHED] (`deterministic_tsql_if_0071_0094_bucketworsened`) | `BucketWorsened` | Not specified |
| 🟠 2 | Determine Reason [MATCHED] (`deterministic_decision_5813_6511_2_reason`) | `Reason` | First matching row wins. SQL type conversion still applies; ELSE includes false or NULL predicates. |
| 🟢 3 | Calculate overdue days [MATCHED] (`rule__1`) | `DpdDays` | Determine the number of days past due for accounts with a last payment due date. |
| 🟢 4 | Set overdue days to zero [MATCHED] (`rule__2`) | `DpdDays, DpdBucket` | For accounts without a last payment due date, set the overdue days to zero. |
| 🟢 5 | Update overdue bucket [MATCHED] (`rule__3`) | `DpdBucket` | Classify accounts into overdue buckets based on the number of overdue days. |
| 🟢 6 | Flag worsened accounts [MATCHED] (`rule__4`) | `BucketWorsened` | Flag accounts that have worsened in their overdue status. |
| 🔴 7 | Calculate overdue penalty [CONFLICT] (`rule__5`) | `PenalInterestAmount` | Calculate the overdue penalty based on the overdue bucket and the number of overdue days. |
| 🔴 8 | Adjust penalty for facility type [CONFLICT] (`rule__6`) | `AdjustedPenalty` | Adjust the overdue penalty based on the facility type. |
| 🔴 9 | Stage adjusted penalties [CONFLICT] (`rule__7`) | `AccountId, DpdBucket, FacilityType, AdjustedPenalty` | Stage adjusted penalties into a temporary table for further processing. |
| 🟠 10 | Merge staged penalties [LLM_ONLY] (`rule__8`) | `Target.DpdBucket, Target.AdjustedPenalty, Target.LastUpdatedDate` | Merge staged penalties into the overdue bucket history table. |
| 🟢 11 | Insert into collections queue [MATCHED] (`rule__9`) | `AccountId, EscalationDate, Reason` | Insert records into the collections queue for accounts that have worsened. |
| 🔴 12 | Insert into audit log [CONFLICT] (`rule__10`) | `AccountId, TransitionDate, NewBucket` | Insert records into the overdue bucket audit log for accounts with a recent update. |
...
|---|---|---|---|---|---|---|
| 1 | Determine BucketWorsened (deterministic_tsql_if_0071_0094_bucketworsened) | Not cited | source \| Lines 71-94 | Not cited | Not cited | Verified |
| 2 | Determine Reason (deterministic_decision_5813_6511_2_reason) | Not cited | source \| Lines 143-157 | Not cited | Not cited | Verified |
| 3 | Calculate overdue days (rule__1) | UPDATE A SET A.DpdDays = DATEDIFF(DAY, A.LastPaymentDueDate, @ProcessDate) FROM PRO.LoanAccountCal A WHERE A.LastPaymentDueDate IS NOT NULL AND A.LastPaymentDueDate <= @ProcessDate | Not cited | Not cited | Not cited | Verified |
| 4 | Set overdue days to zero (rule__2) | UPDATE A SET A.DpdDays = 0, A.DpdBucket = 'NOT_APPLICABLE' FROM PRO.LoanAccountCal A WHERE A.LastPaymentDueDate IS NULL | Not cited | Not cited | Not cited | Verified |
| 5 | Update overdue bucket (rule__3) | UPDATE A SET A.DpdBucket = ( CASE WHEN A.DpdDays IS NULL THEN 'NOT_APPLICABLE' WHEN A.DpdDays = 0 THEN 'CURRENT' WHEN A.DpdDays BETWEEN 1 AND 30 THEN 'BUCKET_1_30' WHEN A.DpdDays BETWEEN 31 AND 60 THEN 'BUCKET_31_60' WHEN A.DpdDays BETWEEN 61 AND 90 THEN 'BUC… | Not cited | Not cited | Not cited | Verified |
| 6 | Flag worsened accounts (rule__4) | UPDATE A SET A.BucketWorsened = 'Y' FROM PRO.LoanAccountCal A WHERE A.PrevDpdBucket IS NOT NULL AND A.DpdBucket <> A.PrevDpdBucket AND A.DpdDays > ISNULL(A.PrevDpdDays, 0) | Not cited | Not cited | Not cited | Verified |
| 7 | Calculate overdue penalty (rule__5) | UPDATE A SET A.PenalInterestAmount = ( CASE WHEN A.DpdBucket = 'BUCKET_1_30' THEN (A.OutstandingBalance * 0.02) / 365 * A.DpdDays WHEN A.DpdBucket = 'BUCKET_31_60' THEN (A.OutstandingBalance * 0.03) / 365 * A.DpdDays WHEN A.DpdBucket IN ('BUCKET_61_90', 'BUCK… | Not cited | Not cited | Not cited | Needs Review |
| 8 | Adjust penalty for facility type (rule__6) | UPDATE S SET S.AdjustedPenalty = ( CASE WHEN S.FacilityType IN ('CC', 'OD') THEN S.AdjustedPenalty * 1.10 WHEN S.FacilityType IN ('TL', 'DL') THEN S.AdjustedPenalty * 1.05 ELSE S.AdjustedPenalty END ) FROM #DpdStaging S WHERE S.AdjustedPenalty IS NOT NULL | Not cited | Not cited | Not cited | Needs Review |
| 9 | Stage adjusted penalties (rule__7) | INSERT INTO #DpdStaging (AccountId, DpdBucket, FacilityType, AdjustedPenalty) SELECT A.AccountId, A.DpdBucket, A.FacilityType, A.PenalInterestAmount FROM PRO.LoanAccountCal A WHERE A.BucketWorsened = 'Y' | Not cited | Not cited | Not cited | Needs Review |
| 10 | Merge staged penalties (rule__8) | MERGE PRO.DpdBucketHistory AS Target USING #DpdStaging AS Source ON Target.AccountId = Source.AccountId WHEN MATCHED THEN UPDATE SET Target.DpdBucket = Source.DpdBucket, Target.AdjustedPenalty = Source.AdjustedPenalty, Target.LastUpdatedDate = @ProcessDate WH… | Not cited | Not cited | Not cited | Needs Review |
| 11 | Insert into collections queue (rule__9) | INSERT INTO PRO.CollectionsQueue (AccountId, EscalationDate, Reason) SELECT AccountId, @ProcessDate, CASE WHEN DpdBucket IN ('BUCKET_61_90', 'BUCKET_90_PLUS') AND FacilityType IN ('CC', 'OD') THEN 'SEVERE_DPD_CASH_CREDIT' WHEN DpdBucket IN ('BUCKET_61_90', 'B… | Not cited | Not cited | Not cited | Verified |
| 12 | Insert into audit log (rule__10) | INSERT INTO PRO.DpdBucketAuditLog (AccountId, TransitionDate, NewBucket) SELECT H.AccountId, @ProcessDate, H.DpdBucket FROM PRO.DpdBucketHistory H WHERE H.LastUpdatedDate = @ProcessDate | Not cited | Not cited | Not cited | Needs Review |
...
- **Disposition:** covered_by_rule=118, technical_only=4, uncovered=33
...
| SELECT | uncovered | Lines 59-59 | 04_batch3_nested_block:chunk_text_12, 04_batch3_nested_block | ELSE IF EXISTS (SELECT 1 FROM PRO.LoanAccountCal WHERE PrevDpdBucket IS NOT NULL) |
...
| ELSE | covered_by_rule | samples/07_DPD_Bucket_Classification.sql | full_source | ELSE |
| IF_BRANCH | uncovered | samples/07_DPD_Bucket_Classification.sql | full_source | IF EXISTS (SELECT 1 FROM PRO.LoanAccountCal WHERE LastPaymentDueDate >= @GraceWindowStart) |
...
- **By rule type:** deterministic_decision_table = 2, explicit = 10
- **By validation status:** unverified = 5, verified = 7
...
- `CONFLICT` rule (`recon_4c4201e333d5`): rule__5, 04_batch3_nested_block:chunk_text_07;04_batch3_nested_block:chunk_text_08;04_batch3_nested_block:chunk_text_10;04_batch3_nested_block:chunk_text_12;04_batch3_nested_block:chunk_text_23;04_batch3_nested_block:chunk_text_25;04_batch3_nested_block:chunk_text_26 - Deterministic evidence conflicts with the synthesized claim.
- `CONFLICT` rule (`recon_76ffd2a6e265`): rule__6, 04_batch3_nested_block:chunk_text_07;04_batch3_nested_block:chunk_text_12;04_batch3_nested_block:chunk_text_23;04_batch3_nested_block:chunk_text_25;04_batch3_nested_block:chunk_text_26;04_batch3_nested_block:embedded_09_43 - Deterministic evidence conflicts with the synthesized claim.

## Quality Summary

- **Overall status:** REVIEW_REQUIRED
- **Quality score:** 88.0036231884058/100
- **Statement coverage:** 34 / 55 (61.8%)
- **Rule grounding coverage:** 9 / 12 (75.0%)
...
- `HIGH` Condition Conflict on `rule__6`: Synthesized condition conflicts with deterministic predicate evidence.
- `MEDIUM` Field Conflict on `rule__7`: Synthesized rule affects different fields than the deterministic evidence.
- `MEDIUM` Field Conflict on `rule__10`: Synthesized rule affects different fields than the deterministic evidence.
