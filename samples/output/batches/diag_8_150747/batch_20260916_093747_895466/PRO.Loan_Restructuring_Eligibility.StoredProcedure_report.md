...
| ⚠️ Merge RestructureRegister with temporary table | `Not specified` | Merge the PRO.RestructureRegister table with the #RestructureDecisions table. |
| ⚠️ Update AccountStatus and LastPaymentDueDate | `LastPaymentDueDate, AccountStatus` | Update the AccountStatus and LastPaymentDueDate fields for eligible accounts. |
| ⚠️ Insert into RestructureAuditLog | `AccountId, DecisionDate, EligibleFlag` | Insert records into the PRO.RestructureAuditLog table for accounts approved in the current run. |
...
### R2 — Merge RestructureRegister with temporary table
...
- Merge the RestructureRegister table with the #RestructureDecisions table.

### Decision Logic

| Condition | Result |
|---|---|
| WHEN MATCHED THEN UPDATE SET Target.EligibleFlag = Source.EligibleFlag, Target.RevisedTenureMonths = Source.RevisedTenureMonths, Target.LastEvaluatedDate = @ProcessDate |  |
| WHEN NOT MATCHED BY TARGET THEN INSERT (AccountId, EligibleFlag, RevisedTenureMonths, FirstEvaluatedDate, LastEvaluatedDate) VALUES (Source.AccountId, Source.EligibleFlag, Source.RevisedTenureMonths, @ProcessDate, @ProcessDate) |  |


### R3 — Update AccountStatus and LastPaymentDueDate
...
- Update the AccountStatus and LastPaymentDueDate fields for eligible accounts.
...
### R4 — Insert into RestructureAuditLog
...
- Account was evaluated on the provided ProcessDate and has a 'Y' EligibleFlag
...
If an error occurs during the execution of the procedure, the running process status is updated to indicate that an error has occurred, and the error details are recorded.

## Findings / Needs Review

- The specific conditions for updating the RestructureEligible field and the RevisedTenureMonths field are not detailed in the extraction.
