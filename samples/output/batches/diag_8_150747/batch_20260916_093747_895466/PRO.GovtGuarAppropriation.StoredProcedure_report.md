...
|---|---|---|
| ⚠️ Reset AppGovGur to zero | `AppGovGur` | For accounts not currently being processed, the AppGovGur field is reset to zero. |
...
| ⚠️ Update AppGovGur with GovtGtyAmt | `AppGovGur` | Update the AppGovGur field for accounts without calculated government guarantee amounts. |
...
- For accounts not currently being processed, the AppGovGur field is reset to zero.
...
### R4 — Update AppGovGur with GovtGtyAmt
...
The procedure uses a TRY-CATCH block to handle any potential errors during execution. The procedure handles exceptions by updating the running process status for the 'GovtGuarAppropriation' process, marking it as not completed and incrementing the error count.
