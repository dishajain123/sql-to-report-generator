...
| Run Timestamp | `2026-09-16T09:40:36.600913+00:00` |
| Object ID | `obj_b4a046e0936e` |
...
| Item | Value |
|---|---|
| Run ID | `telemetry_fd156386afb6` |
| Total LLM Calls | `5` |
| Successful Calls | `5` |
| Failed Calls | `0` |
| Prompt Tokens | `36049` |
| Completion Tokens | `8174` |
| Total Tokens | `44223` |
...
| synthesis | 2 | 2 | 0 | 13938 | available |
| synthesis_revision | 2 | 2 | 0 | 25449 | available |

## Business Rule Summary

| Priority | Rule | Output | Business Purpose |
|---|---|---|---|
| ⚠️ 1 | Reset AppGovGur to zero [MATCHED] (`rule__1`) | `AppGovGur` | For accounts not currently being processed, the AppGovGur field is reset to zero. |
| ⚠️ 2 | Calculate government guarantee [MATCHED] (`rule__2`) | `GovGur` | Calculate the government guarantee amount for eligible accounts and store it in a temporary table. |
...
| ⚠️ 4 | Update AppGovGur with GovtGtyAmt [MATCHED] (`rule__4`) | `AppGovGur` | Update the AppGovGur field for accounts without calculated government guarantee amounts. |
...
| 2 | Calculate government guarantee (rule__2) | SELECT A.AccountEntityID,(CASE WHEN SUM(A.NetBalance) OVER (PARTITION BY A.CustomerEntityId) > 0 THEN (A.GovtGtyAmt * (A.NetBalance / SUM(A.NetBalance) OVER (PARTITION BY A.CustomerEntityId))) END) GovGur INTO #TEMPTABLEAppGovGur FROM ##ACCOUNTCAL A WHERE A.F… | Not cited | 00_main_body+nested_block:embedded_02_13 | Not cited | Verified |
| 3 | Update AppGovGur with calculated amount (rule__3) | UPDATE A SET A.AppGovGur=B.GovGur FROM ##ACCOUNTCAL A INNER JOIN #TEMPTABLEAppGovGur B ON A.AccountEntityID=B.AccountEntityID WHERE A.FacilityType IN('BP','BD') | Not cited | 00_main_body+nested_block:embedded_03_14 | Not cited | Verified |
| 4 | Update AppGovGur with GovtGtyAmt (rule__4) | UPDATE A SET A.AppGovGur=GovtGtyAmt FROM ##ACCOUNTCAL A WHERE NOT (A.FacilityType IN('BP','BD')) | Not cited | 00_main_body+nested_block:embedded_04_15 | Not cited | Verified |
