...
| Run Timestamp | `2026-09-16T09:41:59.130873+00:00` |
| Object ID | `obj_8ece2542e210` |
...
| Item | Value |
|---|---|
| Run ID | `telemetry_e99e6791cf2c` |
| Total LLM Calls | `10` |
| Successful Calls | `10` |
| Failed Calls | `0` |
| Prompt Tokens | `111134` |
| Completion Tokens | `19295` |
| Total Tokens | `130429` |
| Telemetry Availability | `available` |

| Stage | Calls | Success | Failure | Tokens | Availability |
|---|---:|---:|---:|---:|---|
| extraction | 1 | 1 | 0 | 6385 | available |
| synthesis | 7 | 7 | 0 | 70628 | available |
| synthesis_revision | 2 | 2 | 0 | 53416 | available |

## Business Rule Summary

| Priority | Rule | Output | Business Purpose |
|---|---|---|---|
| ⚠️ 1 | Calculate coverage ratio [MATCHED] (`rule__1__5`) | `CoverageRatio` | Determine the coverage ratio for each account based on the outstanding balance and provision amount. |
| ⚠️ 2 | Update ReviewReason based on CoverageRatio [MATCHED] (`rule__2__18`) | `ReviewReason` | Update the ReviewReason in the #ProvisionCoverage table based on coverage ratio thresholds. |
| ⚠️ 3 | Update ReviewReason for quarter end [MATCHED] (`rule__8`) | `ReviewReason` | Update the ReviewReason in the #ProvisionCoverage table to append '_QUARTER_END' for accounts with a coverage ratio less than 0.6. |
| ⚠️ 4 | Update ReviewReason for non-quarter end [CONFLICT] (`rule__9`) | `ReviewReason` | Update the ReviewReason in the #ProvisionCoverage table for accounts with a coverage ratio less than 0.5. |
| 🟠 5 | Determine CoverageRatio [MATCHED] (`deterministic_decision_1348_1980_3_coverageratio`) | `CoverageRatio` | First matching row wins. SQL type conversion still applies; ELSE includes false or NULL predicates. |
| 🟠 6 | Determine ReviewReason [MATCHED] (`deterministic_case_0056_0062_2036_reviewreason`) | `ReviewReason` | First matching row wins; ELSE includes false or NULL predicates. |
| 🟠 7 | Determine ReviewReason [MATCHED] (`deterministic_tsql_if_0068_0083_reviewreason`) | `ReviewReason` | Not specified |
| 🟠 8 | Determine Reason [MATCHED] (`deterministic_decision_4861_5218_2_reason`) | `Reason` | First matching row wins. SQL type conversion still applies; ELSE includes false or NULL predicates. |
| ⚠️ 9 | Insert initial data [CONFLICT] (`rule__1__17`) | `AccountId, OutstandingBalance, ProvisionAmount, CoverageRatio, ReviewReason` | Insert initial provisioning coverage data into the #ProvisionCoverage temporary table. |
| ⚠️ 10 | Merge data into PRO.ProvisionCoverageSummary [LLM_ONLY] (`rule__3__19`) | `AccountId, OutstandingBalance, ProvisionAmount, CoverageRatio, FirstSeenDate, LastUpdatedDate` | Merge the #ProvisionCoverage table data into the PRO.ProvisionCoverageSummary table. |
| ⚠️ 11 | Update BelowThresholdFlag [MATCHED] (`rule__4__20`) | `BelowThresholdFlag` | Update the BelowThresholdFlag in the PRO.ProvisionCoverageSummary table based on coverage ratio thresholds. |
| ⚠️ 12 | Insert into PRO.CollectionsQueue [CONFLICT] (`rule__6`) | `AccountId, EscalationDate, Reason` | Insert data into the PRO.CollectionsQueue table based on the coverage ratio and review reason. |
...
|---|---|---|---|---|---|---|
| 1 | Calculate coverage ratio (rule__1__5) | CASE WHEN A.OutstandingBalance > 0 THEN A.ProvisionAmount / A.OutstandingBalance ELSE NULL END | Not cited | 03_batch3_main_body+batch3_nested_block:chunk_text_08 | 03_batch3_main_body+batch3_nested_block:chunk_text_08 | Verified |
| 2 | Update ReviewReason based on CoverageRatio (rule__2__18) | UPDATE S SET S.ReviewReason = S.ReviewReason + '_QUARTER_END' FROM #ProvisionCoverage S WHERE S.CoverageRatio IS NOT NULL AND S.CoverageRatio < 0.6 | Not cited | 03_batch3_main_body+batch3_nested_block:chunk_text_12 | 03_batch3_main_body+batch3_nested_block:chunk_text_12 | Verified |
| 3 | Update ReviewReason for quarter end (rule__8) | UPDATE S SET S.ReviewReason = S.ReviewReason + '_QUARTER_END' FROM #ProvisionCoverage S WHERE S.CoverageRatio IS NOT NULL AND S.CoverageRatio < 0.6 | Not cited | 03_batch3_main_body+batch3_nested_block:chunk_text_12; 03_batch3_main_body+batch3_nested_block:embedded_03_26 | 03_batch3_main_body+batch3_nested_block:chunk_text_12; 03_batch3_main_body+batch3_nested_block:embedded_03_26 | Verified |
| 4 | Update ReviewReason for non-quarter end (rule__9) | UPDATE S SET S.ReviewReason = S.ReviewReason FROM #ProvisionCoverage S WHERE S.CoverageRatio IS NOT NULL AND S.CoverageRatio < 0.5 | Not cited | 03_batch3_main_body+batch3_nested_block:chunk_text_12; 03_batch3_main_body+batch3_nested_block:embedded_04_27 | 03_batch3_main_body+batch3_nested_block:chunk_text_12; 03_batch3_main_body+batch3_nested_block:embedded_04_27 | Needs Review |
| 5 | Determine CoverageRatio (deterministic_decision_1348_1980_3_coverageratio) | Not cited | source \| Lines 40-54 | Not cited | Not cited | Verified |
| 6 | Determine ReviewReason (deterministic_case_0056_0062_2036_reviewreason) | Not cited | source \| Lines 56-62 | Not cited | Not cited | Verified |
| 7 | Determine ReviewReason (deterministic_tsql_if_0068_0083_reviewreason) | Not cited | source \| Lines 68-83 | Not cited | Not cited | Verified |
| 8 | Determine Reason (deterministic_decision_4861_5218_2_reason) | Not cited | source \| Lines 117-124 | Not cited | Not cited | Verified |
| 9 | Insert initial data (rule__1__17) | INSERT INTO #ProvisionCoverage (AccountId, OutstandingBalance, ProvisionAmount, CoverageRatio, ReviewReason) SELECT A.AccountId, A.OutstandingBalance, A.ProvisionAmount, CASE WHEN A.OutstandingBalance > 0 THEN A.ProvisionAmount / A.OutstandingBalance ELSE NUL… | Not cited | Not cited | Not cited | Needs Review |
| 10 | Merge data into PRO.ProvisionCoverageSummary (rule__3__19) | MERGE INTO PRO.ProvisionCoverageSummary WITH (HOLDLOCK) USING (SELECT AccountId, OutstandingBalance, ProvisionAmount, CoverageRatio, @ProcessDate AS FirstSeenDate, @ProcessDate AS LastUpdatedDate FROM #ProvisionCoverage) AS Source ON (Target.AccountId = Sourc… | Not cited | Not cited | Not cited | Needs Review |
| 11 | Update BelowThresholdFlag (rule__4__20) | UPDATE T SET T.BelowThresholdFlag = 'Y' FROM PRO.ProvisionCoverageSummary T WHERE T.CoverageRatio IS NOT NULL AND T.CoverageRatio < 0.5 AND T.LastUpdatedDate = @ProcessDate | Not cited | Not cited | Not cited | Verified |
| 12 | Insert into PRO.CollectionsQueue (rule__6) | INSERT INTO PRO.CollectionsQueue (AccountId, EscalationDate, Reason) SELECT AccountId, @ProcessDate, CASE WHEN CoverageRatio < 0.25 THEN 'SEVERE_PROVISION_SHORTFALL' ELSE 'MODERATE_PROVISION_SHORTFALL' END FROM #ProvisionCoverage WHERE ReviewReason LIKE '%UND… | Not cited | Not cited | Not cited | Needs Review |
...
- **Disposition:** covered_by_rule=79, technical_only=9, uncovered=16
...
| STATEMENT | covered_by_rule | Lines 7-7 | 03_batch3_main_body+batch3_nested_block:chunk_text_04, 03_batch3_main_body+batch3_nested_block | DECLARE @QuarterStartDate DATE = DATEADD(MONTH, -3, @ProcessDate) |
| STATEMENT | covered_by_rule | Lines 9-9 | 03_batch3_main_body+batch3_nested_block:chunk_text_05, 03_batch3_main_body+batch3_nested_block | IF OBJECT_ID('tempdb..#ProvisionCoverage') IS NOT NULL |
| STATEMENT | covered_by_rule | Lines 10-10 | 03_batch3_main_body+batch3_nested_block:chunk_text_06, 03_batch3_main_body+batch3_nested_block | DROP TABLE #ProvisionCoverage |
...
| UPDATE | covered_by_rule | Lines 37-50 | 03_batch3_main_body+batch3_nested_block:chunk_text_09, 03_batch3_main_body+batch3_nested_block | UPDATE S SET S.ReviewReason = ( CASE WHEN S.CoverageRatio IS NULL THEN 'NO_BALANCE' WHEN S.CoverageRatio < 0.25 THEN 'SEVERE_UNDERCOVER' WHEN S.CoverageRatio < 0.5 THEN 'MODERATE_UNDERCOVER' WHEN S.CoverageRatio >= 0.5 AND S.CoverageRati... |
| STATEMENT | covered_by_rule | Lines 51-51 | 03_batch3_main_body+batch3_nested_block:chunk_text_10, 03_batch3_main_body+batch3_nested_block | IF @ProcessDate >= DATEADD(MONTH, 1, @QuarterStartDate) |
| STATEMENT | covered_by_rule | Lines 1-1 | 03_batch3_main_body+batch3_nested_block:chunk_text_11, 03_batch3_main_body+batch3_nested_block | BEGIN |
...
| STATEMENT | covered_by_rule | Lines 1-1 | 03_batch3_main_body+batch3_nested_block:chunk_text_15, 03_batch3_main_body+batch3_nested_block | BEGIN |
...
| UPDATE | covered_by_rule | Lines 107-109 | 03_batch3_main_body+batch3_nested_block:chunk_text_22, 03_batch3_main_body+batch3_nested_block | UPDATE PRO.ACLRUNNINGPROCESSSTATUS SET COMPLETED = 'Y', ERRORDATE = NULL, ERRORDESCRIPTION = NULL, COUNT = ISNULL(COUNT, 0) + 1 WHERE RUNNINGPROCESSNAME = 'Provision_Coverage_Merge' |
| STATEMENT | covered_by_rule | Lines 111-111 | 03_batch3_main_body+batch3_nested_block:chunk_text_23, 03_batch3_main_body+batch3_nested_block | END TRY |
...
| UPDATE | covered_by_rule | Lines 37-50 | 03_batch3_main_body+batch3_nested_block:embedded_02_25, 03_batch3_main_body+batch3_nested_block | UPDATE S SET S.ReviewReason = ( CASE WHEN S.CoverageRatio IS NULL THEN 'NO_BALANCE' WHEN S.CoverageRatio < 0.25 THEN 'SEVERE_UNDERCOVER' WHEN S.CoverageRatio < 0.5 THEN 'MODERATE_UNDERCOVER' WHEN S.CoverageRatio >= 0.5 AND S.CoverageRati... |
...
| UPDATE | covered_by_rule | Lines 91-99 | 03_batch3_main_body+batch3_nested_block:embedded_07_30, 03_batch3_main_body+batch3_nested_block | UPDATE T SET T.BelowThresholdFlag = 'N' FROM PRO.ProvisionCoverageSummary T WHERE T.CoverageRatio >= 0.5 AND T.LastUpdatedDate = @ProcessDate -- Rule 7: record a review case for every account newly below -- threshold - the escalation que... |
...
| UPDATE | covered_by_rule | Lines 107-109 | 03_batch3_main_body+batch3_nested_block:embedded_09_32, 03_batch3_main_body+batch3_nested_block | UPDATE PRO.ACLRUNNINGPROCESSSTATUS SET COMPLETED = 'Y', ERRORDATE = NULL, ERRORDESCRIPTION = NULL, COUNT = ISNULL(COUNT, 0) + 1 WHERE RUNNINGPROCESSNAME = 'Provision_Coverage_Merge' |
...
| UPDATE | covered_by_rule | samples/09_Provision_Coverage_Merge.sql | 03_batch3_main_body+batch3_nested_block:chunk_text_20, 03_batch3_main_body+batch3_nested_block:chunk_text_20, 03_batch3_main_body+batch3_nested_block | UPDATE T SET T.BelowThresholdFlag = 'N' FROM PRO.ProvisionCoverageSummary T WHERE T.CoverageRatio >= 0.5 AND T.LastUpdatedDate = @ProcessDate -- Rule 7: record a review case for every account newly below -- threshold - the escalation que... |
...
| UPDATE | covered_by_rule | samples/09_Provision_Coverage_Merge.sql | 03_batch3_main_body+batch3_nested_block:chunk_text_22, 03_batch3_main_body+batch3_nested_block:chunk_text_22, 03_batch3_main_body+batch3_nested_block | UPDATE PRO.ACLRUNNINGPROCESSSTATUS SET COMPLETED = 'Y', ERRORDATE = NULL, ERRORDESCRIPTION = NULL, COUNT = ISNULL(COUNT, 0) + 1 WHERE RUNNINGPROCESSNAME = 'Provision_Coverage_Merge' |
...
| UPDATE | covered_by_rule | unavailable | 03_batch3_main_body+batch3_nested_block:embedded_07_30, 03_batch3_main_body+batch3_nested_block:embedded_07_30, 03_batch3_main_body+batch3_nested_block | UPDATE T SET T.BelowThresholdFlag = 'N' FROM PRO.ProvisionCoverageSummary T WHERE T.CoverageRatio >= 0.5 AND T.LastUpdatedDate = @ProcessDate -- Rule 7: record a review case for every account newly below -- threshold - the escalation que... |
...
- **By rule type:** deterministic_decision_table = 4, explicit = 8
- **By validation status:** unverified = 4, verified = 8
...
- **Quality score:** 76.32575757575758/100
...
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: UPDATE T.BelowThresholdFlag WHERE T.CoverageRatio < 0.5 AND T.LastUpdatedDate = @ProcessDate
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: UPDATE S SET S.ReviewReason = S.ReviewReason + '_QUARTER_END' FROM #ProvisionCoverage S WHERE S.CoverageRatio IS NOT NULL AND S.CoverageRatio < 0.5
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: UPDATE T SET T.BelowThresholdFlag = 'Y' FROM PRO.ProvisionCoverageSummary T WHERE T.CoverageRatio < 0.5 AND T.LastUpdatedDate = @ProcessDate
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: UPDATE PRO.ProvisionCoverageSummary SET BelowThresholdFlag = 'Y' WHERE NOT CoverageRatio IS NULL AND CoverageRatio < 0.5 AND LastUpdatedDate = @ProcessDate
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: MERGE INTO PRO.ProvisionCoverageSummary USING #ProvisionCoverage AS Source ON Target.AccountId = Source.AccountId WHEN MATCHED THEN UPDATE SET Target.OutstandingBalance = Source.OutstandingBalance, Target.ProvisionAmount = Source.ProvisionAmount, Target.CoverageRatio = Source.CoverageRatio, Target.LastUpdatedDate = @ProcessDate WHEN NOT MATCHED BY TARGET THEN INSERT (AccountId, OutstandingBalance, ProvisionAmount, CoverageRatio, FirstSeenDate, LastUpdatedDate) VALUES (Source.AccountId, Source.OutstandingBalance, Source.ProvisionAmount, Source.CoverageRatio, @ProcessDate, @ProcessDate)
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: UPDATE PRO.ProvisionCoverageSummary SET T.BelowThresholdFlag = 'Y' FROM PRO.ProvisionCoverageSummary T WHERE T.CoverageRatio < 0.5 AND T.LastUpdatedDate = @ProcessDate
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: MERGE INTO PRO.ProvisionCoverageSummary WITH (HOLDLOCK) USING (SELECT AccountId, OutstandingBalance, ProvisionAmount, CoverageRatio, @ProcessDate AS FirstSeenDate, @ProcessDate AS LastUpdatedDate FROM #ProvisionCoverage) AS Source ON (Target.AccountId = Source.AccountId) WHEN MATCHED THEN UPDATE SET Target.OutstandingBalance = Source.OutstandingBalance, Target.ProvisionAmount = Source.ProvisionAmount, Target.CoverageRatio = Source.CoverageRatio, Target.LastUpdatedDate = Source.LastUpdatedDate WHEN NOT MATCHED BY TARGET THEN INSERT (AccountId, OutstandingBalance, ProvisionAmount, CoverageRatio, FirstSeenDate, LastUpdatedDate) VALUES (Source.AccountId, Source.OutstandingBalance, Source.ProvisionAmount, Source.CoverageRatio, Source.FirstSeenDate, Source.LastUpdatedDate);
