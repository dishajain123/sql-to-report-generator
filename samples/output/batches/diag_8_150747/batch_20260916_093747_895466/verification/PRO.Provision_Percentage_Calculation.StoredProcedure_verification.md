...
| Run Timestamp | `2026-09-16T09:41:18.741944+00:00` |
| Object ID | `obj_890a2d3ea714` |
...
| Item | Value |
|---|---|
| Run ID | `telemetry_55f9f2039650` |
| Total LLM Calls | `7` |
| Successful Calls | `7` |
| Failed Calls | `0` |
| Prompt Tokens | `52672` |
| Completion Tokens | `9684` |
| Total Tokens | `62356` |
| Telemetry Availability | `available` |

| Stage | Calls | Success | Failure | Tokens | Availability |
|---|---:|---:|---:|---:|---|
| extraction | 1 | 1 | 0 | 4914 | available |
| synthesis | 4 | 4 | 0 | 24122 | available |
| synthesis_revision | 2 | 2 | 0 | 33320 | available |

## Business Rule Summary

| Priority | Rule | Output | Business Purpose |
|---|---|---|---|
| ⚠️ 1 | Determine Provision Percentage [MATCHED] (`rule__1`) | `ProvisionPct` | Set the provision percentage based on the asset class of the account. |
| ⚠️ 2 | Increase Provision Percentage [MATCHED] (`rule__2`) | `ProvisionPct` | Increase the provision percentage by 10% for unsecured accounts with a non-standard asset class. |
| ⚠️ 3 | Calculate Provision Amount [CONFLICT] (`rule__3`) | `ProvisionAmount` | Calculate the provision amount based on the outstanding balance and provision percentage. |
| ⚠️ 4 | Increment Run Count [MATCHED] (`rule__6`) | `RunCount` | Increment the run count for the 'Provision_Percentage_Calculation' process. |
| ⚠️ 5 | Ensure Provision Amount Does Not Exceed Balance [MATCHED] (`rule__4`) | `ProvisionAmount` | Ensure the provision amount does not exceed the outstanding balance. |
| ⚠️ 6 | Flag for Senior Review [MATCHED] (`rule__5`) | `SeniorReviewFlag` | Flag accounts for senior review if the provision amount exceeds one million. |
| ⚠️ 7 | Update run status on error [CONFLICT] (`rule__7`) | `RunStatus, COMPLETED, ErrorDate, ErrorDescription, RunCount` | If an error occurs during the 'Provision_Percentage_Calculation' process, the run status is updated to indicate the process is incomplete,… |
...
|---|---|---|---|---|---|---|
| 1 | Determine Provision Percentage (rule__1) | UPDATE A SET A.ProvisionPct = (CASE A.AssetClass WHEN 'STANDARD' THEN 0.40 WHEN 'SUBSTANDARD' THEN 15.00 WHEN 'DOUBTFUL' THEN 25.00 WHEN 'LOSS' THEN 100.00 ELSE 0.00 END) FROM PRO.AccountCal A WHERE A.SecuredFlag = 'N' AND A.AssetClass <> 'STANDARD' | Not cited | 03_batch3_main_body+batch3_nested_block:chunk_text_04 | Not cited | Verified |
| 2 | Increase Provision Percentage (rule__2) | UPDATE A SET A.ProvisionPct = A.ProvisionPct + 10.00 FROM PRO.AccountCal A WHERE A.SecuredFlag = 'N' AND A.AssetClass <> 'STANDARD' | Not cited | 03_batch3_main_body+batch3_nested_block:chunk_text_04 | Not cited | Verified |
| 3 | Calculate Provision Amount (rule__3) | UPDATE A SET A.ProvisionAmount = (A.OutstandingBalance * A.ProvisionPct) / 100 FROM PRO.AccountCal A WHERE A.OutstandingBalance > 0 | Not cited | 03_batch3_main_body+batch3_nested_block:chunk_text_05 | Not cited | Verified |
| 4 | Increment Run Count (rule__6) | UPDATE PRO.RunStatus SET RunCount = ISNULL(RunCount, 0) + 1 WHERE ProcessName = 'Provision_Percentage_Calculation' | Not cited | 03_batch3_main_body+batch3_nested_block:chunk_text_08 | Not cited | Verified |
...
| 7 | Update run status on error (rule__7) | UPDATE PRO.RunStatus SET COMPLETED = 'N', ErrorDate = GETDATE(), ErrorDescription = ERROR_MESSAGE(), RunCount = ISNULL(RunCount, 0) + 1 WHERE ProcessName = 'Provision_Percentage_Calculation' | Not cited | BEGIN CATCH... END CATCH | source_sql | Verified |
...
- `CONFLICT` rule (`recon_5b50d7c10c09`): rule__3, 03_batch3_main_body+batch3_nested_block, 03_batch3_main_body+batch3_nested_block:chunk_text_05;03_batch3_main_body+batch3_nested_block:chunk_text_06;03_batch3_main_body+batch3_nested_block:embedded_03_12;03_batch3_main_body+batch3_nested_block:embedded_04_13 - Deterministic evidence conflicts with the synthesized claim.
- `CONFLICT` rule (`recon_af07ee3c1c33`): rule__7, BEGIN CATCH... END CATCH, 03_batch3_main_body+batch3_nested_block:chunk_text_08;03_batch3_main_body+batch3_nested_block:embedded_06_15;04_batch3_exception:chunk_text_02;04_batch3_exception:embedded_01_05 - Deterministic evidence conflicts with the synthesized claim.
...
- **Quality score:** 88.78947368421052/100
...
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: UPDATE A SET A.ProvisionPct = (CASE A.AssetClass WHEN 'STANDARD' THEN 0.40 WHEN 'SUBSTANDARD' THEN 15.00 WHEN 'DOUBTFUL' THEN 25.00 WHEN 'LOSS' THEN 100.00 ELSE 0.00 END) FROM PRO.AccountCal A WHERE A.SecuredFlag = 'N' AND A.AssetClass <> 'STANDARD'
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: UPDATE PRO.RunStatus SET RunCount = ISNULL(RunCount, 0) + 1 WHERE ProcessName = 'Provision_Percentage_Calculation'
