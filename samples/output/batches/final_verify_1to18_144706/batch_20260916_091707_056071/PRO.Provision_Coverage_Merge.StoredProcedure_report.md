...
| ⚠️ Append quarter-end to review reason | `ReviewReason` | Append '_QUARTER_END' to the review reason if the process date is after the quarter start date plus one month. |
| Determine Reason | `Reason` | First matching row wins. SQL type conversion still applies; ELSE includes false or NULL predicates. |
| ⚠️ Merge provisioning coverage summary | `AccountId, OutstandingBalance, ProvisionAmount, CoverageRatio, FirstSeenDate, LastUpdatedDate` | Merge the latest provisioning coverage data into the PRO.ProvisionCoverageSummary table. |
| ⚠️ Update BelowThresholdFlag for CoverageRatio < 0.5 | `BelowThresholdFlag` | Set BelowThresholdFlag to 'Y' for accounts with a CoverageRatio less than 0.5 and LastUpdatedDate equal to the process date. |
...
### R3 — Append quarter-end to review reason

**Affected Field:** `ReviewReason`

**Summary:**

- Append '_QUARTER_END' to the review reason if the process date is after the quarter start date plus one month.
...
- Merge the latest provisioning coverage data into the ProvisionCoverageSummary table.


### R6 — Update BelowThresholdFlag for CoverageRatio < 0.5
...
- CoverageRatio is less than 0.5 and last updated date matches the process date.

**Summary:**

- Set BelowThresholdFlag to 'Y' for accounts with a CoverageRatio less than 0.5 and LastUpdatedDate equal to the process date.
...
If an error occurs during the execution of the 'Provision_Coverage_Merge' process, the running process status is updated to reflect the error.
