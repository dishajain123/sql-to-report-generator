...
| Run Timestamp | `2026-09-16T09:17:37.030753+00:00` |
| Object ID | `obj_f88fd9f08d12` |
...
| Item | Value |
|---|---|
| Run ID | `telemetry_8b21f58921ed` |
| Total LLM Calls | `8` |
| Successful Calls | `8` |
| Failed Calls | `0` |
| Prompt Tokens | `65894` |
| Completion Tokens | `11772` |
| Total Tokens | `77666` |
| Telemetry Availability | `available` |

| Stage | Calls | Success | Failure | Tokens | Availability |
|---|---:|---:|---:|---:|---|
| extraction | 1 | 1 | 0 | 5801 | available |
| synthesis | 5 | 5 | 0 | 29430 | available |
| synthesis_revision | 2 | 2 | 0 | 42435 | available |
...
| ⚠️ 2 | Set SMA flag for accounts [CONFLICT] (`rule__4`) | `FlagSma` | Set the SMA flag for accounts based on their SMA stage. |
| ⚠️ 3 | Set SMA reason for accounts [CONFLICT] (`rule__5`) | `SmaReason` | Set the SMA reason for accounts based on their facility type. |
| ⚠️ 4 | Update SMA stage date [MATCHED] (`rule__2`) | `SmaStageDate` | Calculate and update the SMA stage date for accounts that are currently overdue. |
| ⚠️ 5 | Clear SMA status for non-overdue accounts [CONFLICT] (`rule__3`) | `SmaStage, FlagSma, SmaReason` | Clear the SMA status for accounts that are no longer overdue. |
| ⚠️ 6 | Update process completion status [MATCHED] (`rule__6`) | `RunCount, COMPLETED, ErrorDate, ErrorDescription` | Update the process completion status in the RunStatus table to indicate successful completion and increment the run count. |
| ⚠️ 7 | Update run status on error [CONFLICT] (`rule__7`) | `RunStatus, COMPLETED, ErrorDate, ErrorDescription, RunCount` | If an exception occurs, the process run status is marked as incomplete, the error date is set to the current date and time, the error descr… |
...
|---|---|---|---|---|---|---|
| 1 | Determine SMA stage (rule__1) | WHEN A.OverdueDays BETWEEN 1 AND 30 THEN 'SMA_0'; WHEN A.OverdueDays BETWEEN 31 AND 60 THEN 'SMA_1'; WHEN A.OverdueDays BETWEEN 61 AND 90 THEN 'SMA_2'; ELSE NULL | Not cited | 03_batch3_main_body+batch3_nested_block:chunk_text_05 | source_sql | Verified |
| 2 | Set SMA flag for accounts (rule__4) | UPDATE A SET A.FlagSma = CASE WHEN A.SmaStage IS NOT NULL THEN 'Y' ELSE 'N' END FROM PRO.AccountCal A | Not cited | 03_batch3_main_body+batch3_nested_block:chunk_text_06 | source_sql | Verified |
| 3 | Set SMA reason for accounts (rule__5) | UPDATE A SET A.SmaReason = CASE WHEN A.FacilityType IN ('CC', 'OD') THEN 'CASH CREDIT / OVERDRAFT OVERDUE' WHEN A.FacilityType IN ('TL', 'DL') THEN 'TERM LOAN OVERDUE' ELSE 'OTHER FACILITY OVERDUE' END FROM PRO.AccountCal A | Not cited | 03_batch3_main_body+batch3_nested_block:chunk_text_07 | source_sql | Verified |
| 4 | Update SMA stage date (rule__2) | UPDATE A SET A.SmaStageDate = DATEADD(DAY, -A.OverdueDays + 1, @ProcessDate) FROM PRO.AccountCal A WHERE A.FlagSma = 'Y' | Not cited | 03_batch3_main_body+batch3_nested_block:chunk_text_07; 03_batch3_main_body+batch3_nested_block:embedded_04_14 | source_sql | Verified |
| 5 | Clear SMA status for non-overdue accounts (rule__3) | UPDATE A SET A.SmaStage = NULL, A.FlagSma = 'N', A.SmaReason = NULL FROM PRO.AccountCal A WHERE A.OverdueDays = 0 | Not cited | 03_batch3_main_body+batch3_nested_block:embedded_06_16 | source_sql | Verified |
| 6 | Update process completion status (rule__6) | UPDATE PRO.RunStatus SET COMPLETED = 'Y', ErrorDate = NULL, ErrorDescription = NULL, RunCount = ISNULL(RunCount, 0) + 1 WHERE ProcessName = 'SMA_Stage_Marking_Simple' | Not cited | 03_batch3_main_body+batch3_nested_block:chunk_text_09; 03_batch3_main_body+batch3_nested_block:embedded_06_16 | source_sql | Verified |
| 7 | Update run status on error (rule__7) | UPDATE PRO.RunStatus SET COMPLETED = 'N', ErrorDate = GETDATE(), ErrorDescription = ERROR_MESSAGE(), RunCount = ISNULL(RunCount, 0) + 1 WHERE ProcessName = 'SMA_Stage_Marking_Simple' | Not cited | BEGIN CATCH... END CATCH | source_sql | Verified |
...
- **Quality score:** 85.27659574468086/100
...
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: UPDATE A SET A.SmaStage = NULL, A.FlagSma = 'N', A.SmaReason = 'Non-overdue' FROM PRO.AccountCal A WHERE A.OverdueDays = 0
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: UPDATE A SET A.SmaStageDate = NULL FROM PRO.AccountCal A WHERE A.OverdueDays = 0 AND A.FlagSma = 'Y'
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: UPDATE A SET A.SmaReason = CASE WHEN A.FacilityType IN ('CC', 'OD') THEN 'CASH CREDIT / OVERDRAFT OVERDUE' END FROM PRO.AccountCal A
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: UPDATE A SET A.SmaReason = CASE WHEN A.FacilityType IN ('TL', 'DL') THEN 'TERM LOAN OVERDUE' END FROM PRO.AccountCal A
