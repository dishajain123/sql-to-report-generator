...
| Run Timestamp | `2026-09-16T09:25:07.410852+00:00` |
| Object ID | `obj_adfa3b566339` |
...
|---|---:|---:|---:|---:|---|
| extraction | 1 | 1 | 0 | 6739 | available |
| synthesis | 7 | 7 | 0 | 77707 | available |
| synthesis_revision | 1 | 1 | 0 | 26799 | available |

## Business Rule Summary

| Priority | Rule | Output | Business Purpose |
|---|---|---|---|
| 🟢 1 | Update shortfall amount [MATCHED] (`rule__2`) | `ShortfallAmount` | Update the ShortfallAmount in the #CollateralStaging table based on certain conditions. |
| 🟢 2 | Update review priority [MATCHED] (`rule__3`) | `ReviewPriority` | Update the ReviewPriority in the #CollateralStaging table based on certain conditions. |
| 🟢 3 | Read data for processing [MATCHED] (`rule__4`) | `AccountId, @ProcessDate, ShortfallAmount` | Read data from the #CollateralStaging table for further processing. |
| 🟢 4 | Calculate shortfall amount [MATCHED] (`rule__1`) | `ShortfallAmount` | Determine the shortfall amount for accounts with an outstanding balance greater than the collateral value. |
| 🟢 5 | Set shortfall amount to zero [MATCHED] (`rule__2__5`) | `ShortfallAmount` | Set the shortfall amount to zero for accounts with an outstanding balance less than or equal to the collateral value. |
| 🔴 6 | Update review priority for positive shortfall [CONFLICT] (`rule__3__6`) | `ReviewPriority` | Update the review priority for accounts with a positive shortfall amount. |
| 🔴 7 | Update review priority for zero shortfall [CONFLICT] (`rule__4__7`) | `ReviewPriority` | Update the review priority for accounts with a zero shortfall amount. |
| 🔴 8 | Update review priority [CONFLICT] (`rule__2__9`) | `ReviewPriority` | Assign a review priority to each account based on the shortfall amount. |
| 🟢 9 | Mark collateral as stale [MATCHED] (`rule__3__10`) | `CollateralStale` | Mark collateral as stale if no recent valuation exists for the account. |
| 🔴 10 | Set review priority [CONFLICT] (`rule__2__12`) | `ReviewPriority` | Assign a review priority based on the shortfall amount. |
| 🟢 11 | Update collateral stale status [MATCHED] (`rule__4__14`) | `CollateralStale` | Mark collateral as stale if there is no recent valuation. |
| 🔴 12 | Update review priority [CONFLICT] (`rule__3__17`) | `ReviewPriority` | Update the review priority for accounts in #CollateralStaging based on specific conditions. |
| 🟢 13 | Read shortfall and account details [MATCHED] (`rule__6`) | `AccountId, @ProcessDate, ShortfallAmount` | Read shortfall amounts and account IDs from the #CollateralStaging table for further processing. |
| 🟢 14 | Merge collateral position summaries [MATCHED] (`rule__10`) | `Target.AccountId, Source.AccountId, Target.CollateralValue, Source.CollateralValue, Target.ShortfallAmount, Source.ShortfallAmount, Target.ReviewPriority, Source.ReviewPriority, Target.LastUpdatedDate, AccountId, CollateralValue, ShortfallAmount, ReviewPriority, FirstSeenDate, LastUpdatedDate` | Merge collateral position summaries. |
| 🟠 15 | Determine ReviewPriority [MATCHED] (`deterministic_case_0071_0077_2673_reviewpriority`) | `ReviewPriority` | First matching row wins; ELSE includes false or NULL predicates. |
| 🟠 16 | Determine ReviewPriority [MATCHED] (`deterministic_case_0085_0089_3183_reviewpriority`) | `ReviewPriority` | First matching row wins; ELSE includes false or NULL predicates. |
...
|---|---|---|---|---|---|---|
| 1 | Update shortfall amount (rule__2) | UPDATE #CollateralStaging SET S.ShortfallAmount =... WHERE... | Not cited | 03_batch3_main_body+batch3_nested_block:embedded_02_25; 03_batch3_main_body+batch3_nested_block:embedded_03_26 | dependency_0006; dependency_0007; dependency_0008 | Verified |
| 2 | Update review priority (rule__3) | UPDATE #CollateralStaging SET S.ReviewPriority =... WHERE... | Not cited | 03_batch3_main_body+batch3_nested_block:embedded_04_27; 03_batch3_main_body+batch3_nested_block:embedded_05_28 | dependency_0009; dependency_0010 | Verified |
| 3 | Read data for processing (rule__4) | SELECT AccountId, @ProcessDate, ShortfallAmount FROM #CollateralStaging | Not cited | 03_batch3_main_body+batch3_nested_block:embedded_07_30 | dependency_0005; dependency_0006; dependency_0007; dependency_0008; dependency_0009; dependency_0010 | Verified |
| 4 | Calculate shortfall amount (rule__1) | UPDATE S SET S.ShortfallAmount = S.OutstandingBalance - S.CollateralValue FROM #CollateralStaging S WHERE S.OutstandingBalance > S.CollateralValue | Not cited | 03_batch3_main_body+batch3_nested_block:embedded_01_24 | dependency_0001; dependency_0002 | Verified |
| 5 | Set shortfall amount to zero (rule__2__5) | UPDATE S SET S.ShortfallAmount = 0 FROM #CollateralStaging S WHERE S.OutstandingBalance <= S.CollateralValue | Not cited | 03_batch3_main_body+batch3_nested_block:embedded_02_25 | dependency_0003; dependency_0004 | Verified |
| 6 | Update review priority for positive shortfall (rule__3__6) | UPDATE S SET S.ReviewPriority = 'HIGH' FROM #CollateralStaging S WHERE S.ShortfallAmount > 0 | Not cited | 03_batch3_main_body+batch3_nested_block:embedded_03_26 | dependency_0005 | Needs Review |
| 7 | Update review priority for zero shortfall (rule__4__7) | UPDATE S SET S.ReviewPriority = 'LOW' FROM #CollateralStaging S WHERE S.ShortfallAmount = 0 | Not cited | 03_batch3_main_body+batch3_nested_block:embedded_04_27 | dependency_0006 | Needs Review |
| 8 | Update review priority (rule__2__9) | UPDATE S SET S.ReviewPriority = (CASE WHEN S.ShortfallAmount IS NULL THEN 'NONE' WHEN S.ShortfallAmount > 500000 THEN 'URGENT' WHEN S.ShortfallAmount > 100000 THEN 'HIGH' WHEN S.ShortfallAmount > 0 THEN 'STANDARD' END) FROM #CollateralStaging S WHERE S.Outsta… | Not cited | 03_batch3_main_body+batch3_nested_block | 03_batch3_main_body+batch3_nested_block | Needs Review |
| 9 | Mark collateral as stale (rule__3__10) | UPDATE PRO.LoanAccountCal SET A.CollateralStale = 'Y' WHERE A.SecuredFlag = 'Y' AND NOT EXISTS(SELECT 1 FROM PRO.CollateralValuation AS V WHERE V.AccountId = A.AccountId AND V.ValuationDate >= @StaleValuationCutoff) | Not cited | 03_batch3_main_body+batch3_nested_block | 03_batch3_main_body+batch3_nested_block | Verified |
| 10 | Set review priority (rule__2__12) | UPDATE S SET S.ReviewPriority = 'HIGH' FROM #CollateralStaging S WHERE S.ShortfallAmount > 500000 | Not cited | 03_batch3_main_body+batch3_nested_block:embedded_04_27 | 03_batch3_main_body+batch3_nested_block:embedded_04_27 | Needs Review |
| 11 | Update collateral stale status (rule__4__14) | UPDATE A SET A.CollateralStale = 'Y' FROM PRO.LoanAccountCal A WHERE A.SecuredFlag = 'Y' AND NOT EXISTS(SELECT 1 FROM PRO.CollateralValuation AS V WHERE V.AccountId = A.AccountId AND V.ValuationDate >= @StaleValuationCutoff) | Not cited | 03_batch3_main_body+batch3_nested_block:embedded_08_29 | 03_batch3_main_body+batch3_nested_block:embedded_08_29 | Verified |
| 12 | Update review priority (rule__3__17) | UPDATE S SET S.ReviewPriority = 'HIGH' FROM #CollateralStaging S WHERE S.OutstandingBalance > S.CollateralValue | Not cited | 03_batch3_main_body+batch3_nested_block:embedded_04_27 | 03_batch3_main_body+batch3_nested_block:embedded_04_27 | Needs Review |
| 13 | Read shortfall and account details (rule__6) | SELECT AccountId, @ProcessDate, ShortfallAmount FROM #CollateralStaging WHERE ShortfallAmount > 0 | Not cited | 03_batch3_main_body+batch3_nested_block:embedded_07_30 | dependency_0010 | Verified |
| 14 | Merge collateral position summaries (rule__10) | MERGE INTO PRO.CollateralPositionSummary AS Target USING (SELECT AccountId, CollateralValue, ShortfallAmount, ReviewPriority, @ProcessDate AS LastUpdatedDate FROM #CollateralStaging) AS Source ON Target.AccountId = Source.AccountId WHEN MATCHED THEN UPDATE SE… | Not cited | 03_batch3_main_body+batch3_nested_block:embedded_11_34 | dependency_0008 | Verified |
| 15 | Determine ReviewPriority (deterministic_case_0071_0077_2673_reviewpriority) | Not cited | source \| Lines 71-77 | Not cited | Not cited | Verified |
| 16 | Determine ReviewPriority (deterministic_case_0085_0089_3183_reviewpriority) | Not cited | source \| Lines 85-89 | Not cited | Not cited | Verified |
...
| decision_chain_003:branch_001 | S.ShortfallAmount IS NULL THEN 'NONE' WHEN S.ShortfallAmount > 500000 THEN 'URGENT' WHEN S.ShortfallAmount > 100000 THEN 'HIGH' WHEN S.ShortfallAmount > 0 THEN 'STANDARD' ELSE 'NONE' | source |
| decision_chain_003:branch_002 | S.ShortfallAmount > 500000 THEN 'HIGH' WHEN S.ShortfallAmount > 0 THEN 'STANDARD' ELSE 'NONE' | source |
...
- **Disposition:** covered_by_rule=65, technical_only=11, uncovered=19
...
| IF_BRANCH | uncovered | samples/11_Collateral_Valuation_Staging.sql | full_source | S.ShortfallAmount IS NULL THEN 'NONE' WHEN S.ShortfallAmount > 500000 THEN 'URGENT' WHEN S.ShortfallAmount > 100000 THEN 'HIGH' WHEN S.ShortfallAmount > 0 THEN 'STANDARD' ELSE 'NONE' |
| IF_BRANCH | uncovered | samples/11_Collateral_Valuation_Staging.sql | full_source | S.ShortfallAmount > 500000 THEN 'HIGH' WHEN S.ShortfallAmount > 0 THEN 'STANDARD' ELSE 'NONE' |
...
- **By rule type:** deterministic_decision_table = 2, explicit = 14
- **By validation status:** unverified = 5, verified = 11
...
- **Quality score:** 85.46938775510205/100
...
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: SELECT AccountId, @ProcessDate, ShortfallAmount FROM #CollateralStaging
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: UPDATE S SET S.ReviewPriority = 'HIGH' FROM #CollateralStaging S WHERE S.ShortfallAmount > 0
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: UPDATE S SET S.ReviewPriority = 'LOW' FROM #CollateralStaging S WHERE S.ShortfallAmount = 0
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: UPDATE S SET S.ReviewPriority = (CASE WHEN S.ShortfallAmount IS NULL THEN 'NONE' WHEN S.ShortfallAmount > 500000 THEN 'URGENT' WHEN S.ShortfallAmount > 100000 THEN 'HIGH' WHEN S.ShortfallAmount > 0 THEN 'STANDARD' END) FROM #CollateralStaging S WHERE S.OutstandingBalance <= S.CollateralValue
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: UPDATE PRO.LoanAccountCal SET A.CollateralStale = 'Y' WHERE A.SecuredFlag = 'Y' AND NOT EXISTS(SELECT 1 FROM PRO.CollateralValuation AS V WHERE V.AccountId = A.AccountId AND V.ValuationDate >= @StaleValuationCutoff)
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: UPDATE S SET S.ReviewPriority = 'HIGH' FROM #CollateralStaging S WHERE S.ShortfallAmount > 500000
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: UPDATE S SET S.ReviewPriority = 'STANDARD' FROM #CollateralStaging S WHERE S.ShortfallAmount > 0
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: UPDATE A SET A.CollateralStale = 'Y' FROM PRO.LoanAccountCal A WHERE A.SecuredFlag = 'Y' AND NOT EXISTS(SELECT 1 FROM PRO.CollateralValuation AS V WHERE V.AccountId = A.AccountId AND V.ValuationDate >= @StaleValuationCutoff)
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: UPDATE S SET S.ReviewPriority = 'HIGH' FROM #CollateralStaging S WHERE S.OutstandingBalance > S.CollateralValue
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: UPDATE S SET S.ReviewPriority = 'MEDIUM' FROM #CollateralStaging S WHERE S.ShortfallAmount > 0
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: MERGE INTO PRO.CollateralPositionSummary AS Target USING (SELECT AccountId, CollateralValue, ShortfallAmount, ReviewPriority, @ProcessDate AS LastUpdatedDate FROM #CollateralStaging) AS Source ON Target.AccountId = Source.AccountId WHEN MATCHED THEN UPDATE SET Target.CollateralValue = Source.CollateralValue, Target.ShortfallAmount = Source.ShortfallAmount, Target.ReviewPriority = Source.ReviewPriority, Target.LastUpdatedDate = Source.LastUpdatedDate WHEN NOT MATCHED BY TARGET THEN INSERT (AccountId, CollateralValue, ShortfallAmount, ReviewPriority, FirstSeenDate, LastUpdatedDate) VALUES (Source.AccountId, Source.CollateralValue, Source.ShortfallAmount, Source.ReviewPriority, @ProcessDate, @ProcessDate)
