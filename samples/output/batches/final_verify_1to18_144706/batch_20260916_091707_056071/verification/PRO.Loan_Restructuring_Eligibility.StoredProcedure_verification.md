...
| Run Timestamp | `2026-09-16T09:22:13.574943+00:00` |
| Object ID | `obj_e4fc72fc188b` |
...
|---|---:|---:|---:|---:|---|
| extraction | 1 | 1 | 0 | 6843 | available |
| synthesis | 7 | 7 | 0 | 68447 | available |
| synthesis_revision | 1 | 1 | 0 | 29915 | available |
...
| ⚠️ 2 | Insert into temporary table [MATCHED] (`rule__6`) | `AccountId, DecisionDate, EligibleFlag, AssetClassAtEval, RevisedTenureMonths` | Insert records into the #RestructureDecisions table for eligible accounts. |
| ⚠️ 3 | Update RevisedTenureMonths [CONFLICT] (`rule__7`) | `RevisedTenureMonths` | Update the RevisedTenureMonths field in the #RestructureDecisions table based on the OutstandingBalance for eligible accounts. |
| ⚠️ 4 | Update account status and last payment due date [MATCHED] (`rule__9`) | `LastPaymentDueDate, AccountStatus` | Update the AccountStatus and LastPaymentDueDate for eligible accounts. |
| ⚠️ 5 | Insert restructure audit log [MATCHED] (`rule__10`) | `AccountId, DecisionDate, EligibleFlag` | Insert records into the PRO.RestructureAuditLog table for accounts approved in the current run. |
| ⚠️ 6 | Set RestructureEligible to NOT_ASSESSED [MATCHED] (`rule__2`) | `RestructureEligible` | If an account's OutstandingBalance is NULL, set its RestructureEligible status to 'NOT_ASSESSED'. |
| ⚠️ 7 | Set RestructureEligible to SCHEME_CLOSED [LLM_ONLY] (`rule__3`) | `RestructureEligible` | If the process date is not before the scheme cutoff date, set the RestructureEligible status to 'SCHEME_CLOSED'. |
| ⚠️ 8 | Create temporary table [LLM_ONLY] (`rule__5`) | `Not specified` | Create the temporary #RestructureDecisions table. |
| ⚠️ 9 | Merge restructure register [LLM_ONLY] (`rule__8`) | `Target.AccountId, Source.AccountId, Target.EligibleFlag, Source.EligibleFlag, Target.RevisedTenureMonths, Source.RevisedTenureMonths, Target.LastEvaluatedDate, AccountId, EligibleFlag, RevisedTenureMonths, FirstEvaluatedDate, LastEvaluatedDate` | Merge records into the PRO.RestructureRegister table. |
...
|---|---|---|---|---|---|---|
| 1 | Check process date against cutoff date (rule__1) | @ProcessDate < @SchemeCutoffDate | Not cited | 03_batch3_main_body+batch3_nested_block:chunk_text_07 | 03_batch3_main_body+batch3_nested_block:chunk_text_07 | Needs Review |
| 2 | Insert into temporary table (rule__6) | INSERT INTO #RestructureDecisions (AccountId, DecisionDate, EligibleFlag, AssetClassAtEval, RevisedTenureMonths) SELECT A.AccountId, @ProcessDate, A.RestructureEligible, A.AssetClass, NULL FROM PRO.LoanAccountCal A WHERE A.RestructureEligible IS NOT NULL AND… | Not cited | 03_batch3_main_body+batch3_nested_block:chunk_text_20 | 03_batch3_main_body+batch3_nested_block:chunk_text_20 | Verified |
| 3 | Update RevisedTenureMonths (rule__7) | UPDATE D SET D.RevisedTenureMonths = CASE WHEN A.OutstandingBalance > 1000000 THEN 60 WHEN A.OutstandingBalance > 500000 THEN 48 ELSE 36 END FROM #RestructureDecisions D INNER JOIN PRO.LoanAccountCal A ON A.AccountId = D.AccountId WHERE D.EligibleFlag = 'Y' | Not cited | 03_batch3_main_body+batch3_nested_block:chunk_text_19 | 03_batch3_main_body+batch3_nested_block:chunk_text_19 | Needs Review |
| 4 | Update account status and last payment due date (rule__9) | UPDATE A SET A.AccountStatus = 'RESTRUCTURED', A.LastPaymentDueDate = DATEADD(MONTH, D.RevisedTenureMonths, @ProcessDate) FROM PRO.LoanAccountCal A INNER JOIN #RestructureDecisions D ON D.AccountId = A.AccountId WHERE D.EligibleFlag = 'Y' | Not cited | 03_batch3_main_body+batch3_nested_block:chunk_text_20 | 03_batch3_main_body+batch3_nested_block:chunk_text_20 | Verified |
| 5 | Insert restructure audit log (rule__10) | INSERT INTO PRO.RestructureAuditLog (AccountId, DecisionDate, EligibleFlag) SELECT R.AccountId, R.LastEvaluatedDate, R.EligibleFlag FROM PRO.RestructureRegister R WHERE R.LastEvaluatedDate = @ProcessDate AND R.EligibleFlag = 'Y' | Not cited | 03_batch3_main_body+batch3_nested_block:embedded_04_27 | 03_batch3_main_body+batch3_nested_block:embedded_04_27 | Verified |
| 6 | Set RestructureEligible to NOT_ASSESSED (rule__2) | UPDATE A SET A.RestructureEligible = 'NOT_ASSESSED' FROM PRO.LoanAccountCal A WHERE A.OutstandingBalance IS NULL | Not cited | Not cited | Not cited | Verified |
| 7 | Set RestructureEligible to SCHEME_CLOSED (rule__3) | @ProcessDate >= @SchemeCutoffDate | Not cited | Not cited | Not cited | Needs Review |
| 8 | Create temporary table (rule__5) | CREATE TABLE #RestructureDecisions (AccountId VARCHAR(20), DecisionDate DATE, EligibleFlag VARCHAR(15), AssetClassAtEval VARCHAR(20), RevisedTenureMonths INT) | Not cited | Not cited | Not cited | Needs Review |
| 9 | Merge restructure register (rule__8) | MERGE PRO.RestructureRegister AS Target USING #RestructureDecisions AS Source ON Target.AccountId = Source.AccountId WHEN MATCHED THEN UPDATE SET Target.EligibleFlag = Source.EligibleFlag, Target.RevisedTenureMonths = Source.RevisedTenureMonths, Target.LastEv… | Not cited | Not cited | Not cited | Needs Review |
...
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: A.AssetClass = 'AssetClass1' AND A.Balance > 100000 AND A.TenureMonths > 60
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: A.AssetClass = 'AssetClass2' AND A.Balance <= 100000 AND A.TenureMonths <= 60
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: D.AssetClassAtEval = 'AssetClass1' AND D.EligibleFlag = 'Y'
...
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: UPDATE A SET A.RestructureEligible, A.AccountStatus, A.LastPaymentDueDate, A.PriorRestructureCount FROM PRO.LoanAccountCal A INNER JOIN #RestructureDecisions D ON D.AccountId = A.AccountId WHERE A.OutstandingBalance IS NULL AND D.EligibleFlag = 'Y'
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: INSERT INTO #RestructureDecisions (AccountId, DecisionDate, EligibleFlag, AssetClassAtEval, RevisedTenureMonths) SELECT AccountId, @ProcessDate, EligibleFlag, AssetClassAtEval, RevisedTenureMonths FROM PRO.LoanAccountCal WHERE EligibleFlag = 'Y'
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: UPDATE D SET D.RevisedTenureMonths = DATEADD(MONTH, D.RevisedTenureMonths, @ProcessDate) FROM #RestructureDecisions D WHERE D.EligibleFlag = 'Y'
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: MERGE PRO.RestructureRegister AS Target USING (SELECT AccountId, @ProcessDate AS DecisionDate, EligibleFlag, AssetClassAtEval, RevisedTenureMonths FROM #RestructureDecisions) AS Source ON Target.AccountId = Source.AccountId WHEN MATCHED THEN UPDATE SET Target.EligibleFlag = Source.EligibleFlag, Target.RevisedTenureMonths = Source.RevisedTenureMonths, Target.LastEvaluatedDate = Source.DecisionDate WHEN NOT MATCHED BY TARGET THEN INSERT (AccountId, DecisionDate, EligibleFlag, AssetClassAtEval, RevisedTenureMonths) VALUES (Source.AccountId, Source.DecisionDate, Source.EligibleFlag, Source.AssetClassAtEval, Source.RevisedTenureMonths)
