...
| Run Timestamp | `2026-09-16T09:25:54.183141+00:00` |
| Object ID | `obj_9cfe9a17e845` |
...
| Item | Value |
|---|---|
| Run ID | `telemetry_3e88f2644b32` |
| Total LLM Calls | `10` |
| Successful Calls | `10` |
| Failed Calls | `0` |
| Prompt Tokens | `139693` |
| Completion Tokens | `25945` |
| Total Tokens | `165638` |
| Telemetry Availability | `available` |

| Stage | Calls | Success | Failure | Tokens | Availability |
|---|---:|---:|---:|---:|---|
| extraction | 1 | 1 | 0 | 6611 | available |
| synthesis | 7 | 7 | 0 | 91983 | available |
| synthesis_revision | 2 | 2 | 0 | 67044 | available |
...
| ⚠️ 3 | Determine RiskTier [CONFLICT] (`rule__3`) | `RiskTier` | Classify the risk tier for a customer based on their risk score. |
...
| ⚠️ 5 | Stage Updated Risk Scores and Tiers [MATCHED] (`rule__5`) | `Not specified` | Stage the updated risk scores and tiers for customers. |
| ⚠️ 6 | Merge Updated Risk Scores and Tiers into History [LLM_ONLY] (`rule__6`) | `Not specified` | Merge the staged risk scores and tiers into the risk score history. |
| ⚠️ 7 | Log Updated Risk Scores and Tiers for Audit [MATCHED] (`rule__7`) | `Not specified` | Log the updated risk scores and tiers for audit purposes. |
| ⚠️ 8 | Update Customer Risk Profile [CONFLICT] (`rule__8`) | `RiskScore, RiskTier, RecommendedLimitAdjustmentPct` | Update the customer's risk profile with the calculated risk score, risk tier, and recommended limit adjustment percentage. |
...
| 4 | Calculate RecommendedLimitAdjustmentPct (rule__4) | UPDATE C SET C.RecommendedLimitAdjustmentPct = CASE WHEN C.RiskTier = 'LOW' THEN CASE WHEN ISNULL(C.PriorDefaultCount, 0) = 0 THEN 10 ELSE 0 END WHEN C.RiskTier = 'MODERATE' THEN 0 WHEN C.RiskTier = 'HIGH' THEN -15 WHEN C.RiskTier = 'SEVERE' THEN -40 ELSE 0 E… | Not cited | 03_batch3_main_body+batch3_nested_block:embedded_04_32 | dependency_0007; dependency_0008; dependency_0009; dependency_0010; dependency_0011 | Needs Review |
| 5 | Stage Updated Risk Scores and Tiers (rule__5) | INSERT INTO #RiskScoreStaging (CustomerId, RiskScore, RiskTier) SELECT CustomerId, RiskScore, RiskTier FROM PRO.CustomerRiskProfile WHERE RiskScore IS NOT NULL | Not cited | Not cited | Not cited | Verified |
| 6 | Merge Updated Risk Scores and Tiers into History (rule__6) | MERGE PRO.RiskScoreHistory AS Target USING #RiskScoreStaging AS Source ON Target.CustomerId = Source.CustomerId WHEN MATCHED THEN UPDATE SET Target.RiskScore = Source.RiskScore, Target.RiskTier = Source.RiskTier WHEN NOT MATCHED BY TARGET THEN INSERT (Custome… | Not cited | Not cited | Not cited | Needs Review |
| 7 | Log Updated Risk Scores and Tiers for Audit (rule__7) | INSERT INTO PRO.RiskScoreAuditLog (CustomerId, ScoreDate, RiskScore, RiskTier) SELECT CustomerId, @ProcessDate, RiskScore, RiskTier FROM PRO.CustomerRiskProfile WHERE PrevRiskScore IS NULL AND RiskScore IS NOT NULL | Not cited | Not cited | Not cited | Verified |
| 8 | Update Customer Risk Profile (rule__8) | UPDATE PRO.CustomerRiskProfile SET C.RiskScore = (ISNULL(C.OverdueDays, 0) * 0.5) + (ISNULL(C.UtilizationRatio, 0) * 100 * 0.3) + (ISNULL(C.PriorDefaultCount, 0) * 20 * 0.2), C.RiskTier = CASE WHEN C.RiskScore < 20 THEN 'LOW' WHEN C.RiskScore < 50 THEN 'MODER… | Not cited | Not cited | dependency_0007; dependency_0008; dependency_0009; dependency_0010; dependency_0011 | Needs Review |
...
- **Disposition:** covered_by_rule=91, technical_only=2, uncovered=15
...
- `CONFLICT` rule (`recon_1846ba198a5a`): rule__3, 03_batch3_main_body+batch3_nested_block, 03_batch3_main_body+batch3_nested_block:chunk_text_07;03_batch3_main_body+batch3_nested_block:chunk_text_19;03_batch3_main_body+batch3_nested_block:chunk_text_20;03_batch3_main_body+batch3_nested_block:chunk_text_21;03_batch3_main_body+batch3_nested_block:embedded_03_31;03_batch3_main_body+batch3_nested_block:embedded_06_34;03_batch3_main_body+batch3_nested_block:embedded_08_36 - Deterministic evidence conflicts with the synthesized claim.
- `CONFLICT` rule (`recon_e78b6f47b6a9`): rule__4, 03_batch3_main_body+batch3_nested_block, 03_batch3_main_body+batch3_nested_block:chunk_text_10;03_batch3_main_body+batch3_nested_block:chunk_text_14;03_batch3_main_body+batch3_nested_block:embedded_04_32;03_batch3_main_body+batch3_nested_block:embedded_05_33 - Deterministic evidence conflicts with the synthesized claim.
- `LLM_ONLY` rule (`recon_8e56815cd25f`): rule__6 - No deterministic evidence was found for this claim.

## Quality Summary

- **Overall status:** REVIEW_REQUIRED
- **Quality score:** 87.65131578947368/100
...
- `MEDIUM` Field Conflict on `rule__8`: Synthesized rule affects different fields than the deterministic evidence.
...
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: MERGE PRO.RiskScoreHistory AS Target
        USING #RiskScoreStaging AS Source
        ON Target.CustomerId = Source.CustomerId
        WHEN MATCHED THEN
            UPDATE SET Target.RiskScore = Source.RiskScore, Target.RiskTier = Source.RiskTier
        WHEN NOT MATCHED BY TARGET THEN
            INSERT (CustomerId, RiskScore, RiskTier, FirstScoredDate, LastScoredDate)
            VALUES (Source.CustomerId, Source.RiskScore, Source.RiskTier, GETDATE(), GETDATE())
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: INSERT INTO PRO.RiskScoreAuditLog (CustomerId, ScoreDate, RiskScore, RiskTier)
        SELECT CustomerId, GETDATE() AS ScoreDate, RiskScore, RiskTier
        FROM #RiskScoreStaging
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: UPDATE C SET C.RiskTier = CASE WHEN C.RiskScore <= 50 THEN 'LOW' WHEN C.RiskScore > 50 AND C.RiskScore <= 100 THEN 'MODERATE' WHEN C.RiskScore > 100 AND C.RiskScore <= 150 THEN 'HIGH' ELSE 'SEVERE' END FROM PRO.CustomerRiskProfile C WHERE C.RiskScore IS NOT NULL
...
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: UPDATE C
        SET C.RiskTier =
            CASE
                WHEN C.RiskScore < 50 THEN 'LOW'
                WHEN C.RiskScore >= 50 AND C.RiskScore < 75 THEN 'MODERATE'
                WHEN C.RiskScore >= 75 AND C.RiskScore < 100 THEN 'HIGH'
                WHEN C.RiskScore >= 100 THEN 'SEVERE'
            END
        FROM PRO.CustomerRiskProfile C
