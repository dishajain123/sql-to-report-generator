...
| Run Timestamp | `2026-09-16T09:23:05.120914+00:00` |
| Object ID | `obj_8ece2542e210` |
...
| Item | Value |
|---|---|
| Run ID | `telemetry_c5044a1568fb` |
| Total LLM Calls | `10` |
| Successful Calls | `10` |
| Failed Calls | `0` |
| Prompt Tokens | `112529` |
| Completion Tokens | `18668` |
| Total Tokens | `131197` |
| Telemetry Availability | `available` |

| Stage | Calls | Success | Failure | Tokens | Availability |
|---|---:|---:|---:|---:|---|
| extraction | 1 | 1 | 0 | 6370 | available |
| synthesis | 7 | 7 | 0 | 70237 | available |
| synthesis_revision | 2 | 2 | 0 | 54590 | available |

## Business Rule Summary

| Priority | Rule | Output | Business Purpose |
|---|---|---|---|
| ⚠️ 1 | Calculate CoverageRatio [MATCHED] (`rule__1`) | `CoverageRatio` | Determine the coverage ratio for accounts with a non-zero outstanding balance. |
| ⚠️ 2 | Update ReviewReason based on CoverageRatio [CONFLICT] (`rule__2`) | `ReviewReason` | Update the ReviewReason field in the #ProvisionCoverage table based on the CoverageRatio. |
| ⚠️ 3 | Append quarter-end to review reason [MATCHED] (`rule__3`) | `ReviewReason` | Append '_QUARTER_END' to the review reason if the process date is after the quarter start date plus one month. |
| 🟠 4 | Determine CoverageRatio [MATCHED] (`deterministic_decision_1348_1980_3_coverageratio`) | `CoverageRatio` | First matching row wins. SQL type conversion still applies; ELSE includes false or NULL predicates. |
| 🟠 5 | Determine ReviewReason [MATCHED] (`deterministic_case_0056_0062_2036_reviewreason`) | `ReviewReason` | First matching row wins; ELSE includes false or NULL predicates. |
| 🟠 6 | Determine ReviewReason [MATCHED] (`deterministic_tsql_if_0068_0083_reviewreason`) | `ReviewReason` | Not specified |
| 🟠 7 | Determine Reason [MATCHED] (`deterministic_decision_4861_5218_2_reason`) | `Reason` | First matching row wins. SQL type conversion still applies; ELSE includes false or NULL predicates. |
| ⚠️ 8 | Insert initial data [CONFLICT] (`rule__9`) | `AccountId, OutstandingBalance, ProvisionAmount, CoverageRatio, ReviewReason` | Insert initial provisioning coverage data into the #ProvisionCoverage temporary table. |
| ⚠️ 9 | Merge provisioning coverage summary [LLM_ONLY] (`rule__5`) | `AccountId, OutstandingBalance, ProvisionAmount, CoverageRatio, FirstSeenDate, LastUpdatedDate` | Merge the latest provisioning coverage data into the PRO.ProvisionCoverageSummary table. |
| ⚠️ 10 | Update BelowThresholdFlag for CoverageRatio < 0.5 [LLM_ONLY] (`rule__6`) | `BelowThresholdFlag` | Set BelowThresholdFlag to 'Y' for accounts with a CoverageRatio less than 0.5 and LastUpdatedDate equal to the process date. |
| ⚠️ 11 | Insert provisioning coverage data [MATCHED] (`rule__12`) | `AccountId, EscalationDate, Reason` | Insert provisioning coverage data into the PRO.CollectionsQueue table based on the CoverageRatio. |
...
|---|---|---|---|---|---|---|
| 1 | Calculate CoverageRatio (rule__1) | CASE WHEN A.OutstandingBalance > 0 THEN A.ProvisionAmount / A.OutstandingBalance ELSE NULL END | Not cited | 03_batch3_main_body+batch3_nested_block:chunk_text_08 | Not cited | Verified |
| 2 | Update ReviewReason based on CoverageRatio (rule__2) | CASE WHEN S.CoverageRatio IS NULL THEN 'NO_BALANCE' WHEN S.CoverageRatio < 0.25 THEN 'SEVERE_UNDERCOVER' WHEN S.CoverageRatio < 0.5 THEN 'MODERATE_UNDERCOVER' WHEN S.CoverageRatio >= 0.5 AND S.CoverageRatio < 1 THEN 'ADEQUATE' ELSE 'FULLY_COVERED' END | Not cited | 03_batch3_main_body+batch3_nested_block:chunk_text_12 | Not cited | Needs Review |
| 3 | Append quarter-end to review reason (rule__3) | UPDATE S SET S.ReviewReason = S.ReviewReason + '_QUARTER_END' FROM #ProvisionCoverage S WHERE S.CoverageRatio IS NOT NULL AND S.CoverageRatio < 0.6 | Not cited | 03_batch3_main_body+batch3_nested_block:chunk_text_12 | Not cited | Verified |
| 4 | Determine CoverageRatio (deterministic_decision_1348_1980_3_coverageratio) | Not cited | source \| Lines 40-54 | Not cited | Not cited | Verified |
| 5 | Determine ReviewReason (deterministic_case_0056_0062_2036_reviewreason) | Not cited | source \| Lines 56-62 | Not cited | Not cited | Verified |
| 6 | Determine ReviewReason (deterministic_tsql_if_0068_0083_reviewreason) | Not cited | source \| Lines 68-83 | Not cited | Not cited | Verified |
| 7 | Determine Reason (deterministic_decision_4861_5218_2_reason) | Not cited | source \| Lines 117-124 | Not cited | Not cited | Verified |
| 8 | Insert initial data (rule__9) | INSERT INTO #ProvisionCoverage (AccountId, OutstandingBalance, ProvisionAmount, CoverageRatio, ReviewReason) SELECT A.AccountId, A.OutstandingBalance, A.ProvisionAmount, CASE WHEN A.OutstandingBalance > 0 THEN A.ProvisionAmount / A.OutstandingBalance ELSE NUL… | Not cited | Not cited | Not cited | Needs Review |
| 9 | Merge provisioning coverage summary (rule__5) | MERGE PRO.ProvisionCoverageSummary AS Target USING #ProvisionCoverage AS Source ON Target.AccountId = Source.AccountId WHEN MATCHED THEN UPDATE SET Target.OutstandingBalance = Source.OutstandingBalance, Target.ProvisionAmount = Source.ProvisionAmount, Target.… | Not cited | Not cited | Not cited | Needs Review |
| 10 | Update BelowThresholdFlag for CoverageRatio < 0.5 (rule__6) | UPDATE PRO.ProvisionCoverageSummary SET BelowThresholdFlag = 'Y' WHERE T.CoverageRatio < 0.5 AND T.LastUpdatedDate = @ProcessDate | Not cited | Not cited | Not cited | Needs Review |
| 11 | Insert provisioning coverage data (rule__12) | INSERT INTO PRO.CollectionsQueue (AccountId, EscalationDate, Reason) SELECT AccountId, @ProcessDate, CASE WHEN CoverageRatio < 0.25 THEN 'SEVERE_PROVISION_SHORTFALL' ELSE 'MODERATE_PROVISION_SHORTFALL' END FROM #ProvisionCoverage WHERE ReviewReason LIKE '%UND… | Not cited | Not cited | Not cited | Verified |
...
| decision_chain_006:branch_001 | CoverageRatio < 0.25 | source |
| decision_chain_006:branch_002 | ELSE | source |
...
| IF_BRANCH | covered_by_rule | samples/09_Provision_Coverage_Merge.sql | full_source | CoverageRatio < 0.25 |
| ELSE | covered_by_rule | samples/09_Provision_Coverage_Merge.sql | full_source | ELSE |
...
- **By rule type:** deterministic_decision_table = 4, explicit = 7
- **By validation status:** unverified = 4, verified = 7
...
- **Quality score:** 75.34545454545454/100
...
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: MERGE PRO.ProvisionCoverageSummary AS Target USING #ProvisionCoverage AS Source ON Target.AccountId = Source.AccountId WHEN MATCHED THEN UPDATE SET Target.OutstandingBalance = Source.OutstandingBalance, Target.ProvisionAmount = Source.ProvisionAmount, Target.CoverageRatio = Source.CoverageRatio, Target.LastUpdatedDate = Source.LastUpdatedDate WHEN NOT MATCHED BY TARGET THEN INSERT (AccountId, OutstandingBalance, ProvisionAmount, CoverageRatio, FirstSeenDate, LastUpdatedDate) VALUES (Source.AccountId, Source.OutstandingBalance, Source.ProvisionAmount, Source.CoverageRatio, GETDATE(), GETDATE())
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: UPDATE PRO.ProvisionCoverageSummary SET BelowThresholdFlag = 'Y' WHERE CoverageRatio < 0.5 AND LastUpdatedDate = @ProcessDate
...
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: UPDATE S SET S.ReviewReason = CASE WHEN CoverageRatio < 0.25 THEN 'SEVERE_PROVISION_SHORTFALL_QUARTER_END' ELSE 'MODERATE_PROVISION_SHORTFALL_QUARTER_END' END FROM #ProvisionCoverage S WHERE ReviewReason LIKE '%UNDERCOVER%' AND CoverageRatio IS NOT NULL
