...
| Run Timestamp | `2026-09-16T10:07:12.774150+00:00` |
| Object ID | `obj_890a2d3ea714` |
...
| Item | Value |
|---|---|
| Run ID | `telemetry_dbc6e8ec43d5` |
| Total LLM Calls | `7` |
| Successful Calls | `7` |
| Failed Calls | `0` |
| Prompt Tokens | `51694` |
| Completion Tokens | `9172` |
| Total Tokens | `60866` |
| Telemetry Availability | `available` |

| Stage | Calls | Success | Failure | Tokens | Availability |
|---|---:|---:|---:|---:|---|
| extraction | 1 | 1 | 0 | 4831 | available |
| synthesis | 4 | 4 | 0 | 23582 | available |
| synthesis_revision | 2 | 2 | 0 | 32453 | available |

## Business Rule Summary

| Priority | Rule | Output | Business Purpose |
|---|---|---|---|
| 🟢 1 | Determine Provision Percentage [MATCHED] (`rule__1__2`) | `ProvisionPct` | Set the provision percentage for accounts based on their asset class. |
| 🟢 2 | Increase provision percentage [MATCHED] (`rule__1__5`) | `ProvisionPct` | For unsecured accounts with a non-standard asset class, the provision percentage is increased by 10%. |
| ⚠️ 3 | Calculate Provision Amount [CONFLICT] (`rule__2__6`) | `ProvisionAmount` | Calculate the provision amount for accounts with a positive outstanding balance. |
| 🟢 4 | Limit provision amount [MATCHED] (`rule__3__7`) | `ProvisionAmount` | The provision amount is capped at the outstanding balance if it exceeds it. |
| 🟢 5 | Set senior review flag [MATCHED] (`rule__4`) | `SeniorReviewFlag` | A senior review flag is set for accounts with a provision amount over 1 million. |
| 🟢 6 | Update run status [MATCHED] (`rule__5`) | `RunCount, COMPLETED, ErrorDate, ErrorDescription` | The process run status is updated to mark the process as completed and increment the run count. |
| 🔴 7 | Update run status on error [CONFLICT] (`rule__1__10`) | `RunStatus, COMPLETED, ErrorDate, ErrorDescription, RunCount` | If an error occurs during the provision percentage calculation, update the run status to reflect the error. |
...
|---|---|---|---|---|---|---|
| 1 | Determine Provision Percentage (rule__1__2) | CASE A.AssetClass WHEN 'STANDARD' THEN 0.40 WHEN 'SUBSTANDARD' THEN 15.00 WHEN 'DOUBTFUL' THEN 25.00 WHEN 'LOSS' THEN 100.00 ELSE 0.00 END | Not cited | 03_batch3_main_body+batch3_nested_block:chunk_text_01 | Not cited | Verified |
| 2 | Increase provision percentage (rule__1__5) | UPDATE A SET A.ProvisionPct = A.ProvisionPct + 10.00 FROM PRO.AccountCal A WHERE A.SecuredFlag = 'N' AND A.AssetClass <> 'STANDARD' | Not cited | Not cited | Not cited | Verified |
| 3 | Calculate Provision Amount (rule__2__6) | UPDATE A SET A.ProvisionAmount = (A.OutstandingBalance * A.ProvisionPct) / 100 FROM PRO.AccountCal A WHERE A.OutstandingBalance > 0 | Not cited | Not cited | Not cited | Needs Review |
| 4 | Limit provision amount (rule__3__7) | UPDATE A SET A.ProvisionAmount = A.OutstandingBalance FROM PRO.AccountCal A WHERE A.ProvisionAmount > A.OutstandingBalance | Not cited | Not cited | Not cited | Verified |
| 5 | Set senior review flag (rule__4) | UPDATE A SET A.SeniorReviewFlag = 'Y' FROM PRO.AccountCal A WHERE A.ProvisionAmount > 1000000 | Not cited | Not cited | Not cited | Verified |
| 6 | Update run status (rule__5) | UPDATE PRO.RunStatus SET COMPLETED = 'Y', ErrorDate = NULL, ErrorDescription = NULL, RunCount = ISNULL(RunCount, 0) + 1 WHERE ProcessName = 'Provision_Percentage_Calculation' | Not cited | Not cited | Not cited | Verified |
| 7 | Update run status on error (rule__1__10) | UPDATE PRO.RunStatus SET COMPLETED = 'N', ErrorDate = GETDATE(), ErrorDescription = ERROR_MESSAGE(), RunCount = ISNULL(RunCount, 0) + 1 WHERE ProcessName = 'Provision_Percentage_Calculation' | Not cited | Not cited | source_sql | Needs Review |
...
- `CONFLICT` rule (`recon_2a47aa4a8141`): rule__2__6, 03_batch3_main_body+batch3_nested_block:chunk_text_05;03_batch3_main_body+batch3_nested_block:embedded_03_12 - Deterministic evidence conflicts with the synthesized claim.
- `CONFLICT` rule (`recon_ac679e82670a`): rule__1__10, 03_batch3_main_body+batch3_nested_block:chunk_text_08;03_batch3_main_body+batch3_nested_block:embedded_06_15;04_batch3_exception:chunk_text_02;04_batch3_exception:embedded_01_05 - Deterministic evidence conflicts with the synthesized claim.
...
- `HIGH` Condition Conflict on `rule__2__6`: Synthesized condition conflicts with deterministic predicate evidence.
- `MEDIUM` Field Conflict on `rule__1__10`: Synthesized rule affects different fields than the deterministic evidence.
