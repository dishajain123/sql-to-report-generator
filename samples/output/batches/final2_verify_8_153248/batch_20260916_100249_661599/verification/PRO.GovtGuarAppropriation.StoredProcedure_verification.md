...
| Run Timestamp | `2026-09-16T10:06:30.377748+00:00` |
| Object ID | `obj_b4a046e0936e` |
...
|---|---:|---:|---:|---:|---|
| extraction | 1 | 1 | 0 | 4732 | available |
| synthesis | 2 | 2 | 0 | 13774 | available |
| synthesis_revision | 2 | 2 | 0 | 25376 | available |

## Business Rule Summary

| Priority | Rule | Output | Business Purpose |
|---|---|---|---|
| 🟠 1 | Determine GovGur [MATCHED] (`deterministic_case_0034_0036_879_govgur`) | `GovGur` | First matching row wins; ELSE includes false or NULL predicates. |
| 🟢 2 | Reset AppGovGur to zero [MATCHED] (`rule__1`) | `AppGovGur` | If the customer is not currently being processed, the AppGovGur field is reset to zero. |
| 🟢 3 | Calculate government guarantee amount [MATCHED] (`rule__2`) | `GovGur` | For accounts with a facility type of 'BP' or 'BD' and a non-zero government guarantee amount, calculate the government guarantee amount bas… |
| 🟢 4 | Update AppGovGur from temporary table [MATCHED] (`rule__3`) | `AppGovGur` | For accounts with a facility type of 'BP' or 'BD', update the AppGovGur field using the calculated values from the temporary table. |
| 🟢 5 | Update AppGovGur for non-BP/BD accounts [MATCHED] (`rule__4`) | `AppGovGur` | For accounts with a facility type other than 'BP' or 'BD', update the AppGovGur field using the government guarantee amount directly. |
...
|---|---|---|---|---|---|---|
| 1 | Determine GovGur (deterministic_case_0034_0036_879_govgur) | Not cited | source \| Lines 34-36 | Not cited | Not cited | Verified |
| 2 | Reset AppGovGur to zero (rule__1) | UPDATE A SET A.AppGovGur =0 FROM ##ACCOUNTCAL A INNER JOIN ##CustomerCal B ON A.CustomerEntityID=B.CustomerEntityID WHERE B.FlgProcessing='N' | Not cited | Not cited | Not cited | Verified |
| 3 | Calculate government guarantee amount (rule__2) | SELECT A.AccountEntityID,(CASE WHEN SUM(A.NetBalance) OVER (PARTITION BY A.CustomerEntityId) > 0 THEN (A.GovtGtyAmt * (A.NetBalance / SUM(A.NetBalance) OVER (PARTITION BY A.CustomerEntityId))) END) GovGur INTO #TEMPTABLEAppGovGur FROM ##ACCOUNTCAL A WHERE A.F… | Not cited | Not cited | Not cited | Verified |
| 4 | Update AppGovGur from temporary table (rule__3) | UPDATE A SET A.AppGovGur=B.GovGur FROM ##ACCOUNTCAL A INNER JOIN #TEMPTABLEAppGovGur B ON A.AccountEntityID=B.AccountEntityID WHERE A.FacilityType IN('BP','BD') | Not cited | Not cited | Not cited | Verified |
| 5 | Update AppGovGur for non-BP/BD accounts (rule__4) | UPDATE A SET A.AppGovGur=GovtGtyAmt FROM ##ACCOUNTCAL A WHERE NOT (A.FacilityType IN('BP','BD')) | Not cited | Not cited | Not cited | Verified |
...
- **Disposition:** covered_by_rule=18, technical_only=11, uncovered=19

| Construct | Status | Source location | Statement / chunk | Evidence |
|---|---|---|---|---|
| STATEMENT | uncovered | Lines 1-2 | 00_main_body+nested_block:chunk_text_01, 00_main_body+nested_block | BEGIN SET NOCOUNT ON |
| UPDATE | uncovered | Lines 4-6 | 00_main_body+nested_block:chunk_text_02, 00_main_body+nested_block | BEGIN TRY /*-----UPDATE AppGovGur =0 --------------------------*/ |
...
| STATEMENT | uncovered | Lines 12-12 | 00_main_body+nested_block:chunk_text_04, 00_main_body+nested_block | IF OBJECT_ID('TEMPDB..#TEMPTABLEAppGovGur') IS NOT NULL |
| STATEMENT | uncovered | Lines 13-13 | 00_main_body+nested_block:chunk_text_05, 00_main_body+nested_block | DROP TABLE #TEMPTABLEAppGovGur |
...
| UPDATE | uncovered | Lines 37-40 | 00_main_body+nested_block:chunk_text_10, 00_main_body+nested_block | DROP TABLE #TEMPTABLEAppGovGur -----------------Added for DashBoard 04-03-2021 --Update BANDAUDITSTATUS set CompletedCount=CompletedCount+1 where BandName='ASSET CLASSIFICATION' |
| STATEMENT | uncovered | Lines 42-42 | 00_main_body+nested_block:chunk_text_11, 00_main_body+nested_block | END TRY |
...
| UPDATE | uncovered | unavailable | 00_main_body+nested_block:embedded_05_16, 00_main_body+nested_block:embedded_05_16, 00_main_body+nested_block | UPDATE PRO.ACLRUNNINGPROCESSSTATUS SET COMPLETED='Y',ERRORDATE=NULL,ERRORDESCRIPTION=NULL,COUNT=ISNULL(COUNT,0)+1 WHERE RUNNINGPROCESSNAME='GovtGuarAppropriation' |
...
- **Quality score:** 85.11111111111111/100
