...
| Run Timestamp | `2026-09-16T10:07:56.575763+00:00` |
| Object ID | `obj_8ece2542e210` |
...
| Item | Value |
|---|---|
| Run ID | `telemetry_8887e0095a14` |
| Total LLM Calls | `10` |
| Successful Calls | `10` |
| Failed Calls | `0` |
| Prompt Tokens | `112171` |
| Completion Tokens | `19390` |
| Total Tokens | `131561` |
| Telemetry Availability | `available` |

| Stage | Calls | Success | Failure | Tokens | Availability |
|---|---:|---:|---:|---:|---|
| extraction | 1 | 1 | 0 | 6470 | available |
| synthesis | 7 | 7 | 0 | 70512 | available |
| synthesis_revision | 2 | 2 | 0 | 54579 | available |

## Business Rule Summary

| Priority | Rule | Output | Business Purpose |
|---|---|---|---|
| ⚠️ 1 | Insert initial coverage data [CONFLICT] (`rule__2`) | `CoverageRatio, AccountId, OutstandingBalance, ProvisionAmount, ReviewReason` | Insert initial provisioning coverage data into the #ProvisionCoverage table for accounts with a non-null outstanding balance. |
| ⚠️ 2 | Calculate CoverageRatio [MATCHED] (`rule__3`) | `CoverageRatio` | Determine the coverage ratio for accounts with a non-zero outstanding balance. |
| ⚠️ 3 | Update ReviewReason based on CoverageRatio [CONFLICT] (`rule__4`) | `ReviewReason` | Update the ReviewReason in the #ProvisionCoverage table based on the CoverageRatio. |
| ⚠️ 4 | Update ReviewReason for CoverageRatio < 0.6 [MATCHED] (`rule__5`) | `ReviewReason` | Append '_QUARTER_END' to the ReviewReason for accounts with a CoverageRatio less than 0.6. |
| ⚠️ 5 | Update BelowThresholdFlag for CoverageRatio < 0.5 [MATCHED] (`rule__6`) | `BelowThresholdFlag` | Set BelowThresholdFlag to 'Y' for accounts with a CoverageRatio less than 0.5 and a LastUpdatedDate equal to the process date. |
| ⚠️ 6 | Insert provisioning shortfall severity [CONFLICT] (`rule__8`) | `Reason` | Insert provisioning shortfall severity into PRO.CollectionsQueue based on the CoverageRatio. |
| 🟠 7 | Determine ReviewReason [MATCHED] (`deterministic_case_0056_0062_2036_reviewreason`) | `ReviewReason` | First matching row wins; ELSE includes false or NULL predicates. |
| 🟠 8 | Determine ReviewReason [MATCHED] (`deterministic_tsql_if_0068_0083_reviewreason`) | `ReviewReason` | Not specified |
| 🟠 9 | Determine Reason [MATCHED] (`deterministic_decision_4861_5218_2_reason`) | `Reason` | First matching row wins. SQL type conversion still applies; ELSE includes false or NULL predicates. |
| ⚠️ 10 | Merge provisioning coverage summary [LLM_ONLY] (`rule__7`) | `Target.AccountId, Source.AccountId, Target.OutstandingBalance, Source.OutstandingBalance, Target.ProvisionAmount, Source.ProvisionAmount, Target.CoverageRatio, Source.CoverageRatio, Target.LastUpdatedDate, AccountId, OutstandingBalance, ProvisionAmount, CoverageRatio, FirstSeenDate, LastUpdatedDate` | Merge provisioning coverage summary data into the PRO.ProvisionCoverageSummary table. |
...
|---|---|---|---|---|---|---|
| 1 | Insert initial coverage data (rule__2) | INSERT INTO #ProvisionCoverage (AccountId, OutstandingBalance, ProvisionAmount, CoverageRatio, ReviewReason) SELECT A.AccountId, A.OutstandingBalance, A.ProvisionAmount, CASE WHEN A.OutstandingBalance > 0 THEN A.ProvisionAmount / A.OutstandingBalance ELSE NUL… | Not cited | 03_batch3_main_body+batch3_nested_block:chunk_text_08 | 03_batch3_main_body+batch3_nested_block:chunk_text_08 | Needs Review |
| 2 | Calculate CoverageRatio (rule__3) | CASE WHEN A.OutstandingBalance > 0 THEN A.ProvisionAmount / A.OutstandingBalance ELSE NULL END | Not cited | 03_batch3_main_body+batch3_nested_block:chunk_text_08 | 03_batch3_main_body+batch3_nested_block:chunk_text_08 | Verified |
| 3 | Update ReviewReason based on CoverageRatio (rule__4) | UPDATE S SET S.ReviewReason = (CASE WHEN S.CoverageRatio IS NULL THEN 'NO_BALANCE' WHEN S.CoverageRatio < 0.25 THEN 'SEVERE_UNDERCOVER' WHEN S.CoverageRatio < 0.5 THEN 'MODERATE_UNDERCOVER' WHEN S.CoverageRatio >= 0.5 AND S.CoverageRatio < 1 THEN 'ADEQUATE' E… | Not cited | 03_batch3_main_body+batch3_nested_block:chunk_text_12 | 03_batch3_main_body+batch3_nested_block:chunk_text_12 | Needs Review |
| 4 | Update ReviewReason for CoverageRatio < 0.6 (rule__5) | UPDATE S SET S.ReviewReason = S.ReviewReason + '_QUARTER_END' FROM #ProvisionCoverage S WHERE S.CoverageRatio IS NOT NULL AND S.CoverageRatio < 0.6 | Not cited | 03_batch3_main_body+batch3_nested_block:chunk_text_12 | 03_batch3_main_body+batch3_nested_block:chunk_text_12 | Verified |
| 5 | Update BelowThresholdFlag for CoverageRatio < 0.5 (rule__6) | UPDATE PRO.ProvisionCoverageSummary SET T.BelowThresholdFlag = 'Y' WHERE T.CoverageRatio < 0.5 AND T.LastUpdatedDate = @ProcessDate | Not cited | 03_batch3_main_body+batch3_nested_block:embedded_09_32 | 03_batch3_main_body+batch3_nested_block:embedded_09_32 | Verified |
| 6 | Insert provisioning shortfall severity (rule__8) | INSERT INTO PRO.CollectionsQueue (AccountId, EscalationDate, Reason) SELECT AccountId, @ProcessDate, CASE WHEN CoverageRatio < 0.25 THEN 'SEVERE_PROVISION_SHORTFALL' ELSE 'MODERATE_PROVISION_SHORTFALL' END FROM #ProvisionCoverage WHERE ReviewReason LIKE '%UND… | Not cited | 03_batch3_main_body+batch3_nested_block:embedded_08_31 | 03_batch3_main_body+batch3_nested_block:embedded_08_31 | Needs Review |
| 7 | Determine ReviewReason (deterministic_case_0056_0062_2036_reviewreason) | Not cited | source \| Lines 56-62 | Not cited | Not cited | Verified |
| 8 | Determine ReviewReason (deterministic_tsql_if_0068_0083_reviewreason) | Not cited | source \| Lines 68-83 | Not cited | Not cited | Verified |
| 9 | Determine Reason (deterministic_decision_4861_5218_2_reason) | Not cited | source \| Lines 117-124 | Not cited | Not cited | Verified |
| 10 | Merge provisioning coverage summary (rule__7) | MERGE PRO.ProvisionCoverageSummary AS Target USING #ProvisionCoverage AS Source ON Target.AccountId = Source.AccountId WHEN MATCHED THEN UPDATE SET Target.OutstandingBalance = Source.OutstandingBalance, Target.ProvisionAmount = Source.ProvisionAmount, Target.… | Not cited | Not cited | Not cited | Needs Review |
...
- **Disposition:** covered_by_rule=78, technical_only=9, uncovered=17
...
| IF_BRANCH | uncovered | samples/09_Provision_Coverage_Merge.sql | full_source | T.CoverageRatio IS NOT NULL AND T.CoverageRatio < 0.5 AND T.LastUpdatedDate = @ProcessDate |
...
- **By rule type:** deterministic_decision_table = 3, explicit = 7
- **By validation status:** unverified = 4, verified = 6
...
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: UPDATE #ProvisionCoverage SET S.ReviewReason = 'SEVERE_PROVISION_SHORTFALL' WHERE CoverageRatio < 0.25
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: SELECT AccountId, @ProcessDate, CASE WHEN CoverageRatio < 0.25 THEN 'SEVERE_PROVISION_SHORTFALL' ELSE 'MODERATE_PROVISION_SHORTFALL' END FROM #ProvisionCoverage
...
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: UPDATE PRO.ProvisionCoverageSummary SET T.BelowThresholdFlag = 'Y' WHERE T.CoverageRatio < 0.5 AND T.LastUpdatedDate = @ProcessDate
