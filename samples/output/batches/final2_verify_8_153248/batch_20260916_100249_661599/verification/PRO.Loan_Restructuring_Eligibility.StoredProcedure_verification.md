...
| Run Timestamp | `2026-09-16T10:05:28.278896+00:00` |
| Object ID | `obj_e4fc72fc188b` |
...
| Item | Value |
|---|---|
| Run ID | `telemetry_01945f9fc770` |
| Total LLM Calls | `10` |
| Successful Calls | `10` |
| Failed Calls | `0` |
| Prompt Tokens | `107707` |
| Completion Tokens | `20215` |
| Total Tokens | `127922` |
| Telemetry Availability | `available` |

| Stage | Calls | Success | Failure | Tokens | Availability |
|---|---:|---:|---:|---:|---|
| extraction | 1 | 1 | 0 | 7928 | available |
| synthesis | 7 | 7 | 0 | 67766 | available |
| synthesis_revision | 2 | 2 | 0 | 52228 | available |

## Business Rule Summary

| Priority | Rule | Output | Business Purpose |
|---|---|---|---|
| 🔴 1 | Determine restructuring eligibility [CONFLICT] (`rule__1`) | `RestructureEligible` | Set the 'RestructureEligible' field for loan accounts based on asset class, prior restructure count, and overdue days. |
| 🔴 2 | Update revised tenure months [CONFLICT] (`rule__4`) | `RevisedTenureMonths` | Update the 'RevisedTenureMonths' field in the '#RestructureDecisions' table based on the outstanding balance. |
| 🔴 3 | Update account status and payment due date [CONFLICT] (`rule__5`) | `AccountStatus, LastPaymentDueDate, PriorRestructureCount` | Update the account status, last payment due date, and prior restructure count for eligible accounts. |
| 🔴 4 | Insert into restructure audit log [CONFLICT] (`rule__6`) | `AccountId, DecisionDate, EligibleFlag` | Insert records into the 'PRO.RestructureAuditLog' table for accounts approved in the current run. |
| 🟠 5 | Set restructuring eligibility to 'SCHEME_CLOSED' [LLM_ONLY] (`rule__2`) | `RestructureEligible` | Set the 'RestructureEligible' field to 'SCHEME_CLOSED' for all accounts if the process date is not before the scheme cutoff date. |
| 🟢 6 | Set restructuring eligibility to 'NOT_ASSESSED' [MATCHED] (`rule__3`) | `RestructureEligible` | Set the 'RestructureEligible' field to 'NOT_ASSESSED' for accounts with a NULL outstanding balance. |
...
|---|---|---|---|---|---|---|
| 1 | Determine restructuring eligibility (rule__1) | UPDATE A SET A.RestructureEligible = CASE WHEN A.AssetClass IN ('STANDARD', 'SMA') THEN CASE WHEN A.PriorRestructureCount IS NULL OR A.PriorRestructureCount = 0 THEN CASE WHEN A.OverdueDays <= 60 THEN 'Y' | Not cited | 03_batch3_main_body+batch3_nested_block:chunk_text_07 | Not cited | Needs Review |
| 2 | Update revised tenure months (rule__4) | UPDATE D SET D.RevisedTenureMonths = (CASE WHEN A.OutstandingBalance > 1000000 THEN 60 WHEN A.OutstandingBalance > 500000 THEN 48 ELSE 36 END) FROM #RestructureDecisions D INNER JOIN PRO.LoanAccountCal A ON A.AccountId = D.AccountId WHERE D.EligibleFlag = 'Y' | Not cited | 03_batch3_main_body+batch3_nested_block:chunk_text_19 | Not cited | Needs Review |
| 3 | Update account status and payment due date (rule__5) | UPDATE A SET A.AccountStatus = 'RESTRUCTURED', A.LastPaymentDueDate = DATEADD(MONTH, D.RevisedTenureMonths, @ProcessDate), A.PriorRestructureCount = ISNULL(A.PriorRestructureCount, 0) + 1 FROM PRO.LoanAccountCal A INNER JOIN #RestructureDecisions D ON D.Accou… | Not cited | 03_batch3_main_body+batch3_nested_block:chunk_text_20 | Not cited | Needs Review |
| 4 | Insert into restructure audit log (rule__6) | INSERT INTO PRO.RestructureAuditLog (AccountId, DecisionDate, EligibleFlag) SELECT R.AccountId, R.LastEvaluatedDate, R.EligibleFlag FROM PRO.RestructureRegister R WHERE R.LastEvaluatedDate = @ProcessDate AND R.EligibleFlag = 'Y' | Not cited | 03_batch3_main_body+batch3_nested_block:embedded_04_27 | Not cited | Needs Review |
| 5 | Set restructuring eligibility to 'SCHEME_CLOSED' (rule__2) | UPDATE A SET A.RestructureEligible = 'SCHEME_CLOSED' FROM PRO.LoanAccountCal A | Not cited | Not cited | Not cited | Needs Review |
| 6 | Set restructuring eligibility to 'NOT_ASSESSED' (rule__3) | UPDATE A SET A.RestructureEligible = 'NOT_ASSESSED' FROM PRO.LoanAccountCal A WHERE A.OutstandingBalance IS NULL | Not cited | Not cited | Not cited | Verified |
...
| decision_chain_003:branch_001 | A.AssetClass IN ('STANDARD', 'SMA') AND (A.PriorRestructureCount IS NULL OR A.PriorRestructureCount = 0) AND A.OverdueDays <= 60 | source |
| decision_chain_003:branch_002 | A.AssetClass IN ('STANDARD', 'SMA') AND (A.PriorRestructureCount IS NULL OR A.PriorRestructureCount = 0) AND A.OverdueDays > 60 | source |
| decision_chain_003:branch_003 | A.AssetClass IN ('STANDARD', 'SMA') AND A.PriorRestructureCount = 1 AND A.OverdueDays <= 30 AND A.OutstandingBalance IS NOT NULL | source |
| decision_chain_003:branch_004 | A.AssetClass IN ('STANDARD', 'SMA') AND (A.PriorRestructureCount = 1 AND (A.OverdueDays > 30 OR A.OutstandingBalance IS NULL)) | source |
| decision_chain_003:branch_005 | A.AssetClass NOT IN ('STANDARD', 'SMA') | source |
...
- **Disposition:** covered_by_rule=76, technical_only=5, uncovered=25
...
| IF_BRANCH | covered_by_rule | samples/08_Loan_Restructuring_Eligibility.sql | full_source | A.AssetClass IN ('STANDARD', 'SMA') AND (A.PriorRestructureCount IS NULL OR A.PriorRestructureCount = 0) AND A.OverdueDays <= 60 |
| IF_BRANCH | covered_by_rule | samples/08_Loan_Restructuring_Eligibility.sql | full_source | A.AssetClass IN ('STANDARD', 'SMA') AND (A.PriorRestructureCount IS NULL OR A.PriorRestructureCount = 0) AND A.OverdueDays > 60 |
| IF_BRANCH | uncovered | samples/08_Loan_Restructuring_Eligibility.sql | full_source | A.AssetClass IN ('STANDARD', 'SMA') AND A.PriorRestructureCount = 1 AND A.OverdueDays <= 30 AND A.OutstandingBalance IS NOT NULL |
| IF_BRANCH | uncovered | samples/08_Loan_Restructuring_Eligibility.sql | full_source | A.AssetClass IN ('STANDARD', 'SMA') AND (A.PriorRestructureCount = 1 AND (A.OverdueDays > 30 OR A.OutstandingBalance IS NULL)) |
| IF_BRANCH | uncovered | samples/08_Loan_Restructuring_Eligibility.sql | full_source | A.AssetClass NOT IN ('STANDARD', 'SMA') |
...
| CASE_BRANCH | uncovered | Lines 109-109 | unavailable | WHEN MATCHED THEN |
| CASE_BRANCH | uncovered | Lines 114-114 | unavailable | WHEN NOT MATCHED BY TARGET THEN |
...
- **By validation status:** unverified = 5, verified = 1
...
- **Quality score:** 85.58805031446539/100
...
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: A.AssetClass = 'SpecificAssetClass' AND A.DpdDays > 90
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: A.AssetClass = 'DifferentAssetClass' AND A.DpdDays <= 90
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: D.AssetClassAtEval = 'SpecificAssetClass' AND D.EligibleFlag = 'Y'
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: UPDATE A SET A.RestructureEligible = CASE WHEN A.AssetClass IN ('STANDARD', 'SMA') THEN CASE WHEN A.PriorRestructureCount IS NULL OR A.PriorRestructureCount = 0 THEN CASE WHEN A.OverdueDays <= 60 THEN 'Y'
...
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: UPDATE #RestructureDecisions SET D.RevisedTenureMonths = RevisedTenureMonths
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: UPDATE A SET A.RestructureEligible = 'N' FROM PRO.LoanAccountCal A WHERE A.OutstandingBalance IS NULL, UPDATE A SET A.RestructureEligible = 'Y' FROM PRO.LoanAccountCal A WHERE A.OverdueDays <= 30 AND A.OutstandingBalance IS NOT NULL
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: UPDATE #RestructureDecisions SET D.RevisedTenureMonths = specific value WHERE D.EligibleFlag = 'Y'
...
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: A.RestructureEligible IS NOT NULL AND A.RestructureEligible <> 'NOT_ASSESSED' AND D.EligibleFlag = 'Y'
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: UPDATE PRO.LoanAccountCal SET A.RestructureEligible = 'Y' WHERE specific condition 1, UPDATE PRO.LoanAccountCal SET A.RestructureEligible = 'N' WHERE specific condition 2
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: UPDATE #RestructureDecisions SET D.RevisedTenureMonths = new value WHERE specific condition
