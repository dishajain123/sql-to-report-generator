...
> 2 chunk extraction call(s) and 1 rule synthesis section(s) failed after retries and were excluded rather than aborting the whole run. This report may be missing evidence or business rules from the affected portion of the source. Re-run to attempt full coverage.
...
- Chunk '02_batch2_declaration' (batch2_declaration) technical extraction returned malformed JSON and needs manual review.
- Chunk '03_batch3_main_body+batch3_nested_block' (batch3_main_body+batch3_nested_block) technical extraction returned malformed JSON and needs manual review.
