...
| Run Timestamp | `2026-09-16T03:34:16.636687+00:00` |
| Object ID | `obj_ff7556b8a40e` |
...
| Item | Value |
|---|---|
| Run ID | `telemetry_22e65bdf283d` |
| Total LLM Calls | `4` |
| Successful Calls | `4` |
| Failed Calls | `0` |
| Prompt Tokens | `137121` |
| Completion Tokens | `15487` |
| Total Tokens | `152608` |
| Telemetry Availability | `available` |

| Stage | Calls | Success | Failure | Tokens | Availability |
|---|---:|---:|---:|---:|---|
| extraction | 1 | 1 | 0 | 9104 | available |
| synthesis | 2 | 2 | 0 | 71279 | available |
| synthesis_revision | 1 | 1 | 0 | 72225 | available |

## Business Rule Summary

| Priority | Rule | Output | Business Purpose |
|---|---|---|---|
| 🟢 1 | Calculate DpdDays for due accounts [MATCHED] (`rule__1__9`) | `DpdDays` | For loan accounts with a due date, calculate the number of days past due based on the process date. |
| 🟢 2 | Set DpdDays for non-due accounts [MATCHED] (`rule__2__10`) | `DpdDays, DpdBucket` | For loan accounts without a due date, set the number of days past due to zero and the bucket to 'NOT_APPLICABLE'. |
| 🟢 3 | Classify accounts into overdue buckets [MATCHED] (`rule__3__11`) | `DpdBucket` | Classify loan accounts into overdue buckets based on the number of days past due. |
| 🟢 4 | Determine worsened bucket status [MATCHED] (`rule__4__12`) | `BucketWorsened` | Determine if the account's overdue status has worsened since the last process run. |
| 🟢 5 | Calculate penalty interest amount [MATCHED] (`rule__5__13`) | `PenalInterestAmount` | Calculate the penalty interest amount for loan accounts based on the overdue bucket classification. |
| 🟢 6 | Adjust penalty interest amount by facility type [MATCHED] (`rule__6__14`) | `AdjustedPenalty` | Adjust the penalty interest amount based on the facility type for accounts with worsened status. |
| 🟢 7 | Stage adjusted penalty interest amount [MATCHED] (`rule__7__15`) | `AccountId, DpdBucket, FacilityType, AdjustedPenalty` | Stage the adjusted penalty interest amount for further processing. |
| 🟢 8 | Merge staged data into DpdBucket history [MATCHED] (`rule__8__16`) | `Target.DpdBucket, Target.AdjustedPenalty, Target.LastUpdatedDate` | Merge the staged data into the DpdBucket history table, updating existing records or inserting new ones. |
| 🟢 9 | Insert into collections queue [MATCHED] (`rule__9`) | `AccountId, EscalationDate, Reason` | Insert records into the collections queue for accounts that have worsened. |
| 🟠 10 | Determine BucketWorsened [MATCHED] (`deterministic_tsql_if_0071_0094_bucketworsened`) | `BucketWorsened` | Not specified |
| 🟠 11 | Determine Reason [MATCHED] (`deterministic_decision_5813_6511_2_reason`) | `Reason` | First matching row wins. SQL type conversion still applies; ELSE includes false or NULL predicates. |
| 🟠 12 | Update DpdDays for certain conditions [LLM_ONLY] (`rule__1`) | `DpdDays` | Update the DpdDays field in the PRO.LoanAccountCal table based on specific conditions. |
| 🟠 13 | Update DpdBucket based on DpdDays and FacilityType [LLM_ONLY] (`rule__2`) | `DpdBucket` | Update the DpdBucket field in the PRO.LoanAccountCal table based on the DpdDays and FacilityType. |
| 🔴 14 | Update BucketWorsened and GracePeriodApplied [CONFLICT] (`rule__3`) | `BucketWorsened, GracePeriodApplied` | Update the BucketWorsened and GracePeriodApplied fields in the PRO.LoanAccountCal table based on the DpdBucket and FacilityType. |
| 🟠 15 | Update PenalInterestAmount [LLM_ONLY] (`rule__4`) | `PenalInterestAmount` | Update the PenalInterestAmount field in the PRO.LoanAccountCal table. |
| 🟠 16 | Insert and update records in #DpdStaging [LLM_ONLY] (`rule__5`) | `AccountId, DpdBucket, FacilityType, AdjustedPenalty` | Insert records into the #DpdStaging table and update the AdjustedPenalty field based on certain conditions. |
| 🟠 17 | Merge records into PRO.DpdBucketHistory [LLM_ONLY] (`rule__6`) | `AccountId, DpdBucket, AdjustedPenalty, LastUpdatedDate` | Merge records into the PRO.DpdBucketHistory table. |
| 🟠 18 | Insert records into PRO.CollectionsQueue [LLM_ONLY] (`rule__7`) | `AccountId, EscalationDate, Reason` | Insert records into the PRO.CollectionsQueue table. |
| 🟠 19 | Insert records into PRO.DpdBucketAuditLog [LLM_ONLY] (`rule__8`) | `AccountId, TransitionDate, NewBucket` | Insert records into the PRO.DpdBucketAuditLog table. |
...
|---|---|---|---|---|---|---|
| 1 | Calculate DpdDays for due accounts (rule__1__9) | UPDATE A SET A.DpdDays = DATEDIFF(DAY, A.LastPaymentDueDate, @ProcessDate) FROM PRO.LoanAccountCal A WHERE A.LastPaymentDueDate IS NOT NULL AND A.LastPaymentDueDate <= @ProcessDate | Not cited | 04_batch3_nested_block:chunk_text_04 | dependency_0009; dependency_0010; dependency_0011; dependency_0012 | Verified |
| 2 | Set DpdDays for non-due accounts (rule__2__10) | UPDATE A SET A.DpdDays = 0, A.DpdBucket = 'NOT_APPLICABLE' FROM PRO.LoanAccountCal A WHERE A.LastPaymentDueDate IS NULL | Not cited | 04_batch3_nested_block:chunk_text_05 | dependency_0013; dependency_0014; dependency_0015 | Verified |
| 3 | Classify accounts into overdue buckets (rule__3__11) | UPDATE A SET A.DpdBucket = ( CASE WHEN A.DpdDays IS NULL THEN 'NOT_APPLICABLE' WHEN A.DpdDays = 0 THEN 'CURRENT' WHEN A.DpdDays BETWEEN 1 AND 30 THEN 'BUCKET_1_30' WHEN A.DpdDays BETWEEN 31 AND 60 THEN 'BUCKET_31_60' WHEN A.DpdDays BETWEEN 61 AND 90 THEN 'BUC… | Not cited | 04_batch3_nested_block:chunk_text_06 | dependency_0016; dependency_0017; dependency_0018 | Verified |
| 4 | Determine worsened bucket status (rule__4__12) | UPDATE A SET A.BucketWorsened = 'Y' FROM PRO.LoanAccountCal A WHERE A.PrevDpdBucket IS NOT NULL AND A.DpdBucket <> A.PrevDpdBucket AND A.DpdDays > ISNULL(A.PrevDpdDays, 0) | Not cited | 04_batch3_nested_block:chunk_text_10 | dependency_0021; dependency_0022; dependency_0023; dependency_0024; dependency_0025; dependency_0026; _(+13 more instance(s) not shown)_ | Verified |
| 5 | Calculate penalty interest amount (rule__5__13) | UPDATE A SET A.PenalInterestAmount = ( CASE WHEN A.DpdBucket = 'BUCKET_1_30' THEN (A.OutstandingBalance * 0.02) / 365 * A.DpdDays WHEN A.DpdBucket = 'BUCKET_31_60' THEN (A.OutstandingBalance * 0.03) / 365 * A.DpdDays WHEN A.DpdBucket IN ('BUCKET_61_90', 'BUCK… | Not cited | 04_batch3_nested_block:chunk_text_07 | dependency_0020; dependency_0021; dependency_0022; dependency_0023; dependency_0024; dependency_0025; _(+14 more instance(s) not shown)_ | Verified |
| 6 | Adjust penalty interest amount by facility type (rule__6__14) | UPDATE S SET S.AdjustedPenalty = ( CASE WHEN S.FacilityType IN ('CC', 'OD') THEN S.AdjustedPenalty * 1.10 WHEN S.FacilityType IN ('TL', 'DL') THEN S.AdjustedPenalty * 1.05 ELSE S.AdjustedPenalty END ) FROM #DpdStaging S WHERE S.AdjustedPenalty IS NOT NULL | Not cited | 04_batch3_nested_block:chunk_text_24 | dependency_0039; dependency_0040; dependency_0041; dependency_0042; dependency_0043 | Verified |
| 7 | Stage adjusted penalty interest amount (rule__7__15) | INSERT INTO #DpdStaging (AccountId, DpdBucket, FacilityType, AdjustedPenalty) SELECT A.AccountId, A.DpdBucket, A.FacilityType, A.PenalInterestAmount FROM PRO.LoanAccountCal A WHERE A.BucketWorsened = 'Y' | Not cited | 04_batch3_nested_block:chunk_text_23 | dependency_0040; dependency_0041; dependency_0042; dependency_0043 | Verified |
| 8 | Merge staged data into DpdBucket history (rule__8__16) | MERGE PRO.DpdBucketHistory AS Target USING #DpdStaging AS Source ON Target.AccountId = Source.AccountId WHEN MATCHED THEN UPDATE SET Target.DpdBucket = Source.DpdBucket, Target.AdjustedPenalty = Source.AdjustedPenalty, Target.LastUpdatedDate = @ProcessDate WH… | Not cited | 04_batch3_nested_block:chunk_text_25 | dependency_0043; dependency_0044; dependency_0045 | Verified |
| 9 | Insert into collections queue (rule__9) | INSERT INTO PRO.CollectionsQueue (AccountId, EscalationDate, Reason) SELECT AccountId, @ProcessDate, CASE WHEN DpdBucket IN ('BUCKET_61_90', 'BUCKET_90_PLUS') AND FacilityType IN ('CC', 'OD') THEN 'SEVERE_DPD_CASH_CREDIT' WHEN DpdBucket IN ('BUCKET_61_90', 'B… | Not cited | 04_batch3_nested_block:chunk_text_26 | dependency_0045; dependency_0046; dependency_0047; dependency_0048; dependency_0049 | Verified |
...
| 11 | Determine Reason (deterministic_decision_5813_6511_2_reason) | Not cited | source \| Lines 143-157 | Not cited | Not cited | Verified |
| 12 | Update DpdDays for certain conditions (rule__1) | DpdDays > 90 | Not cited | Not cited | Not cited | Needs Review |
| 13 | Update DpdBucket based on DpdDays and FacilityType (rule__2) | DpdDays > 90 AND FacilityType IN ('CC', 'OD') | Not cited | Not cited | Not cited | Needs Review |
| 14 | Update BucketWorsened and GracePeriodApplied (rule__3) | DpdBucket IN ('BUCKET_61_90', 'BUCKET_90_PLUS') AND FacilityType IN ('CC', 'OD') | Not cited | Not cited | Not cited | Needs Review |
| 15 | Update PenalInterestAmount (rule__4) | PenalInterestAmount is not null | Not cited | Not cited | Not cited | Needs Review |
| 16 | Insert and update records in #DpdStaging (rule__5) | AccountId, DpdBucket, FacilityType, AdjustedPenalty are not null | Not cited | Not cited | Not cited | Needs Review |
| 17 | Merge records into PRO.DpdBucketHistory (rule__6) | Target.AccountId, Source.AccountId, Target.DpdBucket, Source.DpdBucket, Target.AdjustedPenalty, Source.AdjustedPenalty, Target.LastUpdatedDate, AccountId, DpdBucket, AdjustedPenalty, FirstFlaggedDate, LastUpdatedDate are not null | Not cited | Not cited | Not cited | Needs Review |
| 18 | Insert records into PRO.CollectionsQueue (rule__7) | AccountId, EscalationDate, Reason are not null | Not cited | Not cited | Not cited | Needs Review |
| 19 | Insert records into PRO.DpdBucketAuditLog (rule__8) | AccountId, TransitionDate, NewBucket are not null | Not cited | Not cited | Not cited | Needs Review |

### Decision-Chain Branch Provenance

| Branch | Condition | Source Location |
|---|---|---|
| case_0043_0050_1482:branch_001 | A.DpdDays IS NULL | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql \| Lines 44-45 \| Chunk 04_batch3_nested_block \| Statement 04_batch3_nested_block:chunk_text_07 |
| case_0043_0050_1482:branch_002 | A.DpdDays = 0 | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql \| Lines 45-46 \| Chunk 04_batch3_nested_block \| Statement 04_batch3_nested_block:chunk_text_07 |
| case_0043_0050_1482:branch_003 | A.DpdDays BETWEEN 1 AND 30 | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql \| Lines 46-47 \| Chunk 04_batch3_nested_block \| Statement 04_batch3_nested_block:chunk_text_07 |
| case_0043_0050_1482:branch_004 | A.DpdDays BETWEEN 31 AND 60 | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql \| Lines 47-48 \| Chunk 04_batch3_nested_block \| Statement 04_batch3_nested_block:chunk_text_07 |
| case_0043_0050_1482:branch_005 | A.DpdDays BETWEEN 61 AND 90 | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql \| Lines 48-49 \| Chunk 04_batch3_nested_block \| Statement 04_batch3_nested_block:chunk_text_07 |
| case_0043_0050_1482:branch_006 | ELSE | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql \| Lines 49-50 \| Chunk 04_batch3_nested_block \| Statement 04_batch3_nested_block:chunk_text_07 |
| case_0059_0064_2167:branch_001 | A.DpdBucket = 'BUCKET_1_30' | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql \| Lines 60-61 \| Chunk 04_batch3_nested_block \| Statement 04_batch3_nested_block:chunk_text_08 |
| case_0059_0064_2167:branch_002 | A.DpdBucket = 'BUCKET_31_60' | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql \| Lines 61-62 \| Chunk 04_batch3_nested_block \| Statement 04_batch3_nested_block:chunk_text_10 |
| case_0059_0064_2167:branch_003 | A.DpdBucket IN ('BUCKET_61_90', 'BUCKET_90_PLUS') | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql \| Lines 62-63 \| Chunk 04_batch3_nested_block \| Statement 04_batch3_nested_block:chunk_text_10 |
| case_0059_0064_2167:branch_004 | ELSE | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql \| Lines 63-64 \| Chunk 04_batch3_nested_block \| Statement 04_batch3_nested_block:chunk_text_12 |
| tsql_if_0071_0094:branch_001 | EXISTS (SELECT 1 FROM PRO.LoanAccountCal WHERE LastPaymentDueDate >= @GraceWindowStart) | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql \| Lines 72-78 \| Chunk 04_batch3_nested_block \| Statement 04_batch3_nested_block:chunk_text_18 |
| tsql_if_0071_0094:branch_002 | EXISTS (SELECT 1 FROM PRO.LoanAccountCal WHERE PrevDpdBucket IS NOT NULL) | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql \| Lines 80-87 \| Chunk 04_batch3_nested_block \| Statement 04_batch3_nested_block:chunk_text_22 |
| tsql_if_0071_0094:branch_003 | ELSE | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql \| Lines 89-93 \| Chunk 04_batch3_nested_block \| Statement 04_batch3_nested_block:chunk_text_23 |
| case_0117_0121_4529:branch_001 | S.FacilityType IN ('CC', 'OD') | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql \| Lines 118-119 \| Chunk 04_batch3_nested_block \| Statement 04_batch3_nested_block:chunk_text_25 |
| case_0117_0121_4529:branch_002 | S.FacilityType IN ('TL', 'DL') | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql \| Lines 119-120 \| Chunk 04_batch3_nested_block \| Statement 04_batch3_nested_block:chunk_text_25 |
| case_0117_0121_4529:branch_003 | ELSE | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql \| Lines 120-121 \| Chunk 04_batch3_nested_block \| Statement 04_batch3_nested_block:chunk_text_25 |
| decision_5813_6511_2:branch_001 | DpdBucket IN ('BUCKET_61_90', 'BUCKET_90_PLUS') AND FacilityType IN ('CC', 'OD') | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql \| Lines 143-157 \| Chunk 04_batch3_nested_block \| Statement 04_batch3_nested_block:chunk_text_26 |
| decision_5813_6511_2:branch_002 | DpdBucket IN ('BUCKET_61_90', 'BUCKET_90_PLUS') | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql \| Lines 143-157 \| Chunk 04_batch3_nested_block \| Statement 04_batch3_nested_block:chunk_text_26 |
| decision_5813_6511_2:branch_003 | ELSE | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql \| Lines 143-157 \| Chunk 04_batch3_nested_block \| Statement 04_batch3_nested_block:chunk_text_26 |
...
| decision_chain_008:branch_001 | S.FacilityType IN ('CC', 'OD') | source |
| decision_chain_008:branch_002 | S.FacilityType IN ('TL', 'DL') | source |
| decision_chain_008:branch_003 | ELSE | source |
| decision_chain_009:branch_001 | DpdBucket IN ('BUCKET_61_90', 'BUCKET_90_PLUS') AND FacilityType IN ('CC', 'OD') | source |
| decision_chain_009:branch_002 | DpdBucket IN ('BUCKET_61_90', 'BUCKET_90_PLUS') | source |
...
- **Executable constructs:** 152
- **Disposition:** covered_by_rule=134, technical_only=4, uncovered=14
...
| UPDATE | covered_by_rule | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql / Lines 21-174 | 04_batch3_nested_block:chunk_text_04, 04_batch3_nested_block:chunk_text_04, 04_batch3_nested_block | UPDATE A SET A.DpdDays = DATEDIFF(DAY, A.LastPaymentDueDate, @ProcessDate) FROM PRO.LoanAccountCal A WHERE A.LastPaymentDueDate IS NOT NULL AND A.LastPaymentDueDate <= @ProcessDate -- Rule 2: accounts with no due date on record cannot be... |
| UPDATE | covered_by_rule | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql / Lines 21-174 | 04_batch3_nested_block:chunk_text_05, 04_batch3_nested_block:chunk_text_05, 04_batch3_nested_block | UPDATE A SET A.DpdDays = 0, A.DpdBucket = 'NOT_APPLICABLE' FROM PRO.LoanAccountCal A WHERE A.LastPaymentDueDate IS NULL -- Rule 3: multi-branch bucket classification by DPD range |
| UPDATE | covered_by_rule | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql / Lines 21-174 | 04_batch3_nested_block:chunk_text_06, 04_batch3_nested_block:chunk_text_06, 04_batch3_nested_block | UPDATE A SET A.DpdBucket = ( CASE WHEN A.DpdDays IS NULL THEN 'NOT_APPLICABLE' WHEN A.DpdDays = 0 THEN 'CURRENT' WHEN A.DpdDays BETWEEN 1 AND 30 THEN 'BUCKET_1_30' WHEN A.DpdDays BETWEEN 31 AND 60 THEN 'BUCKET_31_60' WHEN A.DpdDays BETWE... |
| UPDATE | covered_by_rule | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql / Lines 21-174 | 04_batch3_nested_block:chunk_text_07, 04_batch3_nested_block:chunk_text_07, 04_batch3_nested_block | UPDATE A SET A.PenalInterestAmount = ( CASE WHEN A.DpdBucket = 'BUCKET_1_30' THEN (A.OutstandingBalance * 0.02) / 365 * A.DpdDays WHEN A.DpdBucket = 'BUCKET_31_60' THEN (A.OutstandingBalance * 0.03) / 365 * A.DpdDays WHEN A.DpdBucket IN... |
...
| UPDATE | covered_by_rule | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql / Lines 21-174 | 04_batch3_nested_block:chunk_text_10, 04_batch3_nested_block:chunk_text_10, 04_batch3_nested_block | UPDATE A SET A.BucketWorsened = 'N', A.GracePeriodApplied = 'Y' FROM PRO.LoanAccountCal A WHERE A.LastPaymentDueDate >= @GraceWindowStart |
...
| UPDATE | covered_by_rule | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql / Lines 21-174 | 04_batch3_nested_block:chunk_text_14, 04_batch3_nested_block:chunk_text_14, 04_batch3_nested_block | UPDATE A SET A.BucketWorsened = 'Y' FROM PRO.LoanAccountCal A WHERE A.PrevDpdBucket IS NOT NULL AND A.DpdBucket <> A.PrevDpdBucket AND A.DpdDays > ISNULL(A.PrevDpdDays, 0) |
| UPDATE | covered_by_rule | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql / Lines 21-174 | 04_batch3_nested_block:chunk_text_18, 04_batch3_nested_block:chunk_text_18, 04_batch3_nested_block | UPDATE A SET A.BucketWorsened = 'N' FROM PRO.LoanAccountCal A |
| INSERT_TEMP | technical_only | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql / Lines 21-174 | 04_batch3_nested_block:chunk_text_23, 04_batch3_nested_block:chunk_text_23, 04_batch3_nested_block | INSERT INTO #DpdStaging (AccountId, DpdBucket, FacilityType, AdjustedPenalty) SELECT A.AccountId, A.DpdBucket, A.FacilityType, A.PenalInterestAmount FROM PRO.LoanAccountCal A WHERE A.BucketWorsened = 'Y' -- Rule 7: derived-assignment UPD... |
| UPDATE_TEMP | technical_only | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql / Lines 21-174 | 04_batch3_nested_block:chunk_text_24, 04_batch3_nested_block:chunk_text_24, 04_batch3_nested_block | UPDATE S SET S.AdjustedPenalty = ( CASE WHEN S.FacilityType IN ('CC', 'OD') THEN S.AdjustedPenalty * 1.10 WHEN S.FacilityType IN ('TL', 'DL') THEN S.AdjustedPenalty * 1.05 ELSE S.AdjustedPenalty END ) FROM #DpdStaging S WHERE S.AdjustedP... |
| MERGE | covered_by_rule | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql / Lines 21-174 | 04_batch3_nested_block:chunk_text_25, 04_batch3_nested_block:chunk_text_25, 04_batch3_nested_block | MERGE PRO.DpdBucketHistory AS Target USING #DpdStaging AS Source ON Target.AccountId = Source.AccountId WHEN MATCHED THEN UPDATE SET Target.DpdBucket = Source.DpdBucket, Target.AdjustedPenalty = Source.AdjustedPenalty, Target.LastUpdated... |
| INSERT | covered_by_rule | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql / Lines 21-174 | 04_batch3_nested_block:chunk_text_26, 04_batch3_nested_block:chunk_text_26, 04_batch3_nested_block | INSERT INTO PRO.CollectionsQueue (AccountId, EscalationDate, Reason) SELECT AccountId, @ProcessDate, CASE WHEN DpdBucket IN ('BUCKET_61_90', 'BUCKET_90_PLUS') AND FacilityType IN ('CC', 'OD') THEN 'SEVERE_DPD_CASH_CREDIT' WHEN DpdBucket... |
| INSERT | covered_by_rule | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql / Lines 21-174 | 04_batch3_nested_block:chunk_text_27, 04_batch3_nested_block:chunk_text_27, 04_batch3_nested_block | INSERT INTO PRO.DpdBucketAuditLog (AccountId, TransitionDate, NewBucket) SELECT H.AccountId, @ProcessDate, H.DpdBucket FROM PRO.DpdBucketHistory H WHERE H.LastUpdatedDate = @ProcessDate |
| UPDATE | covered_by_rule | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql / Lines 21-174 | 04_batch3_nested_block:chunk_text_28, 04_batch3_nested_block:chunk_text_28, 04_batch3_nested_block | UPDATE PRO.ACLRUNNINGPROCESSSTATUS SET COMPLETED = 'Y', ERRORDATE = NULL, ERRORDESCRIPTION = NULL, COUNT = ISNULL(COUNT, 0) + 1 WHERE RUNNINGPROCESSNAME = 'DPD_Bucket_Classification' |
| UPDATE | covered_by_rule | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql / Lines 21-174 | 04_batch3_nested_block:chunk_text_31, 04_batch3_nested_block:chunk_text_31, 04_batch3_nested_block | UPDATE PRO.ACLRUNNINGPROCESSSTATUS SET COMPLETED = 'N', ERRORDATE = GETDATE(), ERRORDESCRIPTION = ERROR_MESSAGE(), COUNT = ISNULL(COUNT, 0) + 1 WHERE RUNNINGPROCESSNAME = 'DPD_Bucket_Classification' |
...
| CASE | covered_by_rule | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql / Lines 44-45 | 04_batch3_nested_block:chunk_text_07, 04_batch3_nested_block | A.DpdDays IS NULL |
| CASE | covered_by_rule | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql / Lines 45-46 | 04_batch3_nested_block:chunk_text_07, 04_batch3_nested_block | A.DpdDays = 0 |
| CASE | covered_by_rule | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql / Lines 46-47 | 04_batch3_nested_block:chunk_text_07, 04_batch3_nested_block | A.DpdDays BETWEEN 1 AND 30 |
| CASE | covered_by_rule | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql / Lines 47-48 | 04_batch3_nested_block:chunk_text_07, 04_batch3_nested_block | A.DpdDays BETWEEN 31 AND 60 |
| CASE | covered_by_rule | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql / Lines 48-49 | 04_batch3_nested_block:chunk_text_07, 04_batch3_nested_block | A.DpdDays BETWEEN 61 AND 90 |
| ELSE | covered_by_rule | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql / Lines 49-50 | 04_batch3_nested_block:chunk_text_07, 04_batch3_nested_block | ELSE |
| CASE | covered_by_rule | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql / Lines 60-61 | 04_batch3_nested_block:chunk_text_08, 04_batch3_nested_block | A.DpdBucket = 'BUCKET_1_30' |
| CASE | covered_by_rule | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql / Lines 61-62 | 04_batch3_nested_block:chunk_text_10, 04_batch3_nested_block | A.DpdBucket = 'BUCKET_31_60' |
| CASE | covered_by_rule | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql / Lines 62-63 | 04_batch3_nested_block:chunk_text_10, 04_batch3_nested_block | A.DpdBucket IN ('BUCKET_61_90', 'BUCKET_90_PLUS') |
| ELSE | covered_by_rule | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql / Lines 63-64 | 04_batch3_nested_block:chunk_text_12, 04_batch3_nested_block | ELSE |
| IF_BRANCH | covered_by_rule | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql / Lines 72-78 | 04_batch3_nested_block:chunk_text_18, 04_batch3_nested_block | EXISTS (SELECT 1 FROM PRO.LoanAccountCal WHERE LastPaymentDueDate >= @GraceWindowStart) |
| IF_BRANCH | covered_by_rule | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql / Lines 80-87 | 04_batch3_nested_block:chunk_text_22, 04_batch3_nested_block | EXISTS (SELECT 1 FROM PRO.LoanAccountCal WHERE PrevDpdBucket IS NOT NULL) |
| ELSE | covered_by_rule | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql / Lines 89-93 | 04_batch3_nested_block:chunk_text_23, 04_batch3_nested_block | ELSE |
| CASE | covered_by_rule | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql / Lines 118-119 | 04_batch3_nested_block:chunk_text_25, 04_batch3_nested_block | S.FacilityType IN ('CC', 'OD') |
| CASE | covered_by_rule | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql / Lines 119-120 | 04_batch3_nested_block:chunk_text_25, 04_batch3_nested_block | S.FacilityType IN ('TL', 'DL') |
| ELSE | covered_by_rule | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql / Lines 120-121 | 04_batch3_nested_block:chunk_text_25, 04_batch3_nested_block | ELSE |
| IF_BRANCH | covered_by_rule | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql / Lines 143-157 | 04_batch3_nested_block:chunk_text_26, 04_batch3_nested_block | DpdBucket IN ('BUCKET_61_90', 'BUCKET_90_PLUS') AND FacilityType IN ('CC', 'OD') |
| IF_BRANCH | covered_by_rule | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql / Lines 143-157 | 04_batch3_nested_block:chunk_text_26, 04_batch3_nested_block | DpdBucket IN ('BUCKET_61_90', 'BUCKET_90_PLUS') |
| ELSE | covered_by_rule | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql / Lines 143-157 | 04_batch3_nested_block:chunk_text_26, 04_batch3_nested_block | ELSE |
| IF_BRANCH | covered_by_rule | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql | full_source | A.DpdDays IS NULL |
| IF_BRANCH | covered_by_rule | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql | full_source | A.DpdDays = 0 |
| IF_BRANCH | covered_by_rule | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql | full_source | A.DpdDays BETWEEN 1 AND 30 |
| IF_BRANCH | covered_by_rule | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql | full_source | A.DpdDays BETWEEN 31 AND 60 |
| IF_BRANCH | covered_by_rule | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql | full_source | A.DpdDays BETWEEN 61 AND 90 |
| ELSE | covered_by_rule | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql | full_source | ELSE |
| IF_BRANCH | covered_by_rule | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql | full_source | A.DpdBucket = 'BUCKET_1_30' |
| IF_BRANCH | covered_by_rule | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql | full_source | A.DpdBucket = 'BUCKET_31_60' |
| IF_BRANCH | covered_by_rule | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql | full_source | A.DpdBucket IN ('BUCKET_61_90', 'BUCKET_90_PLUS') |
| ELSE | covered_by_rule | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql | full_source | ELSE |
| IF_BRANCH | covered_by_rule | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql | full_source | S.FacilityType IN ('CC', 'OD') |
| IF_BRANCH | covered_by_rule | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql | full_source | S.FacilityType IN ('TL', 'DL') |
| ELSE | covered_by_rule | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql | full_source | ELSE |
| IF_BRANCH | covered_by_rule | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql | full_source | DpdBucket IN ('BUCKET_61_90', 'BUCKET_90_PLUS') AND FacilityType IN ('CC', 'OD') |
| IF_BRANCH | covered_by_rule | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql | full_source | DpdBucket IN ('BUCKET_61_90', 'BUCKET_90_PLUS') |
| ELSE | covered_by_rule | /var/folders/d5/q6cyw5854xz1ksr1wj0fmf5r0000gn/T/tmpbyj9on31.sql | full_source | ELSE |
...
- **By rule type:** deterministic_decision_table = 2, explicit = 17
- **By validation status:** unverified = 8, verified = 11
...
- `CONFLICT` tables_written (`recon_303872d07307`): full_source
- `LLM_ONLY` tables_written (`recon_1d5a5a696580`): full_source
- `LLM_ONLY` rule (`recon_f44ff64ab9b0`): rule__1 - No deterministic evidence was found for this claim.
- `LLM_ONLY` rule (`recon_63176898399f`): rule__2 - No deterministic evidence was found for this claim.

## Quality Summary

- **Overall status:** REVIEW_REQUIRED
- **Quality score:** 77.17386162034299/100
- **Statement coverage:** 34 / 55 (61.8%)
- **Rule grounding coverage:** 10 / 19 (52.6%)
...
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: DpdDays > 90 AND FacilityType IN ('CC', 'OD')
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: DpdBucket IN ('BUCKET_61_90', 'BUCKET_90_PLUS') AND FacilityType IN ('CC', 'OD')
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: PenalInterestAmount is not null
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: AccountId, DpdBucket, FacilityType, AdjustedPenalty are not null
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: Target.AccountId, Source.AccountId, Target.DpdBucket, Source.DpdBucket, Target.AdjustedPenalty, Source.AdjustedPenalty, Target.LastUpdatedDate, AccountId, DpdBucket, AdjustedPenalty, FirstFlaggedDate, LastUpdatedDate are not null
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: AccountId, EscalationDate, Reason are not null
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: AccountId, TransitionDate, NewBucket are not null
