...
| Run Timestamp | `2026-09-03T23:26:30.296585+00:00` |
| Object ID | `obj_4cb17b45df7c` |
...
|---|---:|---:|---:|---:|---|
| extraction | 1 | 1 | 0 | 4718 | available |
| synthesis | 1 | 1 | 0 | 11688 | available |
| synthesis_revision | 2 | 2 | 0 | 30801 | available |

## Business Rule Summary

| Priority | Rule | Output | Business Purpose |
|---|---|---|---|
| 🟠 1 | Classify account as Standard [LLM_ONLY] (`rule_01`) | `asset_classification, provisioning_percentage` | Classifies the account as Standard if overdue days are less than or equal to 90. |
...
| 🟠 3 | Classify account as Doubtful1 [LLM_ONLY] (`rule_03`) | `asset_classification, provisioning_percentage` | Classifies the account as Doubtful1 if overdue days are between 366 and 1095 and doubtful since days are less than or equal to 365. |
| 🟠 4 | Classify account as Doubtful2 [LLM_ONLY] (`rule_04`) | `asset_classification, provisioning_percentage` | Classifies the account as Doubtful2 if overdue days are between 366 and 1095 and doubtful since days are greater than 365. |
| 🟠 5 | Classify account as Loss [LLM_ONLY] (`rule_05`) | `asset_classification, provisioning_percentage` | Classifies the account as Loss if doubtful since days are greater than 1095. |
| 🟠 6 | Classify account as Doubtful3 [LLM_ONLY] (`rule_06`) | `asset_classification, provisioning_percentage` | Classifies the account as Doubtful3 if doubtful since days are not greater than 1095. |
...
| 🟠 8 | Handle NO_DATA_FOUND exception [LLM_ONLY] (`rule_08`) | `Not specified` | Handles the NO_DATA_FOUND exception by inserting an error record into the NPA_AUDIT_LOG table and raising the exception. |
| 🟠 9 | Handle OTHERS exception [LLM_ONLY] (`rule_09`) | `Not specified` | Handles the OTHERS exception by inserting an error record into the NPA_AUDIT_LOG table and raising the exception. |
...
| 7 | Calculate provisioning amount (rule_07) | v_provision_amt := (v_outstanding_amt - v_unsecured_amt) * (v_provision_pct / 100) + (v_unsecured_amt * ((v_provision_pct + 10) / 100)) | Not cited | Not cited | Not cited | Needs Review |
| 8 | Handle NO_DATA_FOUND exception (rule_08) | WHEN NO_DATA_FOUND THEN INSERT INTO NPA_AUDIT_LOG (account_id, old_classification, new_classification, changed_on) VALUES (p_account_id, NULL, 'ACCOUNT_NOT_FOUND', SYSDATE); RAISE; | Not cited | Not cited | Not cited | Needs Review |
...
- `LLM_ONLY` rule (`recon_2f61ed17021d`): no direct provenance - No deterministic evidence was found for this claim.
- `LLM_ONLY` rule (`recon_831a9be17f5a`): no direct provenance - No deterministic evidence was found for this claim.
- `LLM_ONLY` rule (`recon_78ea4697b1da`): no direct provenance - No deterministic evidence was found for this claim.
- `LLM_ONLY` rule (`recon_e29debc033cb`): no direct provenance - No deterministic evidence was found for this claim.
