...
| Run Timestamp | `2026-09-03T22:53:14.246636+00:00` |
| Object ID | `obj_4cb17b45df7c` |
...
|---|---:|---:|---:|---:|---|
| extraction | 1 | 1 | 0 | 4560 | available |
| synthesis | 1 | 1 | 0 | 11556 | available |
| synthesis_revision | 1 | 1 | 0 | 15172 | available |
...
| 🟠 3 | Classify account as Doubtful1 [LLM_ONLY] (`rule_03`) | `asset_classification, provisioning_percentage` | Classifies the account as Doubtful1 if overdue days are between 366 and 1095 and doubtful since days are 365 or less. |
| 🟠 4 | Classify account as Doubtful2 [LLM_ONLY] (`rule_04`) | `asset_classification, provisioning_percentage` | Classifies the account as Doubtful2 if overdue days are between 366 and 1095 and doubtful since days are more than 365. |
| 🟠 5 | Classify account as Loss [LLM_ONLY] (`rule_05`) | `asset_classification, provisioning_percentage` | Classifies the account as Loss if overdue days are more than 1095 and doubtful since days are more than 1095. |
| 🟠 6 | Classify account as Doubtful3 [LLM_ONLY] (`rule_06`) | `asset_classification, provisioning_percentage` | Classifies the account as Doubtful3 if overdue days are more than 1095 and doubtful since days are 1095 or less. |
...
| 🟠 8 | Handle NO_DATA_FOUND exception [LLM_ONLY] (`rule_08`) | `Not specified` | Handles the NO_DATA_FOUND exception by inserting a record into the NPA_AUDIT_LOG table. |
| 🟠 9 | Handle OTHERS exception [LLM_ONLY] (`rule_09`) | `Not specified` | Handles the OTHERS exception by inserting a record into the NPA_AUDIT_LOG table and raising the exception. |
...
| 5 | Classify account as Loss (rule_05) | v_overdue_days > 1095 AND v_doubtful_since > 1095 | Not cited | Not cited | Not cited | Needs Review |
| 6 | Classify account as Doubtful3 (rule_06) | v_overdue_days > 1095 AND NOT (v_doubtful_since > 1095) | Not cited | Not cited | Not cited | Needs Review |
...
- `LLM_ONLY` rule (`recon_b6a48bb1e71b`): no direct provenance - No deterministic evidence was found for this claim.
...
- Could not trace the stated source evidence back to a successfully parsed technical extraction record: v_overdue_days > 1095 AND NOT (v_doubtful_since > 1095)
